# -*- coding: utf-8 -*-
"""事件腿细分：回购 vs 增持（反转 ACCUM/DOWNTREND 中）alpha 差异"""
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    cands.append({"entry_date": bs[i + 1]["t"], "typ": typ, "deep": deep,
...
print("\n=== 回购 vs 增持（反转事件腿）===")
report("全部反转事件", cands)
report("回购", [c for c in cands if c["typ"] == "BUYBACK"])
report("增持", [c for c in cands if c["typ"] == "INCREASE"])
report("回购+DEEP", [c for c in cands if c["typ"] == "BUYBACK" and c["deep"]])
report("增持+DEEP", [c for c in cands if c["typ"] == "INCREASE" and c["deep"]])
