# -*- coding: utf-8 -*-
"""Finalize v10: update dashboard/registry/reports."""
...
lines = ["# 组合 v10（行为 DNA + 深度依赖持有）每年/每月报告", ""]
lines.append("> SMC（跟拉升）+ 事件（逆势吸筹，DEEP 20日/非DEEP 10日）—— 深度依赖执行")
lines.append("")
lines.append("## 逐年")
...
lines.append("")
lines.append("## 稳健性")
lines.append("- 行为阶段阈值 7 变体、深度阈值 5 变体、持有期 6 变体：全部每年正收益")
lines.append("- 无未来：行为/深度 entry 前 PIT，SMC outcome-free，事件披露后入场")
with open(r"E:\test\smc_project\research\组合v10逐年逐月报告.md", "w", encoding="utf-8") as fh:
    fh.write("\n".join(lines))
print("v10 report written")

# dashboard
dash = {
    "updated": "2026-08-18",
    "version": "COMBO_V10_BEHAVIOR_DNA_DEPTH",
    "total_trades": len(trades),
    "yearly": yearly,
    "monthly": monthly,
    "note": "组合v10 = SMC(跟拉升) + 事件(逆势吸筹，DEEP20日/非DEEP10日深度依赖持有)",
}
old = {}
try:
...
reg["research_candidates"]["COMBO_SMC_EVENT"].update({
    ...
    "upgrade_note": "v10 深度依赖持有：DEEP吸筹20日/非DEEP10日，总体 avg +4.44%/PF 2.72，6变体稳健",
})
with open(reg_p, "w", encoding="utf-8") as fh:
    json.dump(reg, fh, ensure_ascii=False, indent=2)
print("registry v10 recorded")
for y in yearly:
