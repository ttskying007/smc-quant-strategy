# -*- coding: utf-8 -*-
"""买点深入研究：事件信号后入场时机/位置对比
T+1开盘 vs T+1最低 vs T+2开盘 vs T+1收盘 → 找最优入场（价格合理性验证）"""
...
for date, code, title in cur.fetchall():
    ...
    if i + 16 >= len(bs):
        continue
    events.append({"bs": bs, "i": i, "entry_date": bs[i + 1]["t"]})
conn.close()
print("事件:", len(events))

# entry mode comparison: entry price = T+1 open / T+1 low / T+2 open / T+1 close, exit = T+15 close
modes = {"T+1开盘": lambda b, k: b[k]["o"], "T+1最低": lambda b, k: b[k]["l"],
         "T+1收盘": lambda b, k: b[k]["c"], "T+2开盘": lambda b, k: b[k + 1]["o"],
         "T+3开盘": lambda b, k: b[k + 2]["o"]}
...
for name, f in modes.items():
    ...
    for e in events:
        k = e["i"] + 1
        if name == "T+2开盘":
            if k + 1 >= len(e["bs"]) or k + 1 + 15 >= len(e["bs"]):
                continue
        elif name == "T+3开盘":
            if k + 2 >= len(e["bs"]) or k + 2 + 15 >= len(e["bs"]):
                continue
        else:
            if k + 15 >= len(e["bs"]):
                continue
        ...
        if ep <= 0:
            continue
        exit_i = (k + 1 + 15 if name == "T+2开盘" else (k + 2 + 15 if name == "T+3开盘" else k + 15))
        rs.append({"entry_date": e["entry_date"], "net_pnl_pct": round((e["bs"][exit_i]["c"] / ep - 1) * 100 - 0.20, 4)})
    for t in rs:
        t["t1_violation"] = "False"
        t["year"] = str(t["entry_date"])[:4]
    gate = check_economic_gate(rs)
    o = gate["overall"]
    line = f"{name}: n={o['n']} WR={o['wr']}% avg={o['avg']}% PF={o['pf']}"
