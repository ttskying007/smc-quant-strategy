# -*- coding: utf-8 -*-
"""终极组合验证：皇冠（rank≥6）× 回踩买点 × 分层 TP/SL
全部最优特征叠加的完整效果（买入/卖出/排序最优组合）"""
...
for date, code, title in cur.fetchall():
    ...
    v2_ratio = bs[entry_idx + 1]["v"] / avg_v if avg_v > 0 else 0
    stage_span = 0
    ...
    events.append({"bs": bs, "entry_idx": entry_idx, "ep_open": ep_open, "disc_close": disc_close,
                   "entry_date": bs[entry_idx]["t"], "rank": rs,
...
def run(rank_min, retrace):
    ...
    for e in events:
        ...
        if retrace:
            ...
            ep = limit if e["bs"][e["entry_idx"]]["l"] <= limit else e["ep_open"]
        else:
            ep = e["ep_open"]
...
print("\n=== 终极组合（皇冠 × 回踩 × 分层）===")
report("皇冠(rank≥6) 开盘+分层", run(6, False))
report("皇冠(rank≥6) 回踩+分层", run(6, True))
report("rank≥5 回踩+分层", run(5, True))
report("rank≥4 回踩+分层", run(4, True))