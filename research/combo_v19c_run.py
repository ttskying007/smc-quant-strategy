# -*- coding: utf-8 -*-
"""延续腿更高端变体（VWAP5%+高波动 +3.94%）+ v18 组合测试"""
...
# collect all MARKUP support-retrace with features, filter variants later
all_rows = []
n = 0
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 2):
        ...
        all_rows.append({"symbol": sym, "entry_date": daily[entry_idx]["t"], "src": "CONT",
                         "dev": dev, "vol20": vol20, "net_pnl_pct": round(pnl - FEE, 4)})
    if n % 1500 == 0:
        print(f"  {n} files, all_rows {len(all_rows)}", flush=True)
print("all rows:", len(all_rows))
vols = sorted(r["vol20"] for r in all_rows)
vmed = vols[len(vols) // 2]
cont_hi = [r for r in all_rows if r["dev"] >= 0.05 and r["vol20"] > vmed]
print("延续腿(VWAP5%+高波动):", len(cont_hi))
