# -*- coding: utf-8 -*-
"""子信号跨度 vs 收益：阶段确认跨度（新进入 vs 长期）和 ADX 跨度分组的事件表现
验证信号时效性（跨度久是否衰减）→ 自动化调整"""
...
for date, code, title in cur.fetchall():
    ...
    if adx is None or adx < 20:
        continue
    entry_idx = i + 1
    if entry_idx + 16 >= len(bs) or entry_idx < 120:
        continue
    ...
    if ep <= 0:
        continue
    # stage span: how long stage held before signal
    stage_span = 0
...
print("\n=== 阶段跨度分组（信号时效性）===")
for lo, hi, label in ((0, 5, "新进入(0-5天)"), (6, 15, "中期(6-15天)"), (16, 100, "长期(>15天)")):
    rs = [t for t in events if lo <= t["stage_span"] <= hi]
    report(f"阶段{label}", rs)

print("\n=== ADX 跨度分组 ===")
for lo, hi, label in ((0, 5, "ADX新(0-5天)"), (6, 15, "ADX中(6-15天)"), (16, 100, "ADX久(>15天)")):
    rs = [t for t in events if lo <= t["adx_span"] <= hi]
    report(f"{label}", rs)
