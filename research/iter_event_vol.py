# -*- coding: utf-8 -*-
"""事件腿 + 入场日量能：T+1 放量（大资金响应）vs 缩量（无响应）"""
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    # entry day volume ratio (T+1 vs prev 20d avg)
    if i + 1 < 20:
        continue
    avg_v = sum(bs[k]["v"] for k in range(i + 1 - 20, i + 1)) / 20
    v_ratio = bs[i + 1]["v"] / avg_v if avg_v > 0 else 1
    cands.append({"entry_date": bs[i + 1]["t"], "v_ratio": v_ratio,
...
print("\n=== 事件腿 + 入场日量能 ===")
report("全部", cands)
report("放量(v>1.2x)", [c for c in cands if c["v_ratio"] > 1.2])
report("中等(0.8-1.2)", [c for c in cands if 0.8 <= c["v_ratio"] <= 1.2])
report("缩量(v<0.8)", [c for c in cands if c["v_ratio"] < 0.8])
