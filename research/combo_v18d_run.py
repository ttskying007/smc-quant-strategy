# -*- coding: utf-8 -*-
"""v18d: hybrid event exit - hold up to 15d BUT exit early at structural TP
(pre-entry 60d high pool) if reached. SL = event-day low *0.98 (wide protection).
This keeps the time-confirmation alpha while capturing structural targets."""
...
def replay_hybrid(bs, i, ep):
    """Hold up to HOLD_CAP; exit early at structural TP; SL = event low * 0.98."""
    sl = bs[i]["l"] * 0.98
    if sl >= ep:
        return None
    highs = []
    ...
    if not highs:
        return None
    tp = max(highs)
    exit_price, reason, hold = ep, "HOLD_EXIT", 0
    for k in range(i + 1, min(len(bs), i + HOLD_CAP + 1)):
        bb = bs[k]
        hold += 1
        hi, lo, cl = bb["h"], bb["l"], bb["c"]
        if lo <= sl:
            exit_price, reason = sl, "SL_PROTECT"
            break
        if hi >= tp:
        ...
        exit_price = cl
    if reason == "HOLD_EXIT":
        exit_price = bs[min(len(bs), i + HOLD_CAP) - 1]["c"]
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0 or i + 15 >= len(bs):
        continue
    fixed.append({"entry_date": bs[i]["t"],
                  "net_pnl_pct": round((bs[i + 15]["c"] / ep - 1) * 100 - 0.20, 4), "t1_violation": "False"})
    r = replay_hybrid(bs, i, ep)
    if r:
        pnl, reason, hold = r
        hybrid.append({"entry_date": bs[i]["t"], "net_pnl_pct": pnl, "reason": reason,
                       "hold": hold, "t1_violation": "False"})
conn.close()
print(f"fixed: {len(fixed)}, hybrid: {len(hybrid)}")
print("hybrid exit:", dict(Counter(t["reason"] for t in hybrid)))
