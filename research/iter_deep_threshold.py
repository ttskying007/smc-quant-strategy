# -*- coding: utf-8 -*-
"""事件腿 DEEP 阈值敏感性：ret90 < -0.15 / -0.20（当前）/ -0.25（+量比<0.75）"""
...
for date, code, title in cur.fetchall():
    ...
    if d8 not in dates:
        continue
    i = dates.index(d8)
    st = stage_detailed(bs, i)
    if st not in ("ACCUM", "DOWNTREND"):
        continue
    adx = adx14(bs, i)
    if adx is None or adx < 20 or i < 90:
        continue
    if i + 26 >= len(bs):
        continue
    ep = bs[i + 1]["o"]
    ...
    cands.append({"entry_date": bs[i + 1]["t"], "ret90": ret90, "vt90": vt90, "i": i + 1, "bs": bs, "ep": ep})
conn.close()
...
def run(deep_ret, hold):
    rs = []
    for c in cands:
        is_deep = c["ret90"] < deep_ret and c["vt90"] < 0.75
        h = hold if is_deep else 15
        if c["i"] + h < len(c["bs"]):
            rs.append({"entry_date": c["entry_date"],
                       "net_pnl_pct": round((c["bs"][c["i"] + h]["c"] / c["ep"] - 1) * 100 - 0.20, 4)})
    return rs


print("\n=== 事件腿 DEEP 阈值敏感性（DEEP 持有 20 日）===")
report("ret90<-0.15（浅DEEP）", run(-0.15, 20))
report("ret90<-0.20（当前）", run(-0.20, 20))
report("ret90<-0.25（深DEEP）", run(-0.25, 20))
report("ret90<-0.30（极深）", run(-0.30, 20))
