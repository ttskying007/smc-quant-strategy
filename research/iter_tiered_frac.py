# -*- coding: utf-8 -*-
"""分层 TP/SL 参数深化：TP1 部分平仓比例（40%/50%/60%）+ TP2 触发逻辑
事件腿分层出场最优参数"""
...
for date, code, title in cur.fetchall():
    ...
    if not highs or not lows:
        continue
    highs.sort()
    events.append({"bs": bs, "entry_idx": entry_idx, "ep": ep,
                   "tp1": highs[0], "tp2": highs[1] if len(highs) > 1 else highs[0] * 1.05,
                   "tp3": highs[-1], "sl1": lows[0] * 0.99,
                   "entry_date": bs[entry_idx]["t"]})
conn.close()
print("事件:", len(events))


def tiered(ev, tp1_frac):
    out = []
    for e in ev:
        ep = e["ep"]
        remaining = 1.0
...
for frac in (0.30, 0.40, 0.50, 0.60, 0.70):
    report(f"TP1平仓{frac:.0%}", tiered(events, frac))
