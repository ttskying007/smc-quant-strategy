# -*- coding: utf-8 -*-
"""弱市降仓研究：全市场采样代理（200 只 20 日平均涨跌）识别弱市
验证弱市（proxy<阈值）时事件腿收益显著更低 → 降仓依据"""
...
# market proxy: sample 200 stocks' 20d avg return at each event date
# precompute: for each date, avg 20d return of 200 sampled stocks
import random
random.seed(42)
all_codes = list(code2file.keys())
sample = random.sample(all_codes, min(200, len(all_codes)))
proxy_cache = {}
def market_proxy(d8):
    if d8 in proxy_cache:
        return proxy_cache[d8]
    rets = []
    for code in sample:
        bs = bars_of(code)
        if not bs:
    ...
    v = sum(rets) / len(rets) if rets else 0
    proxy_cache[d8] = v
    return v
...
print("\n=== 全市场代理（200只20日）分组 ===")
report("全部（基准）", events)
for thr, label in ((-0.02, "弱市(proxy<-2%)"), (-0.01, "偏弱(proxy<-1%)"), (0.0, "中性(proxy<0)"), (0.02, "强市(proxy>2%)")):
    rs = [t for t in events if t["proxy"] < thr] if thr <= 0 else [t for t in events if t["proxy"] > thr]
    report(f"{label}", rs)
