# -*- coding: utf-8 -*-
"""v18: v17 + SMC leg VWAP dev>=5% filter (validated +4.32% SMC leg).
Full combo test: SMC(R20+stage+FVG+VWAP5%) + events(15/20d, ADX)."""
...
# events (v17: ACCUM/DOWNTREND + ADX>=20 + 15/20d)
ev = []
seen = set()
...
for t in trades:
    ...
    if st not in ("UPTREND", "MARKUP"):
        continue
    if not any(bs[k]["h"] < bs[k - 2]["l"] for k in range(max(3, i - 12), i)):
        continue
    # VWAP dev >= 5%
    if i < 20:
        continue
...
# save
with open(r"E:\test\smc_project\research\combo_v18_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w18 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src"])
    w18.writeheader()
    for t in combined:
        w18.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?")})
print("combo_v18 saved")
conn.close()
