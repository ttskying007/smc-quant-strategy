# -*- coding: utf-8 -*-
"""强市过滤验证：proxy>2% 事件降仓/跳过后的组合级表现
（修正方向：弱市是抄底甜蜜区，强市无 alpha 应过滤）"""
...
import random
random.seed(42)
sample = random.sample(list(code2file.keys()), min(200, len(code2file)))
proxy_cache = {}
def market_proxy(d8):
...
print("\n=== 强市过滤组合级验证 ===")
report("全部（基准）", events)
report("过滤 proxy>2%", [t for t in events if t["proxy"] <= 0.02])
report("过滤 proxy>1%", [t for t in events if t["proxy"] <= 0.01])
report("过滤 proxy>3%", [t for t in events if t["proxy"] <= 0.03])
