# -*- coding: utf-8 -*-
"""事件 + 业绩预告叠加：同股 30 日内增持/回购 + 业绩预增（内部人+基本面双确认）"""
...
# event dates + perf dates per code
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    # perf within 30 days before or after event
    perfs = perf_map.get(code, [])
    has_perf = any(abs(int(d) - int(p)) <= 30 for p in perfs)
    cands.append({"entry_date": bs[i + 1]["t"], "has_perf": has_perf,
                  "net_pnl_pct": round((bs[i + 1 + hold]["c"] / ep - 1) * 100 - 0.20, 4)})
conn.close()
print("事件信号:", len(cands))
...
print("\n=== 事件 + 业绩预告叠加 ===")
report("事件全部", cands)
report("+30日内业绩预告（双确认）", [c for c in cands if c["has_perf"]])
report("无业绩预告", [c for c in cands if not c["has_perf"]])
