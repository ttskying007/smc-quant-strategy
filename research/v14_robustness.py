# -*- coding: utf-8 -*-
"""v14 robustness: vol threshold sensitivity (p40/p50/p60) for event high-vol filter.
Also check 2025 sample adequacy."""
...
# event candidates with vol + depth-hold pnl (ACCUM/DOWNTREND + ADX>=20)
cands = []
seen = set()
...
def run(pct):
    th = vols[int(len(vols) * pct)]
    ev = [{"entry_date": c["entry_date"], "net_pnl_pct": c["net_pnl_pct"], "t1_violation": "False"} for c in cands if c["vol"] > th]
    for t in ev:
        t["year"] = str(t["entry_date"])[:4]
    gate = check_economic_gate(ev)
    o = gate["overall"]
    line = f"vol>p{int(pct*100)}: n={o['n']} avg={o['avg']:+.2f}% PF={o['pf']} WR={o['wr']}%"
    for y in ("2024", "2025", "2026"):
        ys = [t for t in ev if t["year"] == y]
        if ys:
            line += f" | {y}:n={len(ys)}:{sum(t['net_pnl_pct'] for t in ys)/len(ys):+.2f}%"
    print(line)
