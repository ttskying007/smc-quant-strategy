...
def limit_pct_for(code):
    ...
    ⚠ R23(第八轮审计 P1-12) 诚实修正: 本函数 **不含 ST 判定** —— 只接收
    code, 无名称/ST 标记入参, "ST 5%" 不在本层实现(旧 docstring 的
    "ST 5%（由调用方在名称中判定, 此处按代码）"表述有歧义, 已删)。调用方
    (realtime_prices/profile)需 ST 5% 时必须自行按名称判定后再传入显式
    limit_pct; 未传则 ST 股按板块默认幅度(10/20/30%)——这是**已知保守
    偏差**(ST 实际 5% 涨跌停比板块默认更紧, 保守方向=更难触发限价判定,
    不会产生虚假成交机会), 待 ST 名称数据源接入后由调用方修正。
    返回 0.10 / 0.20 / 0.30。"""
    s = str(code or "")
