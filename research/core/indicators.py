...
- **adx14_of(bars, i)** = Wilder 平滑 ADX(14), 生产(paper_sim EVENT 腿)在用,
  2026-09-12 修复(旧单窗 DX 判定噪声大、双向出错 —— 既错杀好票又误收差票;
  R38o 复核实测: 旧版错杀 610 笔(PF4.01) + 误收 979 笔(PF1.71), Wilder 池
  avg+3.50→+4.56% / PF3.19→4.03。注: 旧注释曾称"系统性偏低", 经 fixture 级
  复核不成立(实测 Wilder 反而更低、旧版通过更多), 已修正为"噪声大"机制描述);
...
def adx14_of(bs, i):
    ...
    旧实现返回单窗 DX(|PDI-MDI|/(PDI+MDI)), 不是 ADX(DX 的平滑均值), 判定
    **噪声大、双向出错**: R38n 在 gen_v20f 同口径实测, 旧版错杀 610 笔好票
    (Wilder 救回 avg+4.15%/PF4.01) + 误收 979 笔差票(avg+1.19%/PF1.71);
    切到 Wilder 后池 n 1642→1547(-6%), avg +3.50→+4.56%, PF 3.19→4.03。

    ⚠ 修正记录(R38o): 本节曾称"旧版系统性偏低(0.33/5.78/5.45 vs 13.06/7.55/
    13.46)", 经 fixture 级复核**不成立** —— 合成序列两版几乎一致, 真实数据
    n=120 Wilder-legacy 中位 -3.66(Wilder 更低), 通过率 legacy 65.0% > Wilder
    56.7%(旧版通过更多), 样例值无法复现。正确机制是"噪声大"而非"偏低"。
    结论(应使用 Wilder)不变, 证据是质量分析而非通过率。

    新实现: Wilder 平滑 TR/+DM/-DM → PDI/MDI → DX → ADX(DX 的 14 期平滑)。"""
    n = 14
    need = n * 2 + 2
    if i < need:
        return None
