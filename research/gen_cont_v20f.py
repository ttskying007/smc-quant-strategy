# -*- coding: utf-8 -*-
"""P2-1b: 生成新延续腿 CSV（VWAP10% + 支撑新鲜度≤5，含 symbol）"""
...
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 11):
        ...
        sigs.append({"symbol": sym, "entry_date": daily[entry_idx]["t"],
                     "src": "CONT", "net_pnl_pct": round((daily[entry_idx + 10]["c"] / ep - 1) * 100 - 0.20, 4)})
...
with open(r"E:\test\smc_project\research\cont_v20f_new.csv", "w", encoding="utf-8-sig", newline="") as fh:
    ...
    for s in sigs:
        w.writerow(s)
print(f"新延续腿 CSV: {len(sigs)} 笔 → cont_v20f_new.csv")
