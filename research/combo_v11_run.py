# -*- coding: utf-8 -*-
"""Test bearish-FVG filter on SMC leg within v10 combo.
SMC leg: TP2-R20 + UPTREND/MARKUP + bearish-FVG presence. Events unchanged.
Compare vs v10 baseline."""
...
# events (v10: strong + ACCUM/DOWNTREND + depth-hold)
ev = []
seen = set()
...
# SMC leg with optional FVG filter
trades = []
...
for t in trades:
    ...
    if st in ("UPTREND", "MARKUP"):
        smc_base.append(t)
        if has_bear_fvg(bs, i):
            smc_fvg.append(t)
print(f"SMC base: {len(smc_base)}, with bear-FVG: {len(smc_fvg)}")
...
print("\n=== v10 + 熊市FVG（SMC腿）===")
combo_report("v10 基线（SMC无FVG）", smc_base)
combo_report("v10 + 熊市FVG过滤", smc_fvg)
conn.close()
