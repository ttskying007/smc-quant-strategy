# -*- coding: utf-8 -*-
"""回购方案×周一叠加：新信息（回购方案/增持计划）+ 周一披露（两个最强事件特征）"""
...
for date, code, title in cur.fetchall():
    ...
    try:
        ...
        wd = -1
    cands.append({"entry_date": bs[i + 1]["t"], "mon": wd == 0,
                  "net_pnl_pct": round((bs[i + 1 + hold]["c"] / ep - 1) * 100 - 0.20, 4)})
conn.close()
print("新信息事件:", len(cands))
...
print("\n=== 新信息 × 周一叠加 ===")
report("新信息事件（全部）", cands)
report("新信息 + 周一", [c for c in cands if c["mon"]])
report("新信息 + 非周一", [c for c in cands if not c["mon"]])
