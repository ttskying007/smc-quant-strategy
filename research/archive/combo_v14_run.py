# -*- coding: utf-8 -*-
"""v14: SMC leg (low-vol, RANGE, bearFVG, UPTREND) + event leg (high-vol, trend, ACCUM/DOWNTREND, depth-hold).
Three-dimension complement: SMC reversal wants low-vol+range; events want high-vol+trend."""
...
# events: ACCUM/DOWNTREND + ADX>=20 + HIGH vol (>median) + depth-hold
ev = []
seen = set()
vols_all = []
event_candidates = []
...
for date, code, title in cur.fetchall():
    ...
    v20 = sum(b["v"] for b in w20) / len(w20)
    v90 = sum(b["v"] for b in w90) / len(w90)
    deep2 = ret90 < -0.20 and (v20 / v90 if v90 else 1) < 0.75
    hold = 20 if deep2 else 10
    if i + hold >= len(bs):
        continue
    ep = bs[i]["o"]
    if ep <= 0:
        continue
    event_candidates.append({"entry_date": bs[i]["t"], "vol": v,
                             "net_pnl_pct": round((bs[i + hold]["c"] / ep - 1) * 100 - 0.20, 4)})
    vols_all.append(v)
vmed = sorted(vols_all)[len(vols_all) // 2]
for e in event_candidates:
    if e["vol"] > vmed:
        ev.append({"symbol": "?", "entry_date": e["entry_date"], "net_pnl_pct": e["net_pnl_pct"], "src": "EVENT"})
print(f"events (high-vol): {len(ev)}/{len(event_candidates)}, volmed={vmed:.4f}")

# SMC: UPTREND/MARKUP + bearFVG + RANGE + low-vol
trades = []
...
for t in trades:
    ...
    if st not in ("UPTREND", "MARKUP"):
        continue
    if not has_bear_fvg(bs, i, 12):
        continue
    adx = adx14(bs, i)
    if adx is not None and adx < 20:
        smc_v13.append(t)
        v = vol20_at(bs, i)
        if v is not None and v <= vmed:
            smc.append(t)
print(f"SMC v13: {len(smc_v13)}, v14 (low-vol): {len(smc)}")
...
with open(r"E:\test\smc_project\research\combo_v14_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w14 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src"])
    w14.writeheader()
    for t in combined:
        w14.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?")})
print("combo_v14 saved")
conn.close()
