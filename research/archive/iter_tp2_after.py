# -*- coding: utf-8 -*-
"""TP2 触发后价格行为：TP2 后继续到 TP3（runner）vs 回落至 TP1（SL 锁利）
验证"TP2 后 SL 锁 TP1"设计的正确性"""
...
stats = {"hit_tp2": 0, "continued_tp3": 0, "retraced_tp1": 0}
...
for date, code, title in cur.fetchall():
    ...
    # find TP2 hit day (must be after TP1 hit)
    tp1_day = None
    tp2_day = None
    for k in range(entry_idx + 1, min(len(bs), entry_idx + 16)):
        if tp1_day is None and bs[k]["h"] >= tp1:
            tp1_day = k
        if tp1_day is not None and bs[k]["h"] >= tp2:
            tp2_day = k
            break
    if tp2_day is None:
        continue
    stats["hit_tp2"] += 1
    # after TP2: reach TP3 before dropping to TP1 (SL lock)?
    reached_tp3 = False
    hit_tp1_sl = False
    for k in range(tp2_day + 1, min(len(bs), entry_idx + 16)):
        if bs[k]["h"] >= tp3:
            reached_tp3 = True
            break
        if bs[k]["l"] <= tp1:
            ...
            break
    if reached_tp3:
        stats["continued_tp3"] += 1
    if hit_tp1_sl:
        stats["retraced_tp1"] += 1
conn.close()

print("=== TP2 触发后价格行为（事件腿）===\n")
n = stats["hit_tp2"]
print(f"TP2 触发样本: {n}")
if n:
    print(f"  继续到 TP3: {stats['continued_tp3']} ({100*stats['continued_tp3']/n:.0f}%)")
    print(f"  回落至 TP1（SL锁利）: {stats['retraced_tp1']} ({100*stats['retraced_tp1']/n:.0f}%)")
    print(f"  两者皆无(持有到期): {n - stats['continued_tp3'] - stats['retraced_tp1']} ({100*(n - stats['continued_tp3'] - stats['retraced_tp1'])/n:.0f}%)")
