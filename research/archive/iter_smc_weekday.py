...
for p in sorted(os.listdir(KT)):
    ...
    for sd in we.build_seeds(sym, daily):
        ...
        if not tr:
            continue
        ed = str(tr["entry_date"])
        try:
            wd = datetime.date(int(ed[:4]), int(ed[4:6]), int(ed[6:8])).weekday()
        except Exception:
            wd = -1
        rows.append({"entry_date": ed, "wd": wd, "net_pnl_pct": tr["net_pnl_pct"], "t1_violation": "False"})
    if n % 1500 == 0:
...
report("全部", rows)
for wd in range(5):
    report(f"{names[wd]}入场", [r for r in rows if r["wd"] == wd])
