# -*- coding: utf-8 -*-
"""多周期三重过滤：周线down + 阶段6-15天 + ADX>15天 的事件表现
验证三重叠加的精选效果（自动化不同周期组合）"""
...
for date, code, title in cur.fetchall():
    ...
    events.append({"entry_date": bs[entry_idx]["t"],
                   ...
                   "weekly": wt, "stage_span": stage_span, "adx_span": adx_span,
                   "stage": st})
conn.close()
print("事件:", len(events))
...
print("\n=== 多周期三重过滤 ===")
report("全部（基准）", events)
report("周线down", [t for t in events if t["weekly"] == "down"])
report("周线down+阶段6-15", [t for t in events if t["weekly"] == "down" and 6 <= t["stage_span"] <= 15])
report("周线down+阶段6-15+ADX>15", [t for t in events if t["weekly"] == "down" and 6 <= t["stage_span"] <= 15 and t["adx_span"] > 15])