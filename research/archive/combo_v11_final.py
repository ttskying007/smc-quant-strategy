# -*- coding: utf-8 -*-
"""Finalize v11: rebuild full combo (SMC+bearFVG + events depth-hold), update dashboard/registry."""
...
for t in trades:
    ...
    if st in ("UPTREND", "MARKUP") and has_bear_fvg(bs, i, 12):
        smc.append(t)
print(f"SMC (UPTREND/MARKUP+bearFVG): {len(smc)}, events: {len(ev)}")
...
for y in ("2024", "2025", "2026"):
    ...
    if ys:
        ...
        yrs[y] = {"n": len(ys), "wr": round(100*w/len(ys), 1), "avg": round(sum(t['net_pnl_pct'] for t in ys)/len(ys), 3), "pf": round(gp/gl, 2) if gl else 0}
        print(f"  {y}: n={len(ys)} WR={100*w/len(ys):.0f}% avg={sum(t['net_pnl_pct'] for t in ys)/len(ys):+.2f}% PF={gp/gl if gl else 0:.2f}")

# save
with open(r"E:\test\smc_project\research\combo_v11_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w11 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src"])
    w11.writeheader()
    for t in combined:
        w11.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?")})
print("combo_v11 saved")
conn.close()
