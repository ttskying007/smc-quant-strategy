# -*- coding: utf-8 -*-
"""大资金持续性：连续放量（T+1/T+2 都放量 vs 单日）的事件表现
验证大资金连续入场（持续追踪）是否更强"""
...
for date, code, title in cur.fetchall():
    ...
    v1 = bs[entry_idx]["v"] / avg_v if avg_v > 0 else 1.0
    v2 = bs[entry_idx + 1]["v"] / avg_v if avg_v > 0 else 1.0
...
print("\n=== 大资金持续性（连续放量）===")
report("全部（基准）", events)
report("单日放量(v1≥2.0)", [t for t in events if t["v1"] >= 2.0])
report("连续放量(v1≥2.0且v2≥1.5)", [t for t in events if t["v1"] >= 2.0 and t["v2"] >= 1.5])
report("连续放量(v1≥1.5且v2≥1.5)", [t for t in events if t["v1"] >= 1.5 and t["v2"] >= 1.5])