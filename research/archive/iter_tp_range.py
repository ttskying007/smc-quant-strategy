# -*- coding: utf-8 -*-
"""TP 范围研究：TP1（swing high）距离分组（近<3% / 中3-8% / 远>8%）的事件收益
验证 TP 范围自动化（近/中/远目标的收益差异）"""
...
print("\n=== TP1 距离分组（范围）===")
report("全部（基准）", events)
for lo, hi, label in ((0, 3, "近(<3%)"), (3, 8, "中(3-8%)"), (8, 100, "远(>8%)")):
    rs = [t for t in events if lo <= t["tp1_dist"] < hi]
    report(f"TP1 {label}", rs)
