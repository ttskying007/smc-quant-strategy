# -*- coding: utf-8 -*-
"""事件入场延迟深化：T+1/T+2/T+3/T+5 入场对比（最优入场窗口）
之前测了 T+1 vs 收盘 vs T+2（T+1 最优）；扩展更晚延迟"""
...
for date, code, title in cur.fetchall():
    ...
    if i + 1 + hold >= len(bs):
        continue
    cands.append({"i": i, "deep": deep, "bs": bs})
conn.close()
...
for delay in (1, 2, 3, 5):
    ...
    for c in cands:
        ...
        if ep <= 0:
            continue
        rs.append({"entry_date": c["bs"][ei]["t"],
                   "net_pnl_pct": round((c["bs"][ei + hold]["c"] / ep - 1) * 100 - 0.20, 4)})
    report(f"T+{delay} 入场", rs)
