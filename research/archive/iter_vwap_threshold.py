# -*- coding: utf-8 -*-
"""Single-dimension tuning: SMC leg VWAP dev threshold 3% vs 5% in full v18 combo.
(Discipline: change ONE dimension, no joint search to avoid overfit.)"""
...
# SMC leg with VWAP threshold param
trades = []
...
def combo_report(label, smc_leg):
    ...
    o = gate["overall"]
    print(f"{label}: n={o['n']} WR={o['wr']}% avg={o['avg']}% PF={o['pf']} payoff={o['payoff']} | SMC腿={len(smc_leg)}")
...
print("\n=== v18 SMC VWAP 阈值（单维度）===")
combo_report("VWAP 3%（样本多）", smc_leg(0.03))
combo_report("VWAP 5%（v18 生产）", smc_leg(0.05))
combo_report("VWAP 7%（更严）", smc_leg(0.07))
conn.close()
