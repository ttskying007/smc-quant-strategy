# -*- coding: utf-8 -*-
"""分层 TP/SL 回测验证：事件腿分层出场（TP1→SL保本/TP2→锁利/TP3→runner）vs 固定15日持有
验证新出场机制是否提升收益（避免止盈过慢/止损过慢）"""
...
# collect events + structure levels
events = []
seen = set()
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    # structure levels: swing highs/lows (nearest)
    highs = []
    lows = []
    ...
    highs.sort()
    tp1 = highs[0]
    tp2 = highs[1] if len(highs) > 1 else tp1 * 1.05
    tp3 = highs[-1]
    sl1 = lows[0] * 0.99
    events.append({"bs": bs, "entry_idx": entry_idx, "ep": ep, "st": st,
                   "tp1": tp1, "tp2": tp2, "tp3": tp3, "sl1": sl1,
                   "entry_date": bs[entry_idx]["t"]})
conn.close()
print("事件:", len(events))


def fixed_hold(ev, hold):
    out = []
    for e in ev:
        i = e["entry_idx"]
        if i + hold >= len(e["bs"]):
            continue
        out.append({"entry_date": e["entry_date"],
                    "net_pnl_pct": round((e["bs"][i + hold]["c"] / e["ep"] - 1) * 100 - 0.20, 4)})
    return out
    ...
    for e in ev:
        ...
        for k in range(e["entry_idx"] + 1, min(len(e["bs"]), e["entry_idx"] + 16)):
            ...
            if not be and bb["h"] >= e["tp1"]:
                pnl += 0.5 * (e["tp1"] / ep - 1) * 100  # 50% at TP1
                remaining = 0.5
                be = True
...
print("\n=== 分层 TP/SL vs 固定持有（事件腿）===")
report("固定15日（当前回测）", fixed_hold(events, 15))
report("分层TP/SL（TP1→TP2→TP3）", tiered_exit(events))
