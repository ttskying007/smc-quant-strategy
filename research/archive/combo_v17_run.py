# -*- coding: utf-8 -*-
"""v17: extend event hold 10->15d to capture the +4% post-exit space found in exec audit.
Exec audit showed all versions sell ~4% early (5d post-exit peak). Test 15d hold."""
...
def event_leg(hold_non_deep, hold_deep):
    ev = []
    seen = set()
    ...
    for date, code, title in cur.fetchall():
        ...
        if adx is None or adx < 20:
            continue
        hold = hold_deep if deep else hold_non_deep
        if i + hold >= len(bs):
            continue
        ep = bs[i]["o"]
        if ep <= 0:
        ...
        ev.append({"symbol": code, "entry_date": bs[i]["t"],
                   ...
                   "t1_violation": "False", "src": "EVENT", "hold": hold})
    return ev


# SMC leg (v16b: UPTREND/MARKUP + bearFVG)
trades = []
...
for t in trades:
    ...
    if st not in ("UPTREND", "MARKUP"):
        continue
    if any(bs[k]["h"] < bs[k - 2]["l"] for k in range(max(3, i - 12), i)):
        smc.append(t)
print("SMC leg:", len(smc))


def combo_report(label, ev_leg):
    combined = smc + ev_leg
    for t in combined:
...
print("\n=== v17（延长持有吃卖早空间）===")
combo_report("v16b基线（10/20日）", event_leg(10, 20))
combo_report("v17a（15/20日）", event_leg(15, 20))
combo_report("v17b（15/25日）", event_leg(15, 25))

# save v17a trades (15/20)
ev17 = event_leg(15, 20)
combined17 = smc + ev17
for t in combined17:
    t.setdefault("t1_violation", "False")
    t["t1_violation"] = str(t.get("t1_violation", "")).lower() in ("true", "1", "yes")
    t["year"] = str(t["entry_date"])[:4]
with open(r"E:\test\smc_project\research\combo_v17_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w17 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src", "hold"])
    w17.writeheader()
    for t in combined17:
        w17.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?"), "hold": t.get("hold", "")})
print("combo_v17 saved")
conn.close()
