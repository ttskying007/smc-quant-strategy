# -*- coding: utf-8 -*-
"""事件腿弱市应对：大盘（沪深300）状态分组 —— 指数 MA20 上/下/超卖
验证指数过滤能否改善弱年（2025）"""
...
# 沪深300 index (000300.SH) trend
idx = bars_of("000300")
idx_dates = [b["t"] for b in idx]

def idx_state(d):
    """index state at date d: above/below MA20, oversold(5d<-5%)"""
    if d not in idx_dates:
        prev = [x for x in idx_dates if x < d]
        if not prev:
            return None
        d = prev[-1]
    i = idx_dates.index(d)
    if i < 20:
        return None
    ma20 = sum(idx[k]["c"] for k in range(i - 20, i)) / 20
    c = idx[i]["c"]
    ret5 = (c / idx[i - 5]["c"] - 1) if i >= 5 else 0
    if c > ma20:
        return "up"
    if ret5 < -0.05:
        return "oversold"  # 超卖（可抄底）
    return "down"
...
print("\n=== 大盘状态分组（事件腿）===")
report("全部（基准）", events)
report("指数up（MA20上）", [t for t in events if t["idx_state"] == "up"])
report("指数down（MA20下）", [t for t in events if t["idx_state"] == "down"])
report("指数超卖(5d<-5%)", [t for t in events if t["idx_state"] == "oversold"])
report("指数down/超卖(过滤up)", [t for t in events if t["idx_state"] != "up"])
