# -*- coding: utf-8 -*-
"""Zone-age test (historical v19 hint): OB just formed then touched (zone_age=1)
may be risky. Our three-TF: touch after OB. Test touch->OB gap impact."""
...
for p in sorted(os.listdir(KT)):
    ...
    for sd in we.build_seeds(sym, daily):
        ...
        if not tr:
            continue
        # zone age = touch - ob (bars); ob_date -> touch_date
        za = day_diff(sd.get("ob_date"), sd.get("touch_date"))
        rows.append({"symbol": sym, "entry_date": tr["entry_date"], "net_pnl_pct": tr["net_pnl_pct"],
                     "zone_age": za, "t1_violation": "False"})
    if n % 1500 == 0:
        print(f"  {n} files, rows {len(rows)}", flush=True)
print(f"rows: {len(rows)}")
from collections import Counter
print("zone_age 分布:", dict(Counter(r["zone_age"] for r in rows)))
...
print("\n=== Zone age（OB→touch 间隔）===")
valid = [r for r in rows if r["zone_age"] is not None]
report("基线", valid)
report("zone_age=1（OB刚形成即触）", [r for r in valid if r["zone_age"] == 1])
report("zone_age=2", [r for r in valid if r["zone_age"] == 2])
report("zone_age>=3（OB成熟后触）", [r for r in valid if r["zone_age"] >= 3])
report("排除 zone_age=1", [r for r in valid if r["zone_age"] != 1])
