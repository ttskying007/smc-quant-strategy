# -*- coding: utf-8 -*-
"""v15 test: relax SMC leg (drop ADX<20 range filter, keep bearFVG) + v13 events.
Goal: restore SMC leg sample size and contribution (v13 SMC leg only 31 trades)."""
...
# SMC leg relaxed: UPTREND/MARKUP + bearFVG (no ADX filter)
trades = []
...
for t in trades:
    ...
    if st not in ("UPTREND", "MARKUP"):
        continue
    if has_bear_fvg(bs, i, 12):
        smc_relaxed.append(t)
print("SMC relaxed (no ADX):", len(smc_relaxed))

combined = smc_relaxed + ev
for t in combined:
...
# save (v16b config = v15)
with open(r"E:\test\smc_project\research\combo_v15_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w15 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src"])
    w15.writeheader()
    for t in combined:
        w15.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?")})
print("combo_v15 saved")
conn.close()
