# -*- coding: utf-8 -*-
"""行为阶段窗口比较：60 日 vs 90 日 对事件腿阶段识别的影响
ACCUM/DOWNTREND 识别窗口偏移 → 信号数量/质量变化"""
...
def run(window):
    ...
    for date, code, title in cur.fetchall():
        ...
        if ep <= 0:
            continue
        out.append({"entry_date": bs[i + 1]["t"], "stage": st,
...
print("=== 行为阶段窗口 60 vs 90 ===")
r60 = run(60)
r90 = run(90)
print(f"60日窗口: {len(r60)} 信号 | 90日窗口: {len(r90)} 信号")
report("60日窗口", r60)
report("90日窗口", r90)
# ACCUM only
print()
report("60日 ACCUM", [t for t in r60 if t["stage"] == "ACCUM"])
report("90日 ACCUM", [t for t in r90 if t["stage"] == "ACCUM"])
conn.close()