...
for date, code, title in cur.fetchall():
    ...
    cands.append({"entry_date": bs[i + 1]["t"], "mon": wd == 0, "vol": v_ratio > 1.2,
...
print("\n=== 事件最强特征组合 ===")
report("全部事件", cands)
report("周一披露", [c for c in cands if c["mon"]])
report("放量", [c for c in cands if c["vol"]])
report("周一+放量（叠加）", [c for c in cands if c["mon"] and c["vol"]])
report("非周一+非放量", [c for c in cands if not c["mon"] and not c["vol"]])
