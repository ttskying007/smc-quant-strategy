# -*- coding: utf-8 -*-
"""不同周期研究：周线趋势过滤（周线 MA10 上/下行）的事件信号表现
多周期组合（周线方向 + 日线信号）vs 单周期"""
...
def weekly_trend(bs, i):
    """周线趋势：按 5 日聚合周线，MA10 周线上/下行"""
    # aggregate weekly bars
    weeks = []
    cur_w = None
    for k in range(max(0, i - 120), i + 1):
        wk = bs[k]["t"][:6]
        if cur_w is None or bs[k]["t"][:8] // 1 != cur_w[0] // 1:
            pass
    # simple: weekly close series via 5-day buckets from i backward
    closes = []
    j = i
...
for wt in ("up", "down"):
    rs = [t for t in events if t["weekly"] == wt]
    report(f"周线{wt}", rs)
