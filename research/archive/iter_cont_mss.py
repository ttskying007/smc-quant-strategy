# -*- coding: utf-8 -*-
"""延续腿 + 回撤后新高确认（MSS 二次确认：回撤后收盘创 10 日新高 = 趋势延续确认）"""
...
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 15):
        ...
        # MSS confirmation: entry close > 10-day high before entry (breakout confirmed)
        if entry_idx < 11:
            continue
        hi10 = max(daily[k]["h"] for k in range(entry_idx - 10, entry_idx))
        mss_ok = daily[entry_idx]["c"] > hi10
        rows.append({"entry_date": daily[entry_idx]["t"], "vol20": vol20, "mss": mss_ok,
...
report("+MSS新高确认", [r for r in low if r["mss"]])
report("无MSS", [r for r in low if not r["mss"]])
