# -*- coding: utf-8 -*-
"""阶段/ADX 跨度组合验证：阶段 6-15 天 × ADX 久 叠加
验证两个跨度特征的组合精选效果（组合级）"""
...
print("\n=== 阶段/ADX 跨度组合精选 ===")
report("全部（基准）", events)
report("阶段6-15天", [t for t in events if 6 <= t["stage_span"] <= 15])
report("ADX>15天", [t for t in events if t["adx_span"] > 15])
report("阶段6-15 + ADX>15", [t for t in events if 6 <= t["stage_span"] <= 15 and t["adx_span"] > 15])
