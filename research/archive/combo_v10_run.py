# -*- coding: utf-8 -*-
"""Combo v10: events with depth-dependent hold (DEEP=20d, non-DEEP=10d) + SMC (UPTREND/MARKUP).
Execution semantic: deeper accumulation needs longer hold to capture bounce."""
...
# events with depth-dependent hold: DEEP=20d, non-DEEP=10d, stage ACCUM/DOWNTREND
ev = []
seen = set()
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    ev.append({"symbol": code, "entry_date": bs[i]["t"], "hold": hold,
               "net_pnl_pct": round((bs[i + hold]["c"] / ep - 1) * 100 - 0.20, 4), "src": "EVENT"})
print("events (depth-hold):", len(ev))
...
with open(r"E:\test\smc_project\research\combo_v10_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w10 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src", "hold"])
    w10.writeheader()
    for t in combined:
        w10.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?"), "hold": t.get("hold", "")})
print("combo_v10 saved")
conn.close()
