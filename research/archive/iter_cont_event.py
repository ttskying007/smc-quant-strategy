# -*- coding: utf-8 -*-
"""延续腿 + 事件叠加：MARKUP 结构支撑信号 + 该股近20日有增持/回购事件 = 共振"""
...
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 15):
        ...
        ev_codes = ev_map.get(code, set())
        has_ev = any(int(entry_d) - int(e) <= 20 and int(entry_d) >= int(e) for e in ev_codes)
        all_rows.append({"entry_date": daily[entry_idx]["t"], "vol20": vol20, "has_ev": has_ev,
...
print("\n=== 延续腿 + 事件叠加（低波动子集）===")
low = [r for r in all_rows if r["vol20"] < vmed]
report("延续腿低波动（基线）", low)
report("+近20日事件（共振）", [r for r in low if r["has_ev"]])
report("无事件", [r for r in low if not r["has_ev"]])
