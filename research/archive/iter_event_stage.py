# -*- coding: utf-8 -*-
"""UPTREND 阶段事件验证：事件 + UPTREND/MARKUP 阶段（回测）vs ACCUM/DOWNTREND
短期（1-3天）被拒信号（格力UPTREND）表现好 → 测 UPTREND 事件是否也有 alpha"""
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    cands[st].append({"entry_date": bs[i + 1]["t"],
...
print("=== 事件 × 行为阶段（ADX≥20 后）===")
for st in ("ACCUM", "DOWNTREND", "UPTREND", "MARKUP", "DISTRIB"):
    report(f"阶段={st}", cands.get(st, []))
