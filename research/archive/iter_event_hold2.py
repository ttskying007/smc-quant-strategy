# -*- coding: utf-8 -*-
"""事件腿持有期敏感性（v20c 后确认）：非DEEP 10/12/15/18 + DEEP 15/20/25"""
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    cands.append({"entry_date": bs[i + 1]["t"], "deep": deep, "i": i + 1, "bs": bs, "ep": ep})
conn.close()
print("events:", len(cands))


def report(label, nd, dp):
    rs = []
    for c in cands:
        hold = dp if c["deep"] else nd
...
print("\n=== 事件腿持有期敏感性 ===")
report("10/20（旧v16b）", 10, 20)
report("12/20", 12, 20)
report("15/20（v20c当前）", 15, 20)
report("15/25", 15, 25)
report("18/20", 18, 20)
report("18/25", 18, 25)
