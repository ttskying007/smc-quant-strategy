# -*- coding: utf-8 -*-
"""延续腿 MARKUP 强度分层：ret60 20-30% / 30-50% / 50%+ 的延续质量"""
...
def stage_and_ret60(bs, i):
    ...
    if ret60 > 0.30 and vt > 1.3:
        return "DISTRIB", ret60
    if ret60 > 0.20 and vt > 1.1:
        return "MARKUP", ret60
    if ret60 > 0:
        return "UPTREND", ret60
    return "DOWNTREND", ret60
...
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 15):
        st, ret60 = stage_and_ret60(daily, i)
        if st != "MARKUP":
            continue
        sl_tmp = None
        ...
        rows.append({"entry_date": daily[entry_idx]["t"], "vol20": vol20, "ret60": ret60,
...
report("基线（低波动）", low)
report("ret60 20-30%（温和拉升）", [r for r in low if 0.20 <= r["ret60"] < 0.30])
report("ret60 30-50%（中强）", [r for r in low if 0.30 <= r["ret60"] < 0.50])
report("ret60 50%+（强拉升）", [r for r in low if r["ret60"] >= 0.50])
