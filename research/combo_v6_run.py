# -*- coding: utf-8 -*-
"""Combo v6: SMC leg restricted to UPTREND behavior stage (large-money markup phase)
+ strong events. Tests if behavior-DNA filter (track current operator) improves combo."""
...
# reuse behavior_at from iter_behavior_dna (import will run it; avoid by inlining minimal)
import sys as _s
_s.path.insert(0, r"E:\test\smc_project\research")
...
def stage_at(symbol, entry_date):
    ...
    if ret60 < -0.15 and vol_trend < 0.9:
        return "ACCUM"
    if ret60 > 0.20 and vol_trend > 1.1:
        return "MARKUP"
    if ret60 > 0.30 and vol_trend > 1.3:
        return "DISTRIB"
    if ret60 > 0:
        return "UPTREND"
    return "DOWNTREND"
...
for t in trades:
    ...
    if r20 is None or not (0 <= r20 < 0.15):
        continue
    st = stage_at(t["symbol"], str(t["entry_date"]))
    if st == "UPTREND":
        smc_uptrend.append(t)
print("SMC (UPTREND stage):", len(smc_uptrend))
