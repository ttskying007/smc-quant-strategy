# -*- coding: utf-8 -*-
"""组合级 ADX 验证：事件腿 ADX20 vs ADX30（+延续腿）→ v20c 组合对比"""
...
def event_leg(adx_min):
    ...
    for date, code, title in cur.fetchall():
        ...
        if ep <= 0:
            continue
        out.append({"entry_date": bs[i + 1]["t"], "src": "EVENT",
                    "net_pnl_pct": round((bs[i + 1 + hold]["c"] / ep - 1) * 100 - 0.20, 4)})
    return out


# continuation from v20c
cont = []
...
e20 = event_leg(20)
e30 = event_leg(30)
print(f"事件 ADX20: {len(e20)} | ADX30: {len(e30)}")
report("组合(事件ADX20+延续)", combo(e20))
report("组合(事件ADX30+延续)", combo(e30))
conn.close()
