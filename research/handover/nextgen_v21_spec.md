...
1. 行数一致性: 与 v20f 同窗口腿数偏差 0(同一事件集、同一过滤链)
   ⚠ 口径修正(R57 冒烟发现): 数据边缘腿(入场日+17bar 晚于 v20f 生成时点的缓存)
   会合法多出(如 002722.SZ 20260824, 9-16 生成时退出窗未完被跳过) —
   "同窗口"= 排除 v20f 生成后数据边缘新增的腿
2. 无前视: 抽 20 腿人工核对结构事件日期<=信号日; `no_lookahead` 3 用例续绿
3. 基线复现: net_pnl_pct 与 v20f 逐腿相等(引擎未动, 只加列)
   ✅ 冒烟已验: 335 重叠腿 0 不一致(--smoke 400)
4. 新列覆盖率: trend_state/supply_layers 非空率 >95%
   ✅ 冒烟已验: EVENT 腿 18 列全填充(300637/002722 实样), CONT 腿趋势/事件/板块/区间已填, 供给列诚实留空
