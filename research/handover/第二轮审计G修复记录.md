...
## G 项批3-4 更新（G08-G25 完成度）

### 已修复（新增批3/4）
- G08 持有期统一 / G09 公告晚间补拉 / G10 分页到空 / G11 强市仓位系数
- G12 追赶池 / G13 稳健性诊断(HHI 0.369/剔最好月PF0.93) / G14 SL分板块ATR(A/B +1.5pp PF)
- G17 真z-score量能 / G18 涨跌停分板块 / G19 core收敛 / G20 off-by-one
- G21 真WF框架 / G22 rank特征统一 / G24 portfolio层 / G25 事件分类统一

### 关键验证结果（G14 落地全量）
| 指标 | G01/G02后 | G14后(ATR SL) |
|---|---:|---:|
| trades | 928 | 1,199 |
| IS avg/PF | +4.40%/2.30 | +3.95%/2.06 |
| OOS avg/PF | -0.60%/0.88 | -0.94%/0.83 |

→ G14 改善风险结构（信号+29%）但 OOS 仍负：单点修复不改策略无 edge 结论
...
## D5 折价区限价入场实验（2026-09-05）→ 实证净负，已回退

| 指标 | 现状(次根开盘) | D5限价(挂zh等回踩5根) |
|---|---:|---:|
| seeds/300只 | 80 | 39 |
| avg% | +2.31 | +0.25 |
| PF | 1.50 | 1.06 |
...
结论：折价限价导致"强setup涨走不回头（未成交）、弱回踩成交为主" → 质量净负。
与8阶段严格化教训一致：**更严/更折价 ≠ 更好**。已回退，保留次根开盘入场。

## 核心决策点结论（证据链）
1. G01/G02 去伪后 OOS -0.60%（原+4.06%虚增）
2. G14 SL-ATR 改善风险结构但仍 OOS -0.94%
3. G13 剔除最好月后 PF 0.93（转亏）
4. G21 真WF 7段OOS 3正4负
5. D5 折价限价净负（本轮）

→ **当前 SMC 引擎无稳定 OOS edge，两条改进路径(严格/折价)均实证无效**
→ **建议：聚焦事件腿（OOS 曾 +5.43%，cap500 后 PF7.75）**；SMC 腿降为研究级观察
