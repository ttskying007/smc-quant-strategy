# -*- coding: utf-8 -*-
"""v13 robustness: ADX threshold sensitivity (15/20/25) for both legs.
SMC wants ADX<th, events want ADX>=th."""
...
# precompute event records (stage + adx + pnl at depth-hold)
event_recs = []
seen = set()
...
for date, code, title in cur.fetchall():
    ...
    if ep <= 0:
        continue
    event_recs.append({"entry_date": bs[i]["t"], "adx": adx,
                       "net_pnl_pct": round((bs[i + hold]["c"] / ep - 1) * 100 - 0.20, 4)})
print("event recs:", len(event_recs))

# SMC records (stage + adx + pnl)
smc_recs = []
trades = []
...
for t in trades:
    ...
    if adx is None:
        continue
    # bear FVG
    has_fvg = any(bs[k]["h"] < bs[k - 2]["l"] for k in range(max(3, i - 12), i))
    if not has_fvg:
        continue
    smc_recs.append({"entry_date": str(t["entry_date"]), "adx": adx, "net_pnl_pct": t["net_pnl_pct"]})
print("SMC recs:", len(smc_recs))


def run(th):
    ev = [{"entry_date": r["entry_date"], "net_pnl_pct": r["net_pnl_pct"], "t1_violation": "False"} for r in event_recs if r["adx"] >= th]
    smc = [{"entry_date": r["entry_date"], "net_pnl_pct": r["net_pnl_pct"], "t1_violation": "False"} for r in smc_recs if r["adx"] < th]
    combined = smc + ev
    for t in combined:
        t["year"] = str(t["entry_date"])[:4]
    gate = check_economic_gate(combined)
    o = gate["overall"]
    line = f"ADX阈={th}: n={o['n']} avg={o['avg']:+.2f}% PF={o['pf']} WR={o['wr']}%"
