# -*- coding: utf-8 -*-
"""延续腿持有期敏感性补全（8/10/12/15/20 日）—— 卖早 4.85% 提示可能持有更久"""
...
# collect signals with entry info
sigs = []
n = 0
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 25):
        ...
        if (daily[entry_idx]["c"] - vw) / vw < 0.05:
            continue
        sigs.append({"entry_date": daily[entry_idx]["t"], "i": entry_idx, "ep": ep, "daily": daily})
    if n % 1500 == 0:
