# -*- coding: utf-8 -*-
"""Combo v9: SMC (UPTREND/MARKUP) + events in DEEP-accumulation or DOWNTREND stage.
DEEP ACCUM events are strongest (avg +6.49-9.10%). Test full combo."""
...
def stage_and_depth(bs, i):
    """Return (stage, is_deep). stage: ACCUM/DOWNTREND/UPTREND/MARKUP/DISTRIB."""
    if i < 91:
        return None, False
    w90 = bs[i - 90:i]
    w60 = bs[i - 60:i]
...
# events: DEEP-ACCUM or DOWNTREND stage
ev = []
seen = set()
...
for date, code, title in cur.fetchall():
    ...
    if i + 10 >= len(bs):
        continue
    st, deep = stage_and_depth(bs, i)
    if st in ("ACCUM", "DOWNTREND") and (st == "DOWNTREND" or deep):
        ep = bs[i]["o"]
        if ep <= 0:
            continue
        ev.append({"symbol": code, "entry_date": bs[i]["t"],
                   "net_pnl_pct": round((bs[i + 10]["c"] / ep - 1) * 100 - 0.20, 4), "src": "EVENT"})
print("events (DEEP-ACCUM/DOWNTREND):", len(ev))

# SMC leg: UPTREND/MARKUP
trades = []
...
# save
with open(r"E:\test\smc_project\research\combo_v9_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w9 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src"])
    w9.writeheader()
    for t in combined:
        w9.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                     "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                     "year": t.get("year", ""), "src": t.get("src", "?")})
print("combo_v9 saved")
conn.close()
