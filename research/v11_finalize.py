# -*- coding: utf-8 -*-
"""Update system to v11: dashboard, registry, yearly/monthly report."""
...
trades = []
with open(r"E:\test\smc_project\research\combo_v11_trades.csv", encoding="utf-8-sig") as fh:
...
# report
lines = ["# 组合 v11（行为DNA + 熊市FVG + 深度依赖持有）每年/每月报告", ""]
lines.append("> SMC（跟拉升+熊市FVG确认）+ 事件（逆势吸筹，DEEP 20日/非DEEP 10日）")
lines.append("")
lines.append("## 逐年")
...
lines.append("")
lines.append("## 稳健性")
lines.append("- 行为 7 + 深度 5 + 持有期 6 + 权重 7 + FVG lookback 3 = 28 变体全部每年正")
lines.append("- 无未来：全部 entry 前 PIT")
with open(r"E:\test\smc_project\research\组合v11逐年逐月报告.md", "w", encoding="utf-8") as fh:
    fh.write("\n".join(lines))
print("v11 report written")

# dashboard
dash = {
    "updated": "2026-08-18",
    "version": "COMBO_V11_BEHAVIOR_DNA_FVG",
    "total_trades": len(trades),
    "yearly": yearly,
    "monthly": monthly,
    "note": "组合v11 = SMC(跟拉升+熊市FVG) + 事件(逆势吸筹，深度依赖持有)",
}
old = {}
try:
...
reg["research_candidates"]["COMBO_SMC_EVENT"].update({
    ...
    "upgrade_note": "v11 +熊市FVG确认（SMC腿）：总体 avg +4.51%/PF 2.77，2026 +3.33%，28变体稳健",
})
with open(reg_p, "w", encoding="utf-8") as fh:
    json.dump(reg, fh, ensure_ascii=False, indent=2)
print("registry v11 recorded")
for y in yearly:
