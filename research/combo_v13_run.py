# -*- coding: utf-8 -*-
"""v13: SMC leg RANGE(ADX<20) + event leg trend(ADX>=20) + depth-hold.
The two legs complement on ADX axis (reversal wants range, events want trend)."""
...
# events: strong + ACCUM/DOWNTREND + ADX>=20 (trend) + depth-hold
ev = []
seen = set()
...
print("events (trend ADX>=20):", len(ev))

# SMC: UPTREND/MARKUP + bearFVG + RANGE(ADX<20)
trades = []
...
for t in trades:
    ...
    if adx is not None and adx < 20:
        smc.append(t)
print("SMC (UPTREND/MARKUP+bearFVG+RANGE):", len(smc))
...
# save v13 trades
with open(r"E:\test\smc_project\research\combo_v13_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
    w13 = csv.DictWriter(fh, fieldnames=["symbol", "entry_date", "net_pnl_pct", "t1_violation", "year", "src"])
    w13.writeheader()
    for t in combined:
        w13.writerow({"symbol": t.get("symbol", ""), "entry_date": t.get("entry_date", ""),
                      "net_pnl_pct": t.get("net_pnl_pct", 0), "t1_violation": t.get("t1_violation", ""),
                      "year": t.get("year", ""), "src": t.get("src", "?")})
print("combo_v13 saved")
conn.close()
