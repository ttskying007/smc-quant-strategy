...
review = {
    "verdicts": [
        ...
        {"id": "P1-7-MECH-FIX", "name": "P1-7 注释方向复核", "result": "⚠ 机制描述修正(结论仍成立)",
         "detail": "core/indicators.py 注释'旧单窗DX系统性偏低'经 fixture 复核**不成立**: "
                   ...
                   "(双向出错), 非'偏低导致门过严'。核心结论(Wilder更好)仍成立 —— 证据是"
                   "R38n 质量分析(PF3.19→4.03), 非通过率。注释已修正(36b4e71, 零逻辑改动, 回归62库全绿)"},
        {"id": "WILDER-REBASE", "name": "P1-7 Wilder 重基线 A/B", "result": "⭐ 四项判据全过(晋级)",
         "detail": "独立分叉同口径 A/B (n 1974→1861): 胜率 61.3→64.4% | avg +3.83→**+4.60%** | "
                   "中位 +1.51→+2.48% | PF 3.08→**3.56** | MDD -629→**-490** | 累计 +7557→+8557% | "
                   "RR<=-1R 23.5→20.6%。逐年 2024 +3.81/PF3.02 → +4.90/3.94; IS PF 2.90→3.65, "
                   "OOS 3.48→3.36(非劣化)。4月 +2.94/2.16 → +5.26/3.65。"
                   "**R38 迄今幅度最大改进**; 冻结线变更需走完整审计, 不得直接改 gen_v20f.py"},
         {"id": "P1-7-AUDIT", "name": "P1-7 审计路径确认", "result": "⭐ 性质=回测追上生产(风险下调)",
          "detail": "决定性确认: 生产 paper_sim.py L314-321 的 adx14_of 是兼容入口, 委托 "
                    "core.indicators.adx14_of(Wilder); L744-745 EVENT 腿门 = Wilder ADX>=20 "
                    "—— **生产早已在用 Wilder**。冻结基线 gen_v20f 才用 legacy 单窗DX。"
                    "故重基线 = '让回测追上生产'(修正回测口径错误), **生产行为零变化**; "
                    "审计风险等级 中→低; 建议与 P1-8(max_hold 15→12)合并一次性统一两处分叉"},
         {"id": "R38-CONVERGE", "name": "R38 迭代收敛(最终)", "result": "两处改进 + 一处口径修正",
          "detail": "16轮 12+ 假设全部测试完毕。真实改进: ①ACCUM×2 温和加权(PF3.21→3.29) "
                    "②**Wilder 重基线(PF3.08→3.56, MDD -629→-490)** —— 后者性质经确认是"
                    "**修正回测与生产的口径分叉**, 非改变策略。扩池三连否 + stage/rank/退出全部"
                    "验证现状 —— 信号层已达边界; 最高价值工作 = P1-7+P1-8 合并重基线(走审计)"},
    ],
    "production": [
        ...
        "⭐⭐ 接线候选②(幅度大, 首选): P1-7 Wilder 重基线(PF 3.08→3.56, MDD -629→-490) —— "
        "A/B 四项判据全过; 需走研究级重基线审计(重跑全事件回测+全测试链+冻结基线重认定)",
        "重基线建议与 P1-8 合并: max_hold=12(生产口径 OOS 更稳健) + Wilder ADX 同批统一, "
         "使回测与生产口径一致",
         "⭐ 前置确认已完成(R38q): 生产 paper_sim EVENT 腿**早已在用 Wilder**(L314-321 兼容入口委托 "
         "core.indicators; L744-745 门=Wilder ADX>=20) —— 重基线性质='回测追上生产', 生产行为零变化, 审计风险 中→低",
    ...
    "iterations": [
        ...
        {"round": "R38o", "commit": "8f628da/36b4e71", "content": "P1-7注释方向复核 + 注释机制描述修正('偏低'→'噪声大')"},
        {"round": "R38p", "commit": "e71a924/5930ae5", "content": "P1-7 Wilder重基线A/B: PF3.08→3.56/MDD-629→-490, 四项判据全过(晋级)"},
        {"round": "R38q", "commit": "-", "content": "P1-7审计路径确认: 生产早已用Wilder → 重基线性质='回测追上生产', 风险中→低"},
        {"round": "R38q", "commit": "-", "content": "P1-7审计路径确认: 生产早已用Wilder(L314-321/L744-745) → 重基线性质='回测追上生产', 风险中→低"},
    ],
