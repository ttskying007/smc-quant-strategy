...
review = {
    "verdicts": [
        ...
        {"id": "REBASE-MERGE", "name": "P1-7+P1-8 合并重基线证据包", "result": "最佳合并口径=Wilder+h12",
         ...
         {"id": "RESEARCH-DISCIPLINE", "name": "研究纪律手册(R38沉淀)", "result": "已归档 19 条",
         ...
         {"id": "INDUSTRY-CLUSTER", "name": "行业聚集效应(强)", "result": "候选成立但待确认",
          "detail": "同行业过去7日增持笔数(trailing, 无前视)梯度: =0 +1.49%/PF1.77 -> >=8 +8.17%/PF21.23。 "
                    "**决定性检验**: 剔除 2024-02 崩盘底队列后仍成立(+7.97%/PF9.58); 剔除 2024 全年亦成立(+9.22%/PF10.24)。 "
                    "**2D分层(最强证据)**: 固定全市场增持计数 mkt_trail 后, 行业内计数 ind_trail 在 [21,60] 与 >=151 "
                    "两个最大层内仍有约 +3.0~3.2pp 增量 -> 非纯全市场择时。 IS/OOS: IS n=415 +7.06%/PF13.89 | OOS n=71 +9.49%/PF9.79"},
         {"id": "INDUSTRY-CAVEAT", "name": "自我修正: 置换检验缺陷", "result": "结论降级为待确认",
          "detail": "置换表面 p≈0.000 (0/30 打乱达真实值), **但该 p 不可信**: 打乱**行业标签**并未破坏 "
                    "**日期聚集** —— 同日交易获同一随机标签仍产生聚集, null 分布被高估(打乱中位 +5.36pp vs 真实 +5.92pp)。 "
                    "正确做法: 需 **date-shuffle** 安慰剂才能剥离日期结构。另: OOS 命中率 21% 显著低于 IS 35% -> 覆盖率漂移待查。 "
                    "**故本轮定性 = 强候选(待确认), 不接线**"},
