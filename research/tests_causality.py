...
_limit_at = src.find('limit_px = round(close_px * 0.99, 3)')
_guard_at = src.find('if sl1 is not None and sl1 >= limit_px')
ok("BAD_SL 几何检查前已计算limit_px", _limit_at >= 0 and _limit_at < _guard_at,
   f"limit_at={_limit_at} guard_at={_guard_at}")
ok("EVENT展示TP与实际runner目标一致", '"tp_price": round(tp2, 3)' in src)
...
_ssrc = open(os.path.join(os.path.dirname(PS.__file__), "current_scanner.py"), encoding="utf-8").read()
ok("SMC生产扫描拒绝历史追赶entry", "if entry_idx != len(daily) - 1:" in _ssrc and "_catchup" not in _ssrc)
