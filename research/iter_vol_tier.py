# -*- coding: utf-8 -*-
"""放量分级验证：v_ratio 分级（<1.2 / 1.2-2.0 / ≥2.0）的事件表现
确认放量分级加分（大资金追踪落地）"""
...
print("\n=== 放量分级 ===")
report("全部（基准）", events)
report("v<1.2（无放量）", [t for t in events if t["v_ratio"] < 1.2])
report("v 1.2-2.0", [t for t in events if 1.2 <= t["v_ratio"] < 2.0])
report("v ≥2.0", [t for t in events if t["v_ratio"] >= 2.0])
