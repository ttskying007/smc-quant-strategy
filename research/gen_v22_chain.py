# -*- coding: utf-8 -*-
"""生成 v22 信号链增强回测 CSV —— R60(用户指令: 回测/选股/前端全链记录)。

本文件 = gen_v21_structure.py 的逐行复制 + **信号链列追加**(引擎/过滤链/阈值零改动):
  在 v21 的 18 个结构列之上追加完整因果信号链(单源 core/chain.py):
    breakout_date / breakout_price / breakout_kind
    retrace_price / retrace_state / retrace_signal
    chain_json (全链: trend + events_tail[BOS/CHoCH 时间+价格] + BSL/SSL + OB + FVG/IFVG)
全部链列 = 信号日因果口径(确认bar<=信号日, 无前视)。
**不写 canonical**: 输出 combo_v22_trades.csv (smoke → combo_v22_smoke.csv)。
...
from core.chain import build_chain                        # ← R60: 全因果信号链(单源)
...
for date, code, title in _all_ann:
    ...
    # R60(v22): 完整因果信号链(单源 core/chain.py)
    _chain = build_chain(bs, i)
    ...
    ev.append({
        ...
        # R60(v22): 链热列 + 全链 JSON
        "breakout_date": _chain["breakout"]["date"],
        "breakout_price": _chain["breakout"]["price"],
        "breakout_kind": _chain["breakout"]["kind"],
        "retrace_price": _chain["retrace"]["price"],
        "retrace_state": _chain["retrace"]["state"],
        "retrace_signal": _chain["retrace"]["signal"],
        "chain_json": json.dumps(_chain, ensure_ascii=False)})
conn.close()
...
for t in cont:
    ...
    # R60(v22): CONT 腿同样带完整信号链
    _chain2 = build_chain(bs2, i2)
    t.update({"board": board_of(code6),
              ...
              "breakout_date": _chain2["breakout"]["date"],
              "breakout_price": _chain2["breakout"]["price"],
              "breakout_kind": _chain2["breakout"]["kind"],
              "retrace_price": _chain2["retrace"]["price"],
              "retrace_state": _chain2["retrace"]["state"],
              "retrace_signal": _chain2["retrace"]["signal"],
              "chain_json": json.dumps(_chain2, ensure_ascii=False)})
...
V21_FIELDS = ["symbol", "entry_date", "src", "net_pnl_pct", "rank",
              ...
              # R60(v22): 完整信号链
              "breakout_date", "breakout_price", "breakout_kind",
              "retrace_price", "retrace_state", "retrace_signal", "chain_json"]
out_path = (r"E:\test\smc_project\research\combo_v22_smoke.csv" if SMOKE > 0
            else r"E:\test\smc_project\research\combo_v22_trades.csv")
