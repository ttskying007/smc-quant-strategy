# -*- coding: utf-8 -*-
"""Save v20 trades (v18反转 + 延续固定10日) and finalize production upgrade."""
...
for p in sorted(os.listdir(KT)):
    ...
    for i in range(80, len(daily) - 2):
        ...
        if (daily[entry_idx]["c"] - vw) / vw < 0.05:
            continue
        if entry_idx + 10 >= len(daily):
            continue
        cont.append({"symbol": sym, "entry_date": daily[entry_idx]["t"], "src": "CONT",
                     "net_pnl_pct": round((daily[entry_idx + 10]["c"] / ep - 1) * 100 - FEE, 4)})
    if n % 1500 == 0:
        print(f"  {n} files, cont {len(cont)}", flush=True)
...
# save trades
with open(r"E:\test\smc_project\research\combo_v20_trades.csv", "w", newline="", encoding="utf-8-sig") as fh:
...
print("combo_v20_trades saved:", len(combined))
...
# report
lines = ["# 组合 v20（反转 + 趋势延续）每年/每月报告", ""]
lines.append("> v20 = v18（SMC反转 + 事件反转）+ 延续腿（MARKUP结构支撑+VWAP5%，固定10日）")
lines.append("> 两大方向互补：反转 2024 强，延续 2025/2026 强 → 每年均衡")
lines.append("")
lines.append("## 逐年")
...
with open(r"E:\test\smc_project\research\组合v20逐年逐月报告.md", "w", encoding="utf-8") as fh:
    fh.write("\n".join(lines))
print("v20 report written")

dash = {
    "updated": "2026-08-19",
    "version": "COMBO_V20_DUAL_DIRECTION",
    "total_trades": len(combined),
    "yearly": yearly,
    "monthly": monthly,
    "note": "组合v20 = 反转(SMC+事件) + 延续(MARKUP结构支撑+VWAP5%固定10日) — 两大方向互补，年度均衡",
}
...
reg["research_candidates"]["COMBO_SMC_EVENT"].update({
    ...
    "upgrade_note": "v20 双方向：反转(v18)+延续(MARKUP结构支撑VWAP5%固定10日)，延续补强2025/2026（+5.54%/+4.09%），avg +6.31%，年度均衡",
})
with open(reg_p, "w", encoding="utf-8") as fh:
    json.dump(reg, fh, ensure_ascii=False, indent=2)
print("registry v20 recorded")
for y in yearly:
