...
*维护者：ttskying007（GitHub 用户名，公开标识） | 最近更新：2026-09-08*

> **安全公告（2026-09-08）**：历史提交中曾包含一处明文凭据（`hermes/scripts/test_twikit_*.py`）。
> 已完成：① 工作区改为环境变量读取凭据；② 全部 git 历史重写清除该凭据并强制推送；
> ③ 该密码已按审计要求作废轮换。`ttskying007` 仅为公开 GitHub 用户名，不构成凭据。
