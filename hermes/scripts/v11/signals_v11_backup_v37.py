...
def detect_fvg_v11(ohlcv: List[Dict], min_width: float = None,
    ...
    for i in range(n - 2):
        ...
        # C2 body check: middle candle must have significant body (≥60% of ATR)
        ...
        if b1['h'] < b3['l']:
            ...
            if gap_pct >= min_width and c2_body_ok:
...
def detect_choch_v11(ohlcv: List[Dict], lookback: int = 15,
                     min_confirm_bars: int = 2, tf: str = 'daily') -> List[Dict]:
    """V11.1 精简CHOCH检测 — 只在真实摆动点检测结构转换
    
    V11.1修复 (vs V11):
    - 只在摆动点(local high/low)处检查, 不在每根K线都检查
    - 要求突破幅度 >1% (过滤噪音)
    - 要求确认K线持续在突破外侧
    - 摆动点级别加持 (lookback越大越强)
    """
    n = len(ohlcv)
    if n < lookback + 10:
        return []
    ...
    # 1. 先找所有摆动点 (效率: O(n))
    swing_highs = []  # [(idx, high_price)]
    swing_lows = []   # [(idx, low_price)]
    ...
    # Skip if no swing points
    if len(swing_highs) < 2 and len(swing_lows) < 2:
        return []
    
    # 2. 在摆动点处检测CHOCH
    
    # --- 看涨CHOCH: 摆动低点后价格突破前一个摆动高点 ---
    # 即: 下降趋势中的最后一个lower high被向上突破
...
def detect_bpr_v11(ohlcv: List[Dict], fvg_signals: List[Dict],
                   tf: str = 'daily') -> List[Dict]:
    """BPR (Balanced Price Range) — 价格回到FVG区域后的测试
    
    实际上是"FVG回测"信号: 价格回到之前形成的FVG区域
    """
    if not fvg_signals:
        return []
    ...
    for fvg in fvg_signals:
        ...
        # 价格进入FVG区域 → BPR信号
        for j in range(mitigated_at, min(mitigated_at + 20, n)):
            bar = ohlcv[j]
            in_zone = bar['l'] <= fvg_upper and bar['h'] >= fvg_lower
            
            if in_zone and j >= mitigated_at:
                # 在FVG区域内出现反转K线 → 确认BPR
                if direction == 'bull':
                    if bar['c'] > bar['o'] and bar['c'] > fvg_lower:
                        sig = Signal(
                            type='BPR_Bull', idx=j, direction='bull',
                            price=bar['c'], timeframe=tf,
                            upper=fvg_upper, lower=fvg_lower,
                            strength=min(7.0, 3.0 + fvg.get('strength', 2.0) * 0.5),
                            confidence=min(0.7, 0.4 + fvg.get('confidence', 0.5) * 0.3),
                            metadata={'fvg_idx': fvg_idx, 'fvg_type': fvg.get('type', '')},
                        )
                        signals.append(sig)
                        break
                else:
                    if bar['c'] < bar['o'] and bar['c'] < fvg_upper:
                        sig = Signal(
                            type='BPR_Bear', idx=j, direction='bear',
                            price=bar['c'], timeframe=tf,
                            upper=fvg_upper, lower=fvg_lower,
                            strength=min(7.0, 3.0 + fvg.get('strength', 2.0) * 0.5),
                            confidence=min(0.7, 0.4 + fvg.get('confidence', 0.5) * 0.3),
...
def detect_ifvg_v11(ohlcv: List[Dict], fvg_signals: List[Dict],
                    tf: str = 'daily') -> List[Dict]:
    """IFVG (Inversion FVG) — 被填充的FVG反向变成支撑/阻力
    
    当FVG被完全填充后:
    - Bull FVG被填充 → 原lower变成空头阻力 (IFVG_Bear)
    - Bear FVG被填充 → 原upper变成多头支撑 (IFVG_Bull)
    ...
    for fvg in fvg_signals:
        ...
        if 'Bull' in fvg.get('type', ''):
            # Bull FVG filled → lower becomes bearish resistance
            sig = Signal(
                type='IFVG_Bear', idx=mitigated_at, direction='bear',
            ...
            # Bear FVG filled → upper becomes bullish support
            sig = Signal(
                type='IFVG_Bull', idx=mitigated_at, direction='bull',
...
def detect_breaker_block_v11(ohlcv: List[Dict], choch_signals: List[Dict],
    ...
    for choch in choch_signals:
        ...
        if choch_dir == 'bull':
            # Bull CHOCH: previous Bear OB becomes support
            relevant_obs = [
            ...
            sig = Signal(
                ...
                upper=last_ob.get('upper', 0),
                lower=last_ob.get('lower', 0),
                strength=min(8.0, 4.0 + choch.get('strength', 3.0) * 0.4),
                confidence=min(0.75, 0.5 + choch.get('confidence', 0.5) * 0.2),
            ...
            # Bear CHOCH: previous Bull OB becomes resistance
            relevant_obs = [
                ob for ob in ob_signals
...
def detect_all_signals_v11(ohlcv: List[Dict], params: Dict = None,
    ...
    # 8. IFVG (Inversion FVG — 需要FVG数据)
    ifvg_signals = detect_ifvg_v11(ohlcv, fvg_signals, tf=tf)
    
    # 9. Breaker Block (需要CHOCH和OB数据)
    brk_signals = detect_breaker_block_v11(ohlcv, choch_signals, ob_signals, tf=tf)
    
    # 10. EQL (Equal Highs/Lows)
    eql_signals = detect_eql_v11(ohlcv, tf=tf)
    
    # 11. OTE (Optimal Trade Entry)
    ote_signals = detect_ote_v11(ohlcv, tf=tf)
    
    # 12. MSS (Market Structure Shift)
    ...
    all_signals = (fvg_signals + sweep_signals + ob_signals +
                   ...
                   ifvg_signals + brk_signals + eql_signals +
