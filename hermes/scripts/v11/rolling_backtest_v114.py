#!/usr/bin/env python3
"""
V11.4 — Scout + 紧窗口Silver/Bronze 版本
============================================
基于V11.3(v7), 改进:
1. 收紧序列窗口 (Gold 4/5/4, Silver 5/4, Bronze 3)
2. 快速时间衰减 (exp(-dist/4) 替代 exp(-dist/20))
3. 修复first_signal字段缺失bug
4. 允许紧窗口Silver/Bronze (之前Scout-only)
5. FVG优先入场 (序列以OB结尾则用FVG作为入场点)

预期: WR~60-65%, 覆盖率提升, RR~8x
"""
...
def analyze_at_point_v114(ohlcv, all_signals, end_idx, params):
    """V11.4分析: Scout + 紧窗口Silver/Bronze"""
    ...
    is_silver = 'SILVER' in seq_name
    is_bronze = 'BRONZE' in seq_name
    ...
    # [V11.4] 允许Scout + 紧窗口Silver(avg_distance<=5)
    # Silver/Bronze仅在信号非常靠近(3-5K线)时才有效
    avg_dist = best_seq.get('avg_distance', 99)
    completeness = best_seq.get('completeness', 0)
    confidence = best_seq.get('confidence', 0)
    
    if is_bronze and avg_dist > 4:
        return None  # Bronze太远 = 噪音
    if is_silver and avg_dist > 6:
        return None  # Silver太远 = 延迟信号
    if is_scout and avg_dist > 0:
        return None  # Scout单信号不应avg_dist
    ...
    # Silver/Bronze额外检查: 需要趋势支持
    if is_silver or is_bronze:
        trend_dir, trend_str = short_trend(ohlcv, end_idx)
        if trend_dir == 'down':
            return None  # 下跌趋势不做多
...
def simulate_trades(ohlcv, all_signals, params):
    ...
    for i in range(ROLL_START, roll_end):
        ...
        entry_info = analyze_at_point_v114(ohlcv, all_signals, i, params)
        ...
        # [V11.4] Silver/Bronze使用FVG入场价(不是OB确认价)
        fvg_entry = seq_result.get('fvg_entry')
        entry_signal = seq_result.get('entry_signal', {})
        
        if fvg_entry:
            entry_price = fvg_entry.get('price', decision.get('entry_price'))
        else:
        ...
        trades.append({
            ...
            'avg_dist': best_seq.get('avg_distance', 0),
        })
        entered_bar = i
...
def main():
    ...
    print(f"{'='*80}")
    print(f"V11.4 回测 -- Scout + 紧窗口Silver/Bronze + Bull-only")
    print(f"  {min(MAX_STOCKS, len(symbols))}/{len(symbols)} stocks")
    print(f"  SL={SL_FIXED}% TP={TP_FIXED}% Scout共振>={SCOUT_MIN_RESONANCE}")
    print(f"  收紧窗口: Gold 4/5/4 Silver 5/4 Bronze 3")
    print(f"  时间衰减: exp(-dist/4) | FVG优先入场")
    print(f"{'='*80}")
    ...
    for idx, sym in enumerate(symbols[:MAX_STOCKS]):
        ...
        if trades:
            ...
            print(f"  [{idx+1:3d}/{MAX_STOCKS}] {sym:12s} "
                  f"NO-TRADE sigs={sigs} phase={phase_r}")
    ...
    print(f"\n{'='*80}")
    print(f"V11.4 SUMMARY — {len(stock_results)} tradable out of {MAX_STOCKS}")
    ...
    if all_trades:
        ...
        dist_avg = sum(t.get('avg_dist', 0) for t in all_trades) / n
        print(f"  Avg hold bars: {hold_avg:.1f} | Avg dist: {dist_avg:.1f}")
    ...
    outpath = OUTPUT_DIR / 'backtest_v11_v114.json'
    out = {
        'timestamp': datetime.now().isoformat(),
        'config': {'version': 'V11.4', 'max_stocks': MAX_STOCKS, 'sl': SL_FIXED, 'tp': TP_FIXED},
        'summary': {
