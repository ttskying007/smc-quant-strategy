...
# ── V470 混合信号引擎 ──
# 信号检测: signals_v11 (14种全量, 已验证的高WR基础)
# 入口过滤: 质量阈值+反转检查+POI回调+序列SCOUT+共振

PROGRESSIVE_BE = [(3, 0.0), (6, 0.3), (10, 0.5), (16, 1.0)]
...
# 60min: 80根 = ~80小时, 扩大窗口提高POI激活率
POI_RETRACE_WINDOW = 80
...
def evaluate_v45_entry(all_signals, all_sigs_up_to_idx, sig, ohlcv, n,
    ...
    if sig_dir == 'bull':
        ...
        if lower > 0 and upper > lower:
            ...
            # 包含entry_bar自身 — 确认bar可能已在POI区域内(立即入场)
...
def run_backtest(symbols, label="V470-60min"):
    """通用回测运行器"""
    ...
    print(f"{'='*80}")
    print(f"V470 - Pine-quality signal engine (FVG/OB/Sweep/CHOCH)")
    print(f"  {len(symbols)} stocks | OB disp filter: 1.3x | State-machine structure")
    print(f"  Trailing: V38.4 3-profile | Bull-only")
    print(f"{'='*80}")
