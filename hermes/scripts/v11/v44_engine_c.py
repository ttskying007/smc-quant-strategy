#!/usr/bin/env python3
"""
SMC V44 Variant C — 恢复V43退出参数
==============================================
与V44的区别:
1. 禁用回踩入场 (直接confirmed_at+1入场)
2. 保留V44信号检测 (OB v14 + 共振)
3. 使用V38.4风格的trailing (更松的阈值 + 多级锁定)
4. 验证: V44信号 + V43退出 = V43性能?
"""
...
RETEST_PARAMS = {
    'max_retest_bars': 0,       # Variant C: disable retest
...
def main():
    ...
    print(f"{'=' * 80}")
    print('V44 Variant C — V44信号 + V43退出 (无回踩)')
    print(f'  {len(symbols)} stocks')
    print(f'  改动: 禁用回踩 | V38.4 trailing | 直接confirmed_at+1入场')
    print(f"{'=' * 80}")
    ...
    if all_trades:
        ...
        # 保存
        outpath = OUTPUT_DIR / 'v44_test_c.json'
        json.dump({
