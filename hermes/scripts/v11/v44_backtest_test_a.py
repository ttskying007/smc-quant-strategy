...
print(f"Testing V44-A on {len(symbols)} stocks...")

from v44_engine_a import (
...
if all_trades:
    ...
    # TP ratio: what fraction of trades hit TP vs other exits
    tp_ratio = n_tp / n * 100 if n > 0 else 0
    ...
    print(f"\n  TP ratio (TP hits / total): {tp_ratio:.1f}%")
    ...
    # 保存结果
    outpath = OUTPUT_DIR / 'v44_test_a.json'
    json.dump({
        'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
        'version': 'V44-A',
        'config': {
            'variant': 'A - Double ATR TP (3.0->6.0), skip nearest swing',
            'atr_mult': 6.0,
            'swing_skip': True,
        },
        'summary': {
