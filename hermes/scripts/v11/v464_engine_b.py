...
OUTPUT_DIR = Path('/root/.hermes/smc_opt_v464_rr7')
...
# ── 最小RR门槛 (只接受projected RR>=此值) ──
MIN_PROJECTED_RR = 7.0  # 过滤低RR交易, 提升均值
...
def calc_v38_trailing(ohlcv, entry_idx, entry_price, initial_sl,
    ...
    """V464 多目标trailing: 原始V463紧trailing + TP1到达后松
    V463原始: BE=0.2%, LK=0.5%, lock 1.0%/1.5%/3.0%/6.0%
    TP1到达后: 松开2x, 让TP2/TP3跑
    """
    sl = initial_sl
    extreme = entry_price
    ...
    multi_tp = structural_tp[4] if structural_tp and len(structural_tp) > 4 else None

    is_bear = (direction == 'bear')
    has_tp = tp_price is not None

    profile = 'tight' if not has_tp else ('bear' if is_bear else 'loose')
    tp_reached = 0
    ...
    for j in range(entry_idx + 1, min(entry_idx + max_hold + 1, n)):
        ...
        if is_bear:
            ...
            # TP到达检查
            if multi_tp and j > entry_idx + 2:
                for ti, (tp_name, tp_pr, tp_p) in enumerate(multi_tp):
                    if tp_reached < ti + 1 and extreme <= tp_pr:
                        tp_reached = ti + 1

            # TP1未到达: V463原始紧trailing
            if tp_reached < 1:
...
def find_macro_swing_high(ohlcv, entry_idx, lookahead=60, lookback=20):
    """V464: 20-bar宏观摆动高 — 捕获中期结构, 非局部小顶
    Returns: {'idx': i, 'price': h} or None
    """
    n = len(ohlcv)
    best = None
    for i in range(entry_idx + 2, min(entry_idx + lookahead, n - 2)):
        is_high = all(ohlcv[i]['h'] >= ohlcv[j]['h']
                     for j in range(max(0, i - lookback), min(n, i + lookback + 1)) if j != i)
        if is_high and ohlcv[i]['h'] > max(ohlcv[entry_idx]['c'], ohlcv[entry_idx]['h']):
            if best is None or ohlcv[i]['h'] < ohlcv[best['idx']]['h']:
                best = {'idx': i, 'price': ohlcv[i]['h']}
    return best


def find_macro_swing_low(ohlcv, entry_idx, lookahead=60, lookback=20):
    """V464: 20-bar宏观摆动低"""
    n = len(ohlcv)
    best = None
    for i in range(entry_idx + 2, min(entry_idx + lookahead, n - 2)):
...
def calc_v45_sl(ohlcv, entry_idx, entry_price, signal, entry_type, direction, params, all_signals):
    """V464 SL: 信号边界 > 摆动点 > ATR自适应 (原始距离, 不紧缩)"""
    sig_type = signal.get('type', '')

    # 1. 信号边界SL (FVG lower/upper, OB lower/upper — 原始距离)
    if direction == 'bull':
        if 'FVG' in sig_type:
...
def calc_v45_tp(ohlcv, entry_idx, entry_price, signal, direction, all_signals, entry_type='FVG'):
    """V464 多级TP: CHOCH > nearest swing > 20-bar swing > fib 1.272 > fib 1.618
    返回 (tp_price, tp_type, tp_pct, tp_idx, multi_tp_list)
    其中 multi_tp_list = [(name, price, pct), ...] 按顺序
    """
    multi_tp = []  # 多级目标列表

    # 0. CHOCH优先 (avgRR=10.71x最高)
    if direction == 'bull':
        ...
        if forward_choch:
            ...
            if tp_price > entry_price:
                ...
                if tp_pct >= 0.3:
                    multi_tp.append(('choch', round(tp_price, 4), round(tp_pct, 2)))
    ...
    if direction == 'bull':
        ...
        if swing and swing['price'] > entry_price:
            ...
            if tp_pct >= 0.5:
                multi_tp.append(('swing_high', round(swing['price'], 4), round(tp_pct, 2)))

    # 2. 20-bar宏观摆动高 (TP2, 更远结构)
    if direction == 'bull':
        macro = find_macro_swing_high(ohlcv, entry_idx, 60, lookback=20)
        if macro and macro['price'] > entry_price:
            # 确保不是同一个
            if not multi_tp or abs(macro['price'] / multi_tp[-1][1] - 1) > 0.02:
                tp_pct = (macro['price'] - entry_price) / entry_price * 100
                if tp_pct >= 1.0:
                    multi_tp.append(('macro_20', round(macro['price'], 4), round(tp_pct, 2)))

    # 3. Fib 1.272扩展 (TP3)
    if multi_tp:
        # 用最近的最近的TP距离做fib基础
        nearest_tp_pct = min(t[2] for t in multi_tp if t[2] > 0)
        fib_1272 = nearest_tp_pct * 1.272
        fib_price = entry_price * (1 + fib_1272 / 100)
        multi_tp.append(('fib_1272', round(fib_price, 4), round(fib_1272, 2)))

    # 4. Fib 1.618扩展 (TP4)
    if multi_tp:
        nearest_tp_pct = min(t[2] for t in multi_tp if t[2] > 0)
        fib_1618 = nearest_tp_pct * 1.618
        fib_price = entry_price * (1 + fib_1618 / 100)
        multi_tp.append(('fib_1618', round(fib_price, 4), round(fib_1618, 2)))

    # 排序: 最近到最远
    multi_tp = sorted(multi_tp, key=lambda x: x[2])

    # 主TP = 第一个 (最近)
    if multi_tp:
        return multi_tp[0][1], multi_tp[0][0], multi_tp[0][2], None, multi_tp
...
def evaluate_v45_entry(all_signals, all_sigs_up_to_idx, sig, ohlcv, n,
    ...
    # TP
    tp_result = calc_v45_tp(
        ohlcv, entry_bar, entry_price, sig, sig_dir, all_signals, entry_type)
    if tp_result:
        tp_price, tp_type, tp_pct, tp_idx, multi_tp = tp_result
    else:
        tp_price, tp_type, tp_pct, tp_idx, multi_tp = None, None, None, None, None

    # 结构TP元组 (带多目标列表)
    structural_tp = (tp_price, tp_type, tp_pct, tp_idx)
    if multi_tp:
        structural_tp = (tp_price, tp_type, tp_pct, tp_idx, multi_tp)
...
def calc_stock_params_v45(ohlcv, symbol):
    """V464 每只股票自适应参数: ATR + BE/LK + SL紧缩乘数"""
    n = len(ohlcv)
    if n < 30:
...
def run_backtest(symbols, label="V464"):
    """V464 回测运行器 — SL紧缩×0.5 + Trailing×2宽松 + 多级TP + 宏观摆动"""
    ...
    print(f"{'='*80}")
    print(f"V464 — RR优化: 原始SL + Trailing×1.2 + 多级TP + 宏观摆动")
    print(f"  {len(symbols)} 只股票 | OB-only+反转 | SL=原始边界/摆动/ATR")
    print(f"  TP=CHOCH→5-bar→20-bar→Fib1.272→Fib1.618 | Trailing×1.2宽松")
    print(f"{'='*80}")
...
if __name__ == '__main__':
    ...
    result = run_backtest(symbols, "V464")

    if result and result.get('all_trades'):
        out_path = OUTPUT_DIR / 'v464_full.json'
