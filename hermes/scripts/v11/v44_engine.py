...
def detect_retest_entries(ohlcv, signals, params=None, tf='daily'):
    ...
    for i in range(MIN_BARS, n - 5):
        ...
        for zone in active_zones:
            ...
            # 计算入场价格
            if zone_direction == 'bull' and bar['l'] <= zone['upper'] and bar['c'] > zone['upper']:
                entry_mode = 'CONFIRM_WICK_RETOUCH_RAW_HIGH'
                entry_price = zone['upper']
            else:
                entry_mode = f'RETEST_{retest_type.upper()}'
...
def calc_structural_tp_v44(ohlcv, entry_idx, entry_price, direction, all_signals):
    ...
    if direction == 'bull':
        swing = find_swing_high_forward(ohlcv, entry_idx, 120)
...
def evaluate_signal_entry_v44(ohlcv, sig_idx, sig, all_sigs_up_to_idx, all_signals,
    ...
    pnl = ((entry_price - exit_price) / entry_price * 100) if is_bear else ((exit_price - entry_price) / entry_price * 100)
    ...
    return {
        ...
        'confirmed_at': confirmed_at,
        'exit_idx': exit_idx,
        'entry_date': ohlcv[entry_bar].get('date', ohlcv[entry_bar].get('t', '')),
        'exit_date': ohlcv[exit_idx].get('date', ohlcv[exit_idx].get('t', '')),
        'signal_date': ohlcv[sig_idx].get('date', ohlcv[sig_idx].get('t', '')),
        ...
        'direction': direction,
        'entry_mode': 'DIRECT_SIGNAL_CLOSE',
...
def _evaluate_retest_entry(ohlcv, retest, all_signals, params, phase):
    ...
    return {
        ...
        'exit_idx': exit_idx,
        'entry_date': ohlcv[entry_idx].get('date', ohlcv[entry_idx].get('t', '')),
        'exit_date': ohlcv[exit_idx].get('date', ohlcv[exit_idx].get('t', '')),
        'signal_date': ohlcv[retest.original_idx].get('date', ohlcv[retest.original_idx].get('t', '')) if retest.original_idx < len(ohlcv) else '',
        'entry_price': round(entry_price, 2),
        'exit_price': round(exit_price, 2),
        'sl': round(sl, 2),
        'pnl_pct': round(pnl, 2),
        ...
        'tp_pct': round(tp_pct, 2) if tp_pct else None,
                ...
                'direction': direction,
                'entry_mode': retest.entry_mode or ('CONFIRM_WICK_RETOUCH_RAW_HIGH' if entry_price == retest.signal_zone_upper else f'RETEST_{retest.retest_type.upper()}'),
                'exit_method': 'tp_hit' if tp_type and tp_price and (
                    (not is_bear and exit_price >= tp_price) or
                    (is_bear and exit_price <= tp_price)
                ) else 'trailing',
