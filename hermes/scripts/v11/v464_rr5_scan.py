#!/usr/bin/env python3
"""V464 RR5 全量4800扫描 — MIN_PROJECTED_RR=5.0"""
...
for mod in list(sys.modules.keys()):
    if 'v464_engine' in mod:
        del sys.modules[mod]
from v11.v464_engine_a import run_backtest, CACHE_DIR, OUTPUT_DIR
...
t0 = time.time()
result = run_backtest(symbols, label="V464-RR5")
...
if result:
    ...
    out_path = OUTPUT_DIR / 'v464_rr5_full.json'
    json.dump(summary, open(str(out_path), 'w'))
    print(f"\nFull scan saved: {out_path}")
    print(f"V464-RR5: {json.dumps(summary)}")
