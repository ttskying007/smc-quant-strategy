...
def calc_v38_trailing(ohlcv, entry_idx, entry_price, initial_sl,
    ...
    """
    V469-B 反向分级Trailing — Grade A紧/C松
    Grade A (强信号): 阈值×1.0 (基线), 紧锁利, 目标高WR
    Grade B (一般): 阈值×1.2, 中等锁利
    Grade C (弱信号): 阈值×1.5, 给更多空间让利润跑
    
    保留V468的3-profile (loose/bear/tight)和渐进BE
    """
    sl = initial_sl
    extreme = entry_price
    ...
    # ── V469-B: 分级宽松度系数 (反向: Grade A紧/C松) ──
    # E方案: 统一松trailing 1.5x (忽略grade)
    G = 1.5  # was: {'A': 1.0, 'B': 1.2, 'C': 1.5}.get(signal_grade, 1.0)
    ...
    for j in range(entry_idx + 1, min(entry_idx + max_hold + 1, n)):
        ...
        if is_bear:
            ...
            if profile == 'loose':
                ...
                elif gain_pct >= 20.0 * G:
                    sl = max(sl, extreme * (1 - 5.0/100))
                elif gain_pct >= 12.0 * G:
...
# ── V469: 信号强度评分 (多信号共振检测) ──
SIGNAL_CLUSTER_WINDOW = 8

def calc_signal_strength(sig, all_sigs_up_to_idx, ohlcv):
    """计算入场信号的强度等级 (A/B/C)
    基于sig前后SIGNAL_CLUSTER_WINDOW内共存的信号类型数量。
    仅用于分级trailing, 不作为入场过滤。
    Returns: 'A'|'B'|'C'
    """
    ...
    final_score = min(0.85, (n_found - 1) * 0.25 + 0.25) + trend_score + quality * 0.20
...
def evaluate_v45_entry(all_signals, all_sigs_up_to_idx, sig, ohlcv, n,
    ...
    # ── V469: 信号强度评分 (仅用于分级trailing) ──
    signal_grade = calc_signal_strength(sig, all_sigs_up_to_idx, ohlcv)
    ...
    # ── V469: 序列质量提升信号等级 ──
    if 'SILVER' in seq_name or 'GOLD' in seq_name or 'PLATINUM' in seq_name:
        if signal_grade == 'C':
            signal_grade = 'B'
        if 'GOLD' in seq_name or 'PLATINUM' in seq_name:
            signal_grade = 'A'
