...
# ── V466 A+B逻辑参数 ──
MIN_PROJECTED_RR = 8.0   # A: 跳过projected RR < 8.0的交易
MULTI_BAR_BE_LOCK = 2    # B: 持仓>=2bar且利润不足时, SL锁定至入场价
    ...
    for j in range(entry_idx + 1, min(entry_idx + max_hold + 1, n)):
        ...
        if is_bear:
            ...
            if tp_price and extreme <= tp_price * 1.05:
                ...
                # MULTI_BAR_BE_LOCK: lock to entry_price if held enough bars with insufficient gain
                if j >= entry_idx + MULTI_BAR_BE_LOCK and gain_pct < be_gain:
...
if __name__ == '__main__':
    ...
    result = run_backtest(symbols, "V466-Daily")

    if result and result.get('all_trades'):
        out_path = OUTPUT_DIR / 'v466_full.json'
