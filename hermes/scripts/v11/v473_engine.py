...
OUTPUT_DIR = Path('/root/.hermes/smc_opt_v473')
...
def run_backtest(symbols, label="V473-60min"):
    """通用回测运行器"""
    ...
    print(f"{'='*80}")
    print(f"V473 — 60min混合引擎 (swing-backward + fallback)")
    print(f"  {len(symbols)} 只股票 | 信号: FVG/OB/Sweep/CHOCH | 过滤: 序列+共振+趋势")
    print(f"  Trailing: V467 渐进式BE+TP感知 | OB: V13 60min优化 | Bull-only")
    print(f"{'='*80}")
