...
OUTPUT_DIR = Path('/root/.hermes/smc_opt_v477')
...
#   hold>=3 AND gain<0     → BE
#   hold>=5 AND gain<0.3%  → BE
#   hold>=8 AND gain<0.5%  → BE
#   hold>=12 AND gain<1.0% → BE
...
def calc_v38_trailing(ohlcv, entry_idx, entry_price, initial_sl,
    ...
    for j in range(entry_idx + 1, min(entry_idx + max_hold + 1, n)):
        ...
        if is_bear:
            ...
            if tp_price and extreme >= tp_price * 0.75:  # V8: 0.75 (was 0.90, too strict)
            ...
            if profile == 'tight':
                ...
                if gain_pct >= 20.0:
                    ...
                    sl = max(sl, entry_price)
            else:  # loose (V8修复: 有效trailing阈值, 消除死区)
                if gain_pct >= 18.0:
                    sl = max(sl, extreme * (1 - 8.0/100))
                elif gain_pct >= 12.0:
                    sl = max(sl, extreme * (1 - 6.0/100))
                elif gain_pct >= 8.0:
                    sl = max(sl, extreme * (1 - 4.0/100))
                elif gain_pct >= 5.0:
                    sl = max(sl, entry_price * (1 + 1.0/100))  # lock 1%, was 0.985(bug)
                elif gain_pct >= 3.0:
                    sl = max(sl, entry_price * (1 + 0.3/100))  # tiny lock
                elif gain_pct >= be_gain:
...
def load_ohlcv(symbol):
    # Try 500-bar first, fall back to 200-bar
    for count in [500, 200]:
        fname = f"{symbol.replace('.','_')}_60min_{count}.json"
...
def find_swing_high_forward(ohlcv, entry_idx, lookahead=200):
    """60min: 跳过8bar, 找远距离摆动高"""
    n = len(ohlcv)
    best = None
    start = max(entry_idx + 8, 0)
    for i in range(start, min(start + lookahead, n - 2)):
        is_high = all(ohlcv[i]['h'] >= ohlcv[j]['h']
                      for j in range(max(0, i - 8), min(n, i + 9)) if j != i)
        if is_high:
...
def calc_v45_sl(ohlcv, entry_idx, entry_price, signal, entry_type, direction, params, all_signals):
    """V476 SL: 100% ATR自适应 (跳过所有边界/摆动点)"""
    sig_type = signal.get('type', '')
    
    # V475: OB trades always use adaptive SL (skip OB boundary check)
    # FVG still uses signal boundary for OB-price zone protection
    ...
    # 2. ATR自适应SL (V8修复: SL=ATR*0.25 而非 ATR*0.09, 下限0.3%保证结构意义)
    atr = calc_atr_v45(ohlcv, entry_idx)
    sl_mult = params.get('sl_mult', 0.3)
    base_sl = max(0.30, min(2.0, atr * sl_mult * 0.8))
    # ATR typically 0.5-1.5% for 60min → SL = 0.12-1.2% (floor at 0.30%)
    # This gives SL structural meaning: ~25-60% of a typical 60min bar's ATR
    if direction == 'bull':
...
def evaluate_v45_entry(all_signals, all_sigs_up_to_idx, sig, ohlcv, n,
                        direction, params, stock_params, symbol=''):
...
def calc_stock_params_v45(ohlcv, symbol):
    ...
    if avg_atr < 1.0:
        vol_class = 'low'
        sl_mult = 0.30
        be_lock = 1.50    # 60min: 1.5%
        look_lock = 3.00  # 60min: 3.0%
        max_hold = 60
    elif avg_atr < 3.0:
        vol_class = 'medium'
        sl_mult = 0.30
        be_lock = 2.00    # 60min: 2.0%
        look_lock = 4.00  # 60min: 4.0%
        max_hold = 60
    else:
        vol_class = 'high'
        sl_mult = 0.25
        be_lock = 3.00    # 60min: 3.0%
        look_lock = 5.00  # 60min: 5.0%
        max_hold = 60
...
def backtest_stock_v45(ohlcv, symbol):
    ...
    for sig in all_signals:
        ...
        result = evaluate_v45_entry(
            ...
            base_params, stock_params, symbol=symbol)
...
def run_backtest(symbols, label="V465-60min"):
    ...
    print(f"{'='*80}")
    print(f"V477 — T+1强制 (跳过同日exit) + 全adaptive SL")
    print(f"  {len(symbols)} 只股票 | 信号: FVG/OB/Sweep/CHOCH | 过滤: 序列+共振+趋势")
    print(f"  SL: 100% ATR-adaptive | T+1: 强制跨日exit | Trailing: V467渐进BE")
    print(f"{'='*80}")
...
if __name__ == '__main__':
    ...
    result = run_backtest(symbols, "V477")

    if result and result.get('all_trades'):
        out_path = OUTPUT_DIR / 'v477_full.json'
