...
from v11.signals_v12 import detect_all_signals_v13_60min as detect_all_signals_v11, calc_adaptive_thresholds
...
OUTPUT_DIR = Path('/root/.hermes/smc_opt_v474')
...
MIN_PROJECTED_RR = 6.0

# ── 交易信号类型白名单 (只这4种生成交易) ──
TRADE_SIGNAL_TYPES = {'FVG_Bull', 'FVG_Bear', 'OB_Bull', 'OB_Bear',
                      'SweepUp', 'SweepDown', 'Sweep', 'CHOCH_Bull', 'CHOCH_Bear'}
...
def evaluate_v45_entry(all_signals, all_sigs_up_to_idx, sig, ohlcv, n,
    ...
    if sig_idx > 5:
        for ps in all_sigs_up_to_idx:
            ps_type = ps.get('type', '')
            ps_idx = ps.get('idx', 0)
            if sig_dir == 'bull' and ('SweepDown' in ps_type or (ps_type == 'Sweep' and ps.get('direction') == 'bull')):
                if 0 < sig_idx - ps_idx <= 5:
                    sweep_fvg_found = True
                    break
            elif sig_dir == 'bear' and ('SweepUp' in ps_type or (ps_type == 'Sweep' and ps.get('direction') == 'bear')):
                if 0 < sig_idx - ps_idx <= 5:
                    sweep_fvg_found = True
                    break
...
def run_backtest(symbols, label="V474-60min"):
    """通用回测运行器"""
    ...
    print(f"{'='*80}")
    print(f"V474 — 60min混合引擎 (swing-backward + fallback)")
    print(f"  {len(symbols)} 只股票 | 信号: FVG/OB/Sweep/CHOCH | 过滤: 序列+共振+趋势")
    print(f"  Trailing: V467 渐进式BE+TP感知 | OB: V13 60min Relaxed优化 | Bull-only")
    print(f"{'='*80}")
