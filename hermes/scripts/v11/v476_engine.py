...
OUTPUT_DIR = Path('/root/.hermes/smc_opt_v476')
...
def run_backtest(symbols, label="V465-60min"):
    ...
    print(f"{'='*80}")
    print(f"V476 — 全adaptive SL (跳过OB边界+摆动点SL)")
    print(f"  {len(symbols)} 只股票 | 信号: FVG/OB/Sweep/CHOCH | 过滤: 序列+共振+趋势")
    print(f"  SL: 100% ATR-adaptive | Trailing: V467渐进BE")
    print(f"{'='*80}")
...
if __name__ == '__main__':
    ...
    result = run_backtest(symbols, "V476")

    if result and result.get('all_trades'):
        out_path = OUTPUT_DIR / 'v476_full.json'
