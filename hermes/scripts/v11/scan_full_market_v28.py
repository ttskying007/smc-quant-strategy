#!/usr/bin/env python3
"""V28 Full Market Scan — Confirmed Entry"""
import json, sys, time
from pathlib import Path
sys.path.insert(0, '/root/.hermes/scripts')
from v11.rolling_backtest_v28 import *
...
print(f"V28 Full Market — {len(symbols)} stocks")
...
print(f"\n{'='*70}")
print(f"V28 FULL MARKET — {len(all_stocks)}/{len(symbols)} | {total_time:.0f}s")
print(f"{'='*70}")
...
outpath=OUTPUT_DIR/'v28_full_merged.json'
json.dump({'timestamp':time.time(),'config':{'version':'V28','confirmed_entry':True},
