#!/usr/bin/env python3
"""
V19 — Phase-Aware Per-Stock Optimizer
========================================
V16全量扫描: 1995/4800 tradable, WR=76.2%, RR=11.1x
V18每股优化: WR从76.9%→82.7% (SL 0.3%, TP 3.0%)

V19创新:
  1. 每股票+每阶段参数: 不同市场阶段(breakout/volatile/ranging)不同SL/TP
  2. 阶段自适应入场: 高波动阶段用tight SL, 低波动用wide SL
  3. 信号序列过滤整合: seq_score>=0.3筛选
  4. 全量扫描: 用优化参数跑全市场

预期: WR=80-85%, Swing覆盖率同V16
"""
...
# Phase-specific parameter maps
PHASE_PARAMS = {
    'breakout':      {'sl': 0.3, 'tp': 3.0, 'seq_f': 0.0, 'min_res': 0.65},  # 趋势强, 紧止损快止盈
    'volatile':      {'sl': 0.3, 'tp': 3.0, 'seq_f': 0.3, 'min_res': 0.70},  # 波动大, 紧止损+信号过滤
    'ranging':       {'sl': 0.5, 'tp': 5.0, 'seq_f': 0.5, 'min_res': 0.75},  # 震荡, 宽止损+严格过滤
    'trending_up':   {'sl': 0.3, 'tp': 5.0, 'seq_f': 0.0, 'min_res': 0.60},  # 上升趋势, 紧止损宽止盈
    'trending_down': {'sl': 0.5, 'tp': 5.0, 'seq_f': 0.3, 'min_res': 0.70},  # 下跌不交易(但bull方向)
}

# Per-stock override (loaded from optimization)
STOCK_OPTIMIZED_PARAMS = {}


def load_phase_params(phase):
    """Get parameters for a specific market phase"""
    return PHASE_PARAMS.get(phase, {'sl': 0.3, 'tp': 3.0, 'seq_f': 0.0, 'min_res': 0.65})
...
def score_signal_sequence(sigs_before):
    """V18 signal sequence scoring"""
    if len(sigs_before) < 3: return 0.5, 'insufficient'
    ...
    if not pattern: return 0.5, 'no-pattern'
    score = 0.5
    if len(pattern) >= 2:
        if pattern[-1] == 'F' and 'S' in pattern[:-1]: score += 0.20
        if pattern[-1] == 'O' and pattern[-2] == 'F': score += 0.10
        if pattern[-1] == 'F' and pattern[-2] == 'O': score += 0.25
    if pattern[-1] == 'F': score += 0.10
    ob_count = pattern.count('O')
    if ob_count >= 4: score -= 0.30
    elif ob_count >= 3: score -= 0.10
    if len(set(pattern)) <= 1: score -= 0.20
    if len(set(pattern)) >= 3 and len(pattern) >= 3: score += 0.10
    return round(max(0.0, min(1.0, score)), 2), ''.join(pattern)
...
def find_best_swing_sl(ohlcv, end_idx, entry_price, sl_cap=0.5, max_dist=20):
...
def calc_sltp(ohlcv, end_idx, entry_price, signal_type='FVG', sl_fixed=0.3, tp_fixed=3.0, sl_cap=0.5):
    fixed_sl = entry_price * (1 - sl_fixed / 100)
    fixed_tp = entry_price * (1 + tp_fixed / 100)
    sl_info = find_best_swing_sl(ohlcv, end_idx, entry_price, sl_cap)
    ...
    if tp_info is not None:
        ...
        actual_rr = tp_pct / sl_pct_actual if sl_pct_actual > 0 else 10
        tp_type = 'swing'
    else:
...
def analyze_at_point(ohlcv, all_signals, end_idx, params, phase_params):
    ...
    if sig_idx < len(ohlcv) - 1 and sig_idx > 30:
        ...
        if bar_vol < avg_vol * 0.8: return None
    ...
    # V19: Signal sequence scoring with phase-adaptive threshold
    seq_score, seq_reason = score_signal_sequence(sigs_before)
    seq_filter = phase_params.get('seq_f', 0.0)
    if seq_score < seq_filter: return None
    
    effective_min_resonance = phase_params.get('min_res', 0.65)
    if signal_type == 'OB':
    ...
    return {
        ...
        'signal_type': signal_type, 'seq_score': seq_score,
        'seq_reason': seq_reason,
    }


def simulate_trades(ohlcv, all_signals, params, phase, symbol):
    n = len(ohlcv); roll_end = n - 10
    trades = []; entered_bar = -999
    phase_params = load_phase_params(phase)
    
    # Per-stock override
    stock_opt = STOCK_OPTIMIZED_PARAMS.get(symbol)
    if stock_opt:
        phase_params = {**phase_params, **stock_opt}
    
    sl_fixed = phase_params['sl']; tp_fixed = phase_params['tp']
    
    for i in range(80, roll_end):
        if i - entered_bar < 15: continue
        entry_info = analyze_at_point(ohlcv, all_signals, i, params, phase_params)
        ...
        seq_score = entry_info['seq_score']; seq_reason = entry_info['seq_reason']
        ...
        trades.append({
            ...
            'hold_bars': exit_idx - i, 'sl_type': swing_params['sl_type'],
            'sl_pct': swing_params['sl_pct'], 'tp_pct': swing_params['tp_pct'],
            'signal_type': signal_type, 'seq_score': seq_score,
            'phase': phase, 'seq_reason': seq_reason,
        })
        entered_bar = i
    return trades
...
def backtest_stock(ohlcv, symbol):
    ...
    params = {**base_params}
    trades = simulate_trades(ohlcv, all_signals, params, phase, symbol)
    if len(trades) < 2:
...
def main():
    ...
    print(f"{'='*80}")
    print("V19 — Phase-Aware Per-Stock Optimizer")
    print(f"  200 stocks test | Phase-adaptive SL/TP/seq filter")
    print(f"{'='*80}")
    print(f"Phase params:")
    for phase, p in PHASE_PARAMS.items():
        print(f"  {phase:15s} SL={p['sl']}% TP={p['tp']}% seqF={p['seq_f']} res={p['min_res']}")
    ...
    for idx, sym in enumerate(symbols[:200]):
        ...
        if trades:
            ...
            print(f"  [{idx+1:3d}/200] {sym:12s} t={perf['n_trades']:2d} WR={perf['win_rate']:.0f}% RR={perf['avg_rr']:.1f}x PF={perf['profit_factor']:.1f} P&L={perf['avg_pnl']:+.2f}% swing={perf.get('swing_sl_pct',0):.0f}% phase={perf.get('phase','?')}")
        else:
            print(f"  [{idx+1:3d}/200] {sym:12s} NO-TRADE sigs={result.get('n_signals',0)} phase={result.get('phase','?')}")
    ...
    if all_trades:
        ...
        print(f"\n{'='*80}")
        print(f"V19 SUMMARY — {len(stock_results)} tradable out of 200 | {total_time:.1f}s")
        print(f"{'='*80}")
        ...
        for phase, cnt in phases.most_common():
            ...
            avg_rr_p = sum(s['avg_rr'] for s in subset)/len(subset)
            print(f"    {phase:15s}: {len(subset):3d} stocks | avg WR={avg_wr:.0f}% | avg RR={avg_rr_p:.1f}x")
        
        # Seq score correlation
        print(f"\n  Seq Score vs WR:")
        for threshold in [0.3, 0.5, 0.6, 0.7]:
            subset = [t for t in all_trades if t.get('seq_score',0) >= threshold]
            if subset:
                swr = sum(1 for t in subset if t['won'])/len(subset)*100
                print(f"    seq>={threshold:.1f}: {len(subset):3d} trades | WR={swr:.0f}%")
        
        outpath = OUTPUT_DIR / 'backtest_v19.json'
        json.dump({
            'timestamp': datetime.now().isoformat(),
            'config': {'version': 'V19', 'phase_params': PHASE_PARAMS},
