...
OUTPUT_DIR = Path('/root/.hermes/smc_opt_v475')
...
def run_backtest(symbols, label="V465-60min"):
    ...
    print(f"{'='*80}")
    print(f"V475 — 强制adaptive SL (跳过OB边界SL)")
    print(f"  {len(symbols)} 只股票 | 信号: FVG/OB/Sweep/CHOCH | 过滤: 序列+共振+趋势")
    print(f"  Trailing: V467渐进BE | SL: adaptive-only | 参数: 每股ATR自适应")
    print(f"{'='*80}")
...
if __name__ == '__main__':
    ...
    result = run_backtest(symbols, "V475")

    if result and result.get('all_trades'):
        out_path = OUTPUT_DIR / 'v475_full.json'
