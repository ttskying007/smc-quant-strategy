...
OUT_DIR = Path('/root/.hermes/smc_opt_v71_smart_money_position')
OUT_DIR.mkdir(parents=True, exist_ok=True)
N = int(sys.argv[1]) if len(sys.argv) > 1 else 0
MAX_HOLD = 60
ENGINE = 'V71_SMART_MONEY_POSITION'
DEFINITION = 'V71_SSL_CHOCH_OB_OBFVG_OTE_REACTION_NEXT_OPEN_T1'
...
def smart_money_pois(ks, lbar, dbar):
    """Return only OB / OB-FVG overlap zones. FVG solo is evidence, not entry."""
    raw = ld.demand_pois(ks, lbar, dbar)
    ob = next((p for p in raw if p.get('type') == 'OB_Demand'), None)
    fvg = next((p for p in raw if p.get('type') == 'FVG_Demand'), None)
    pois=[]
    if ob and fvg:
        ov = overlap_zone(ob, fvg)
        if ov:
            pois.append(ov)
    if ob:
        pois.append({'type':'OB_SMART_MONEY','bar':ob['bar'],'low':ob['low'],'high':ob['high'],'origin':'last_down_before_displacement'})
    return pois
...
def audit(ts):
    ...
    for t in ts:
        ...
        if t['zone_type'] not in ('OB_SMART_MONEY','OB_FVG_SMART_MONEY'):
            fails.append({'symbol':t.get('symbol'),'entry_date':t.get('entry_date'),'issue':'fvg_solo_not_allowed'})
...
def main():
    ...
    (OUT_DIR/'v71_trades.json').write_text(json.dumps(all_trades, ensure_ascii=False, indent=2))
    (OUT_DIR/'v71_picks.json').write_text(json.dumps(picks, ensure_ascii=False, indent=2))
    (OUT_DIR/'v71_report.json').write_text(json.dumps(report, ensure_ascii=False, indent=2))
    print(json.dumps({'metrics':report['metrics'],'audit':report['audit'],'buckets':report['buckets'],'files':{'trades':str(OUT_DIR/'v71_trades.json'),'picks':str(OUT_DIR/'v71_picks.json'),'report':str(OUT_DIR/'v71_report.json')}}, ensure_ascii=False, indent=2)[:12000])
