#!/usr/bin/env python3
"""V491 one-shot frozen strict-T+1 replay for weekly BOS-demand transfer."""
from __future__ import annotations
import csv,json,math,statistics
from collections import Counter,defaultdict
from datetime import datetime
from pathlib import Path
ROOT=Path('/root/.hermes');KDIR=ROOT/'kline_cache';AUD=ROOT/'smc_audit';SRC=AUD/'v490_weekly_bos_demand_transfer_oracle_latest.json'
OUT=AUD/f"v491_weekly_bos_demand_transfer_frozen_t1_no_write_{datetime.now():%Y%m%d_%H%M%S}";LATEST=AUD/'v491_weekly_bos_demand_transfer_frozen_t1_replay_latest.json'
...
STOP={'WEEKLY_DEMAND_SL_T1','SL_GAP_T1','SL_TP_COLLISION_CONSERVATIVE_T1'}
...
def replay(seed,bars,targets):
    ...
    for i in range(first,last+1):
        ...
        if hs:x,price,reason=i,sl,'WEEKLY_DEMAND_SL_T1';break
...
def main():
    ...
    if src.get('decision')!='WEEKLY_BOS_DEMAND_ORACLE_PASS__FROZEN_T1_REPLAY_ALLOWED':raise RuntimeError('V490 gate failed')
    ...
    for i,(sym,items) in enumerate(grouped.items(),1):
        ...
        for s in items:rows.append({**s,'execution_contract':'NEXT_OPEN__WEEKLY_DEMAND_SL1PCT__VISIBLE_WEEKLY_BSL__TIME30__T1__FEE0P2',**replay(s,bars,targets)})
    ...
    file=OUT/'v491_frozen_t1_rows.csv';fields=sorted({k for r in rows for k in r})
    with file.open('w',newline='') as h:w=csv.DictWriter(h,fieldnames=fields);w.writeheader();w.writerows(rows)
    result={'version':'V491_WEEKLY_BOS_DEMAND_TRANSFER_FROZEN_T1_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,
      'frozen_before_outcomes':{'entry':'next open after daily hold','sl':'weekly demand zone_low*0.99','target':'nearest confirmed weekly swing high visible by hold','exit':'strict T+1 target/SL/time30, gap-aware, collision=SL','fee_pct':FEE,'search_count':1,'promotion_gate':GATE},
      ...
      'decision':'WEEKLY_BOS_DEMAND_PROMOTION_GATE_PASS__INDEPENDENT_METRIC_AUDIT_NEXT' if passed else 'WEEKLY_BOS_DEMAND_ECONOMIC_GATE_FAIL__CLOSE_ONTOLOGY_NO_VARIANTS','artifacts':{'out_dir':str(OUT),'rows':str(file),'latest':str(LATEST)}}
    text=json.dumps(result,ensure_ascii=False,indent=2);(OUT/'v491_report.json').write_text(text);LATEST.write_text(text);print(text)
