#!/usr/bin/env python3
"""V416 one-shot frozen T+1 structural replay for V415 structure-flip POIs.

Execution is fixed before outcomes: next-session open after takeover; no exit on
entry day; SL 0.5% below the flip-zone low; TP at the nearest higher confirmed
swing liquidity visible by takeover; 20-session time exit. Per-symbol execution
...
OUT = AUD / f'v416_structure_flip_frozen_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUD / 'v416_structure_flip_frozen_t1_replay_latest.json'
MAX_HOLD = 20
FEE_PCT = 0.15
...
def target_before_entry(swings, takeover_idx, entry_price):
    candidates = [(f(x['price']), int(x['confirm_idx'])) for x in swings.get('highs', [])
                  if int(x['confirm_idx']) <= takeover_idx and f(x['price']) > entry_price]
    return min(candidates, key=lambda x: x[0]) if candidates else (0.0, None)


def simulate(seed, bars, swings):
    takeover_idx = int(seed['takeover_idx'])
    entry_idx = takeover_idx + 1
    ...
    tp, target_confirm_idx = target_before_entry(swings, takeover_idx, entry)
    risk = entry - sl
...
def main():
    ...
    for (combo, symbol), group in grouped.items():
        ...
        busy_until = -1
        for seed in sorted(group, key=lambda x: int(x['takeover_idx'])):
            prospective_entry = int(seed['takeover_idx']) + 1
    ...
    rows_path = OUT / 'v416_trade_rows.csv'
    ...
    report = {
        'version': 'V416_STRUCTURE_FLIP_FROZEN_T1_STRUCTURAL_REPLAY_NO_WRITE',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'source': str(SOURCE),
        'execution_contract': 'takeover -> next-session open; entry-day exit forbidden; SL=zone_low*0.995; TP=nearest higher confirmed swing visible by takeover; SL-first on ambiguous bar; time exit=20 sessions; fee=0.15%; serial per symbol/combo',
        'production_write': False,
        'frontend_write': False,
        'watchlist_write': False,
        'takeover_seeds': len(seeds),
        'trades': len(rows),
        ...
        'invariants': {
            ...
            'negative_time_order': sum(not (int(r['event_idx']) < int(r['touch_idx']) <= int(r['reclaim_idx']) < int(r['takeover_idx']) < int(r['entry_idx']) <= int(r['exit_idx'])) for r in rows),
            'future_target_violation': sum(bool(str(r['target_confirm_idx']).strip()) and int(r['target_confirm_idx']) > int(r['takeover_idx']) for r in rows),
