#!/usr/bin/env python3
"""V429: one frozen T+1 diagnostic replay of independently audited V427 R5 seeds."""
...
SOURCE = AUD / 'v427_po3_breaker_latest.json'
INTEGRITY = AUD / 'v428_po3_breaker_integrity_latest.json'
OUT = AUD / f'v429_po3_breaker_frozen_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUD / 'v429_po3_breaker_frozen_t1_replay_latest.json'
...
def main():
    ...
    with (OUT / 'v429_mark_rows.csv').open('w', newline='') as handle:
    ...
    with (OUT / 'v429_yearly_metrics.csv').open('w', newline='') as handle:
    ...
    report = {
        'version': 'V429_PO3_BREAKER_FROZEN_T1_REPLAY_NO_WRITE',
        ...
        'decision': 'R5_FROZEN_DIAGNOSTIC_COMPLETE__PROMOTION_ONLY_IF_ANNUAL_GATE_PASSES',
        'artifacts': {'out_dir': str(OUT), 'rows': str(OUT / 'v429_mark_rows.csv'), 'yearly_metrics': str(OUT / 'v429_yearly_metrics.csv'), 'latest': str(LATEST)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v429_report.json').write_text(text); LATEST.write_text(text); print(text)
