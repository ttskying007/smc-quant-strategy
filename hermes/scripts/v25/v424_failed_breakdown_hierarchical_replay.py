#!/usr/bin/env python3
"""V424 corrected hierarchical replay for V422 failed-breakdown breakers."""
...
OUT=AUD/f'v424_failed_breakdown_hierarchical_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST=AUD/'v424_failed_breakdown_hierarchical_replay_latest.json'; FEE=.2; HOLD=20
...
def highs(ks):
    return [(i,f(ks[i]['h']),i+10) for i in range(10,len(ks)-10) if all(f(ks[i]['h'])>f(ks[j]['h']) for j in range(i-10,i+11) if j!=i)]
def met(rows):
...
def main():
    ...
    for s in seeds:
        ...
        ei=ti+1; entry=f(ks[ei]['o']); sl=f(s['zone_low'])*.995
    ...
    rp=OUT/'v424_trade_rows.csv'; fields=list(rows[0]) if rows else ['symbol']
    ...
    report={'version':'V424_FAILED_BREAKDOWN_HIERARCHICAL_STRUCTURAL_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),
      'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,'source':str(SRC),
      'frozen_execution_contract':'takeover->next-session open; SL=breaker body low*0.995; TP=nearest higher confirmed pre-entry 10L/10R macro swing high; SL-first; 20-session time exit; 0.2% cost',
