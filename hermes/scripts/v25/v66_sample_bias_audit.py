#!/usr/bin/env python3
"""V66 sample bias / filter funnel audit."""
from __future__ import annotations
import json, collections
from pathlib import Path
ROOT=Path('/root/.hermes')
SNAP=ROOT/'smc_opt_v50_signal/v50_signal_snapshot.json'
TR=ROOT/'smc_opt_v66/v66_trades.json'
PICKS=ROOT/'smc_opt_v66/v66_picks.json'
OUT=ROOT/'smc_audit/v66_sample_bias_audit.json'
...
def main():
    ...
    active=[p for p in picks if p.get('pick_scope') in ('ACTIVE_ENTRY','ACTIVE_CANDIDATE') or p.get('is_active_pick')]
    reject={
        'snapshot_symbols_without_trade': len(set(snap)-trade_syms),
        'trade_symbols': len(trade_syms),
        'active_entry_count': len(active),
        'active_candidate_count': sum(1 for p in picks if p.get('pick_scope')=='ACTIVE_CANDIDATE' or p.get('is_active_pick')),
        'watch_only_count': sum(1 for p in picks if p.get('pick_scope')=='WATCH_ONLY'),
    ...
    watch_count = out['funnel'].get('near_zone_watch_count',0) + out['funnel'].get('watch_only_count',0)
    if len(active)<=1 and watch_count < 100:
