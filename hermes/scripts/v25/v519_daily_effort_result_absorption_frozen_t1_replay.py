#!/usr/bin/env python3
"""V519 single frozen strict-T+1 replay for V517 effort-result absorption.
...
OUT=AUD/f'v519_daily_effort_result_absorption_frozen_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST=AUD/'v519_daily_effort_result_absorption_frozen_t1_replay_latest.json'
...
def main()->None:
    ...
    g=json.loads(V517.read_text()); o=json.loads(V518.read_text())
    if not g.get('support_gate_pass') or not o.get('oracle_pass') or o.get('outcomes_opened'):
        raise RuntimeError('V517/V518 gates not satisfied; frozen replay blocked')
    ...
    csv_path=OUT/'v519_frozen_t1_trades.csv'; fields=list(executed[0].keys()) if executed else []
    if fields:
    ...
    period_artifacts = write_period_reports(
        executed, out_dir=OUT, stem='v519', engine='V519_FROZEN_STRICT_T1_REPLAY',
    ...
    report={'version':'V519_DAILY_EFFORT_RESULT_ABSORPTION_FROZEN_T1_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,'frozen_execution_contract':'eligible-day open; stop=sweep low*0.99; nearest prior visible confirmed swing high target; exits begin following session; SL-first collision; time20; round-trip fee0.20%; serial one-position-per-symbol','source_contract':g['frozen_contract'],'seed_count':len(seeds),'closed_trade_count':len(executed),'open_data_count':len(open_data),'nontradable_or_serial_skip_counts':dict(skipped),'overall':overall,'yearly':yearly,'promotion_gate':GATE,'monthly_trade_count_gate':monthly_gate,'monthly_trade_count_gate_pass':monthly_trade_count_pass,'promotion_checks':promotion,'invariants':invariant,'promotion_gate_pass':all(promotion.values()),'decision':'V519_PROMOTION_GATE_PASS__SCANNER_TIME_CONTRACT_REQUIRED' if all(promotion.values()) else 'V519_FROZEN_REPLAY_FAIL__CLOSE_ONTOLOGY__NO_VARIANTS','artifacts':{'out_dir':str(OUT),'trades':str(csv_path),'period_metrics':period_artifacts['json'],'yearly_metrics':period_artifacts['yearly_csv'],'monthly_metrics':period_artifacts['monthly_csv'],'latest':str(LATEST),'v517':str(V517),'v518':str(V518)}}
    text=json.dumps(report,ensure_ascii=False,indent=2);(OUT/'v519_report.json').write_text(text);LATEST.write_text(text);print(text)
