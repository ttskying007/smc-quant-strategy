#!/usr/bin/env python3
"""V685 no-write, independent lifecycle-safe W->D->60m identity oracle.
...
OUT = AUDIT / f'v685_independent_lifecycle_safe_wdh_identity_oracle_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUDIT / 'v685_independent_lifecycle_safe_wdh_identity_oracle_latest.json'
...
def main() -> None:
    ...
    if source.get('decision') != 'V677_SOURCE_AND_SEMANTIC_AUDIT_PASS__OUTCOME_BLIND_STATE_MACHINE_SEEDS_ALLOWED':
        raise SystemExit('V677 did not authorize V685')
    ...
    for n, path in enumerate(sorted(DAILY.glob('*_raw_daily.json.gz')), 1):
        ...
        if n % 250 == 0:
            print(f'V685 progress {n}: terminal_rows={len(all_rows)} errors={len(errors)}', flush=True)
    fields = sorted({k for row in all_rows for k in row})
    csv_path = OUT / 'v684_terminal_chain_rows.csv'
    ...
    decision = 'V685_INDEPENDENT_LIFECYCLE_ORACLE_IDENTITIES_READY__COMPARE_TO_FROZEN_V684' if not errors and len(identities) == len(ready) and chronology else 'V685_INDEPENDENT_LIFECYCLE_ORACLE_FAIL__STOP_BEFORE_COMPARE'
    report = {'version': 'V685_INDEPENDENT_LIFECYCLE_SAFE_WDH_IDENTITY_ORACLE_NO_WRITE', 'generated_at': datetime.now().isoformat(timespec='seconds'), 'no_write': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False, 'source_gate': source.get('decision'), 'semantic_detector': 'V677 primitives_b independent formulation plus separately audited lifecycle hard-cancels', 'input_symbols': len(list(DAILY.glob('*_raw_daily.json.gz'))), 'terminal_counts': dict(terminal), 'ready_chain_count': len(ready), 'unique_ready_identity_count': len(identities), 'chronology_pass': chronology, 'symbol_errors': len(errors), 'error_samples': errors[:100], 'decision': decision, 'artifact': str(csv_path)}
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v685_report.json').write_text(text, encoding='utf-8'); LATEST.write_text(text, encoding='utf-8')
    print(text)
