#!/usr/bin/env python3
"""V595 outcome-blind PIT holder-demand commitment -> demand-OB retest seeds."""
...
PRE = AUDIT / 'v594_holder_demand_commitment_preregistration_latest.json'
LATEST = AUDIT / 'v595_holder_demand_commitment_seed_latest.json'
OUT = AUDIT / f'v595_holder_demand_commitment_seed_no_outcome_{datetime.now():%Y%m%d_%H%M%S}'
YEARS = ('2023', '2024', '2025')
EXCLUDE = ('进展', '实施完成', '实施结果', '完成', '结果', '终止', '调整', '更正', '时间过半', '期限过半', '到期', '届满', '律师事务所', '法律意见书', '实施情况')
INCLUDE = ('增持计划', '增持公司股份', '增持股份')
...
def events() -> list[dict]:
    ...
    with META.open(encoding='utf-8') as handle:
        for line in handle:
            ...
            if (item.get('kind') != 'HOLDER_INCREASE' or date[:4] not in YEARS or
                    not any(term in title for term in INCLUDE) or any(term in title for term in EXCLUDE)):
                continue
            rows.append({'symbol': str(item.get('symbol') or ''), 'announcement_id': str(item.get('announcement_id') or ''),
                         ...
                         'external_event': 'HOLDER_DEMAND_COMMITMENT', 'title': title})
    canonical = {}
    ...
    for break_i in range(start, min(start + 30, len(rows))):
        ...
        for reclaim_i in range(break_i + 1, min(break_i + 16, len(rows))):
            ...
            if bar['l'] <= zone_high and bar['h'] >= zone_low and bar['c'] >= zone_high and reclaim_i + 1 < len(rows):
                entry_i = reclaim_i + 1
                return 'SEED', {**event, 'response_start_date': rows[start]['d'], 'bsl_anchor_date': rows[bsl_i]['d'],
                    'bsl_anchor_confirm_date': rows[bsl_confirm_i]['d'], 'bsl_break_date': rows[break_i]['d'],
                    'poi_date': rows[poi_i]['d'], 'zone_low': round(zone_low, 6), 'zone_high': round(zone_high, 6),
                    'reclaim_date': rows[reclaim_i]['d'], 'planned_entry_date': rows[entry_i]['d'],
                    'causal_path': 'PIT_HOLDER_DEMAND_COMMITMENT>CONFIRMED_BSL_BREAK>DEMAND_OB_RETEST_RECLAIM>NEXT_OPEN'}
...
def main() -> None:
    ...
    if preregistration['decision'] != 'PREREGISTRATION_COMPLETE__OUTCOME_BLIND_SEED_GENERATION_AUTHORIZED':
        raise RuntimeError('V594 preregistration required')
    ...
    for name, data in (('v595_all_holder_demand_events.csv', all_rows), ('v595_outcome_blind_seeds.csv', seeds)):
    ...
    checks = {'canonical_seed_total': len(seeds) >= 1000, 'canonical_seed_each_year': all(seed_years[y] >= 300 for y in YEARS), 'unique_symbols': len({x['symbol'] for x in seeds}) >= 500}
    invariants = {'no_outcome_or_trade_files_read': True, 'all_events_precede_response': all(x['event_date'] < x['response_start_date'] for x in seeds), 'all_bsl_anchors_confirmed_before_break': all(x['bsl_anchor_confirm_date'] < x['bsl_break_date'] for x in seeds), 'all_causal_nodes_before_entry': all(x['event_date'] < x['bsl_break_date'] < x['reclaim_date'] < x['planned_entry_date'] for x in seeds), 'one_seed_per_symbol_entry_date': len(seeds) == len({(x['symbol'], x['planned_entry_date']) for x in seeds})}
    passed = all(checks.values()) and all(invariants.values())
    report = {'version': 'V595_HOLDER_DEMAND_COMMITMENT_SEED_NO_OUTCOME', 'generated_at': datetime.now().isoformat(timespec='seconds'), 'research_only': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False, 'input_contract': 'V594 frozen holder-demand title rule plus daily OHLC only through each planned entry; no outcome/trade/PnL/exit/target/stop file read.', 'raw_canonical_event_count': len(source), 'raw_canonical_event_years': event_years, 'seed_status_counts': dict(Counter(x['seed_status'] for x in all_rows)), 'canonical_seed_count': len(seeds), 'canonical_seed_years': seed_years, 'unique_symbols': len({x['symbol'] for x in seeds}), 'support_checks': checks, 'invariants': invariants, 'decision': 'V595_SUPPORT_PASS__INDEPENDENT_RAW_ORACLE_REQUIRED' if passed else 'V595_SUPPORT_INSUFFICIENT__NO_OUTCOMES_OPENED__CLOSE_ONTOLOGY', 'artifacts': {'out_dir': str(OUT), 'all_events': str(OUT / 'v595_all_holder_demand_events.csv'), 'seeds': str(OUT / 'v595_outcome_blind_seeds.csv'), 'latest': str(LATEST)}}
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v595_report.json').write_text(text); LATEST.write_text(text); print(text)
