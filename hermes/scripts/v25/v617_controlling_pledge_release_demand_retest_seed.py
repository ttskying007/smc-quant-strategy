#!/usr/bin/env python3
"""V617: outcome-blind seeds for frozen V616 pledge-release ontology."""
...
PRE = AUDIT / 'v616_controlling_pledge_release_demand_retest_preregistration_latest.json'
LATEST = AUDIT / 'v617_controlling_pledge_release_demand_retest_seed_latest.json'
OUT = AUDIT / f'v617_controlling_pledge_release_demand_retest_seed_no_outcome_{datetime.now():%Y%m%d_%H%M%S}'
...
def event_rows(catalog: dict) -> list[dict]:
    ...
    for line in source.open(encoding='utf-8'):
        ...
        eligible = (a_share(symbol) and event_date[:4] in YEARS and item.get('event_kind') == 'CONTROLLING_HOLDER_PLEDGE_RELEASE')
        if eligible:
            row = {
                'symbol': symbol,
                ...
                'title': title,
                'event_kind': 'CONTROLLING_HOLDER_PLEDGE_RELEASE',
                'external_event': 'PIT_CONTROLLING_HOLDER_PLEDGE_RELEASE',
            }
            ...
            if bar['l'] <= high and bar['h'] >= low and bar['c'] >= high and reclaim_i + 1 < len(rows):
                ...
                return 'SEED', {
                    **event,
                    ...
                    'causal_path': 'PIT_CONTROLLING_HOLDER_PLEDGE_RELEASE>CONFIRMED_BSL_ACCEPTANCE>DEMAND_OB_RETEST_RECLAIM>NEXT_OPEN',
                }
...
def main() -> None:
    ...
    if catalog['decision'] != 'SOURCE_CATALOG_COMPLETE__SEMANTIC_PREREGISTRATION_NEXT' or preregistration['decision'] != 'PREREGISTRATION_COMPLETE__OUTCOME_BLIND_SEED_GENERATION_AUTHORIZED':
        raise RuntimeError('V615 catalog and V616 preregistration required')
    ...
    for name, rows in [('v617_all_pledge_release_events.csv', all_rows), ('v617_outcome_blind_seeds.csv', seeds)]:
    ...
    report = {
        'version': 'V617_CONTROLLING_HOLDER_PLEDGE_RELEASE_DEMAND_RETEST_SEED_NO_OUTCOME',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'research_only': True,
        'production_write': False,
        'frontend_write': False,
        'watchlist_write': False,
        'input_contract': 'V615 immutable controlling-holder pledge-release metadata plus daily OHLC only through planned entry; no outcome/trade/PnL/exit/MFE/MAE file read.',
        ...
        'decision': 'V617_SUPPORT_PASS__INDEPENDENT_RAW_ORACLE_REQUIRED' if passed else 'V617_SUPPORT_INSUFFICIENT__NO_OUTCOMES_OPENED__CLOSE_ONTOLOGY',
        'artifacts': {'out_dir': str(OUT), 'all_events': str(OUT / 'v617_all_pledge_release_events.csv'), 'seeds': str(OUT / 'v617_outcome_blind_seeds.csv'), 'latest': str(LATEST)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v617_report.json').write_text(text)
