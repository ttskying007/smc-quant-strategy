#!/usr/bin/env python3
"""V618: independent raw identity Oracle for V617; outcome data is prohibited."""
...
LATEST = AUDIT / 'v618_v617_independent_raw_oracle_latest.json'
OUT = AUDIT / f'v618_v617_independent_raw_oracle_no_outcome_{datetime.now():%Y%m%d_%H%M%S}'
...
def source_events(catalog: dict) -> dict[str, set[str]]:
    ...
    for line in Path(catalog['artifacts']['events']).open(encoding='utf-8'):
        try:
            ...
            continue
        symbol, day = str(item.get('symbol') or ''), d8(item.get('notice_date'))
        if a_share(symbol) and day[:4] in YEARS and item.get('event_kind') == 'CONTROLLING_HOLDER_PLEDGE_RELEASE':
...
def main() -> None:
    ...
    identities = OUT / 'v618_oracle_identities.csv'
    ...
    report = {
        'version': 'V618_V617_INDEPENDENT_RAW_ORACLE_NO_OUTCOME',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'input_contract': 'V617 expected identities plus immutable V615 catalog and daily OHLC only; no outcome, trade, PnL, exit, target, stop, or replay file read.',
        'independent_rebuild': 'Separately rebuilds exact eligible controlling-holder pledge-release dates and post-event BSL acceptance -> demand-OB retest identities without importing V617 code.',
        ...
        'decision': 'V618_ORACLE_PASS__ONE_FROZEN_STRICT_T1_REPLAY_AUTHORIZED' if matched else 'V618_ORACLE_FAIL__NO_REPLAY_ALLOWED',
        'artifacts': {'out_dir': str(OUT), 'oracle_identities': str(identities), 'latest': str(LATEST)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v618_report.json').write_text(text)
