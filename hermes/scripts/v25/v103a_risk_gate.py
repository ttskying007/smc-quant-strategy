#!/usr/bin/env python3
"""V103-A: V102 balanced volume gate plus unified risk_pct >= 0.7 production gate.

Root cause fixed:
- V101 production kept only V100 A_PRODUCTION_CORE, which left the 2024-2026
  backtest window with 54 trades.
- Full audit showed a stable higher-volume subset: A/B/C robust tiers inside
  MIXED market with daily DOWN_STRUCTURE, preserving high WR while increasing
  trade count.
...
OUT_DIR = ROOT / 'smc_opt_v103a_risk_gate'
...
ENGINE = 'V103A_RISK_GATE'
FEE_PCT = 0.12
NET_SUCCESS_PCT = 0.8

BALANCED_TIER_WHITELIST = {
    'A_PRODUCTION_CORE',
    'B_OBSERVE_HIGH_WR',
    'C_ROBUST_OBSERVE_ONLY',
}
BALANCED_MARKET_STATE = 'MIXED'
BALANCED_DAILY_STRUCTURE = 'DOWN_STRUCTURE'
...
def enrich_row(row: Dict[str, Any], dna: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    balanced_volume_gate = (
        x.get('v100_tier') in BALANCED_TIER_WHITELIST
        and x.get('market_state') == BALANCED_MARKET_STATE
        and x.get('daily_structure_state') == BALANCED_DAILY_STRUCTURE
        and fnum(x.get('tp2_rr')) >= 5.0
        and fnum(x.get('expected_tp2_net_pct')) >= NET_SUCCESS_PCT
        and 0 < fnum(x.get('risk_pct')) <= 1.5
    )
    v103a_risk_gate = fnum(x.get('risk_pct')) >= 0.7
    ...
    x['v102_balanced_volume_gate'] = bool(balanced_volume_gate)
    x['v103a_risk_gate'] = bool(v103a_risk_gate)
    x['production_eligible_v102'] = bool((x['production_eligible_v101'] or balanced_volume_gate) and v103a_risk_gate)
    ...
    x['production_grade_v101'] = (
        'A_PRODUCTION' if x['production_eligible_v102']
...
def field_missing(rows: List[Dict[str, Any]]) -> Dict[str, int]:
    keys = [
        ...
        'production_whitelist_v101', 'production_eligible_v101', 'v102_balanced_volume_gate', 'v103a_risk_gate', 'production_eligible_v102', 'production_grade_v101',
...
def main() -> None:
    ...
    production = [r for r in enriched_trades if r.get('production_eligible_v102')]
    bos_continuation_candidates = [r for r in enriched_trades if r.get('combo_candidate_eligible_v101')]
    active_production_picks = [r for r in enriched_active if r.get('production_eligible_v102')]
    candidate_picks = [r for r in enriched_active + enriched_watch if not r.get('production_eligible_v102')]

    report = {
        'engine': ENGINE,
        'version': 'V103-A',
        'source': str(SRC_DIR),
        'contract': 'V103-A risk gate: preserve V102 production contract and require unified risk_pct >= 0.7 for both V101 core and balanced-volume paths.',
        'balanced_tier_whitelist': sorted(BALANCED_TIER_WHITELIST),
        'balanced_market_state': BALANCED_MARKET_STATE,
        'balanced_daily_structure': BALANCED_DAILY_STRUCTURE,
        ...
        'v103a_risk_gate': 'risk_pct >= 0.7 applied after V101 core OR V102 balanced-volume production eligibility',
    ...
    payloads = {
        'v103a_trades.json': enriched_trades,
        'v103a_active_picks.json': active_production_picks,
        'v103a_candidate_picks.json': candidate_picks,
        'v103a_bos_continuation_candidates.json': bos_continuation_candidates,
        'v103a_symbol_dna.json': dna,
        'v103a_report.json': report,
        'v101_trades.json': enriched_trades,
        'v101_active_picks.json': active_production_picks,
        'v101_candidate_picks.json': candidate_picks,
        'v101_bos_continuation_candidates.json': bos_continuation_candidates,
        'v101_symbol_dna.json': dna,
        'v101_report.json': report,
    }
