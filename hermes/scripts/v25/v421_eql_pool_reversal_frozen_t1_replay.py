#!/usr/bin/env python3
"""V421: single frozen T+1 diagnostic replay for V420 R3 EQL-pool reversal.

No signal, entry, threshold, exit, or promotion search.  It only evaluates the
V420 takeover-confirmed rows at the next session open and fixed 5/10/20D marks.
"""
...
SOURCE = AUD / 'v420_eql_pool_reversal_latest.json'
OUT = AUD / f'v421_eql_pool_reversal_frozen_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUD / 'v421_eql_pool_reversal_frozen_t1_replay_latest.json'
...
def annual_gate(metrics_by_year: dict[str, dict]) -> bool:
    return all(
        x.get('n', 0) >= GATE['min_year_n']
        ...
        for x in metrics_by_year.values()
    )
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        if entry <= 0:
            ...
            continue
        row = {k: seed[k] for k in ('symbol', 'combo_key', 'sweep_date', 'event_date', 'poi_date', 'zone_low', 'zone_high', 'takeover_date')}
    ...
    annual_pass = {horizon: annual_gate(by_year) for horizon, by_year in annual.items()}
    sample_possible = len(rows) >= len(YEARS) * GATE['min_year_n']

    with (OUT / 'v421_mark_rows.csv').open('w', newline='') as handle:
        fields = list(rows[0]) if rows else ['symbol', 'combo_key']
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        writer.writerows(rows)
    annual_rows = []
    for horizon, by_year in annual.items():
        for year, values in by_year.items():
            annual_rows.append({'horizon_sessions': horizon, 'entry_year': year, **values})
    with (OUT / 'v421_yearly_metrics.csv').open('w', newline='') as handle:
        fields = list(annual_rows[0]) if annual_rows else ['horizon_sessions', 'entry_year']
        writer = csv.DictWriter(handle, fieldnames=fields)
        writer.writeheader()
        writer.writerows(annual_rows)

    report = {
        'version': 'V421_EQL_POOL_REVERSAL_FROZEN_T1_REPLAY_NO_WRITE',
        ...
        'sample_can_ever_meet_gate': sample_possible,
        'invariants': {
            'all_t1_compliant': all(not r['t1_violation'] for r in rows),
            'no_execution_parameter_search': True,
            'no_production_promotion': True,
            'no_outcome_fields_in_input': source['invariants']['no_outcomes'],
        },
        'decision': ('R3_INSUFFICIENT_FULL_HISTORY_SUPPORT__NO_PROMOTION_OR_THRESHOLD_MINING' if not sample_possible else 'R3_DIAGNOSTIC_COMPLETE__CHECK_FROZEN_ANNUAL_GATE'),
        'artifacts': {'out_dir': str(OUT), 'rows': str(OUT / 'v421_mark_rows.csv'), 'yearly_metrics': str(OUT / 'v421_yearly_metrics.csv'), 'latest': str(LATEST)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v421_report.json').write_text(text)
