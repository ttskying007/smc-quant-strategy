#!/usr/bin/env python3
"""V700 exact scanner-time materialization for V697/V519.
...
# FIX(2026-08-17): V697/V698 previously pointed at the OLD v517/v520 chain artifacts,
# which made admission read the stale v520 audit_pass and mislabel the real chain.
# Now they point at the actual V697->V698->V699 lineage outputs.
V697=AUD/'v697_pure_smc_ssl_reclaim_seed_gate_latest.json'
V698=AUD/'v698_pure_smc_ssl_reclaim_oracle_latest.json'
V699=AUD/'v699_pure_smc_ssl_reclaim_replay_latest.json'
OUT=AUD/f'v700_pure_smc_ssl_reclaim_current_scanner_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST=AUD/'v700_pure_smc_ssl_reclaim_current_scanner_latest.json'
...
def main():
    ...
    g=json.loads(V697.read_text());a=json.loads(V698.read_text());rp=json.loads(V699.read_text());manifest=json.loads((MON/'kline_epoch_current.json').read_text())
    md=d(manifest.get('market_date'))
    # FIX(2026-08-17): admission must reflect the ACTUAL V697/V698/V699 lineage:
    # seed support gate + independent oracle + frozen replay promotion gate.
    admission_eligible = bool(g.get('support_gate_pass')) and bool(a.get('oracle_pass')) and bool(rp.get('promotion_gate_pass')) and manifest.get('status')=='COMMITTED'
    ...
    for p in sorted(KD.glob('*_daily_750.json')):
        ...
        if r:rows.append(r)
    blocked_by={'v697_support_gate':bool(g.get('support_gate_pass')),'v698_independent_oracle':bool(a.get('oracle_pass')),'v699_frozen_replay_promotion_gate':bool(rp.get('promotion_gate_pass')),'committed_epoch':manifest.get('status')=='COMMITTED'}
    decision=(
        'V700_PENDING_NEXT_OPEN_ROWS__AWAIT_NEXT_COMMITTED_OPEN_VALIDATION' if rows and admission_eligible else
        'V700_CURRENT_CANDIDATES_BLOCKED_BY_RESEARCH_GATE__NO_PRODUCTION_WRITE' if rows else
        'V700_NO_CURRENT_PENDING_ROWS__NO_HISTORICAL_FALLBACK'
    )
    report={'version':'V700_PURE_SMC_SSL_RECLAIM_SCANNER_TIME_DRY_RUN_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,'epoch_id':manifest.get('epoch_id'),'market_date':md,'files_seen':seen,'files_on_committed_date':fresh,'contract':g['frozen_contract'],'scanner_contract':'only response_date == committed market_date; scanner emits outcome-blind current candidates only; production admission requires independent research/release gates; next-open acceptance checks are required before BUY_VALID','admission_eligible':bool(admission_eligible),'pending_next_open_count':len(rows),'buy_valid_count':0,'rows':rows,'diagnostic_funnel':{'contract':'current-date only; every partial row is outcome-blind, non-executable and not a pending order','counts':funnel,'partial_rows':partial_rows,'full_current_setup_count':funnel['full_current_setup'],'release_blocker':('V699_FROZEN_REPLAY_PROMOTION_GATE_FAILED' if not rp.get('promotion_gate_pass') else None)},'invariants':{'all_rows_response_on_market_date':all(x['response_date']==md for x in rows),'all_partial_rows_response_on_market_date':all(x['response_date']==md for x in partial_rows),'no_historical_trade_source':True,'no_outcome_fields':all(not any(k in x for k in ('pnl','exit','mfe','mae','entry_price')) for x in rows+partial_rows),'all_production_writes_false':True},'blocked_by':blocked_by,'decision':decision,'artifacts':{'out_dir':str(OUT),'latest':str(LATEST),'v697':str(V697),'v698':str(V698),'v699':str(V699)}}
