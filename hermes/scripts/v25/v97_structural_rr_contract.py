...
OUT = Path('/root/.hermes/smc_opt_v97_structural_rr_contract')
OUT.mkdir(parents=True, exist_ok=True)
ENGINE = 'V97_STRUCTURAL_RR_CONTRACT'
SIGNAL_LAYER = 'V85_SIGNAL_LAYER_ZONE_ENTRY_RECOVERY'
CONTRACT_SOURCE = 'V97_STRUCTURAL_SL_TP_5R_GATE'
RECENT_BARS = 45
MAX_HOLD = 80
...
def classify(rr2: float, rr3: float, has_struct_sl: bool, tp2_type: str, tp3_type: str) -> str:
...
def build_contract(c: Dict[str, Any], ks: List[Dict[str, Any]]) -> Dict[str, Any] | None:
    ...
    grade = classify(rr2, rr3, not sl_mode.startswith('FALLBACK'), tp2.get('target_type','') if tp2 else '', tp3.get('target_type','') if tp3 else '')
    substate = recovery_substate(c, ks)
    row = dict(c)
    row.update({
        'engine': ENGINE, 'signal_engine': SIGNAL_LAYER, 'contract_source': CONTRACT_SOURCE,
        'v97_structural_contract': True, 'v91_gate_reason': gate, 'v90_recovery_substate': substate,
        'entry_mode': entry_mode, 'sl_mode': sl_mode, 'tp_mode': 'STRUCTURAL_TARGET_SPACE_GATE',
        ...
        'setup_status': 'V97_STRUCTURAL_PRODUCTION' if grade == 'A_PRODUCTION' else grade,
...
def main() -> None:
    ...
    (OUT/'v97_structural_trades.json').write_text(json.dumps(all_rows, ensure_ascii=False, indent=2))
    (OUT/'v97_active_picks.json').write_text(json.dumps(active, ensure_ascii=False, indent=2))
    (OUT/'v97_report.json').write_text(json.dumps(report, ensure_ascii=False, indent=2))
