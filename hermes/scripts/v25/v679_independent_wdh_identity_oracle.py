#!/usr/bin/env python3
"""V679 independent outcome-blind oracle for frozen V678 W->D->60m chain identities.
...
V678 = AUDIT / 'v678_outcome_blind_wdh_state_machine_seeds_latest.json'
OUT = AUDIT / f'v679_independent_wdh_identity_oracle_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUDIT / 'v679_independent_wdh_identity_oracle_latest.json'
...
def symbol_chains(symbol: str, daily: list[dict], h60: list[dict]) -> list[dict]:
    ...
    de, he, we = core.primitives_b(daily, 'D'), core.primitives_b(h60, 'M60'), core.primitives_b(weekly, 'W')
    di, hi = index_by_time(daily), index_by_time(h60)
    dssl, dbull, dob = by_type(de, 'SSL_SWEEP_RECLAIM'), by_type(de, 'BULL_BREAK'), by_type(de, 'BULL_EVENT_OB')
    hssl, hbull, hob = by_type(he, 'SSL_SWEEP_RECLAIM'), by_type(he, 'BULL_BREAK'), by_type(he, 'BULL_EVENT_OB')
    ...
    for d1 in dssl:
        ...
        row.update({'h60_reclaim_time': h60[reclaim_i]['t'], 'h60_hold_time': h60[reclaim_i + 1]['t'], 'next_h60_open_time': h60[reclaim_i + 2]['t'], 'terminal': 'SEED_READY'})
        results.append(row)
    return results


def main() -> None:
    source = json.loads(V678.read_text())
    if source.get('decision') != 'V678_OUTCOME_BLIND_CHAIN_SEEDS_READY__INDEPENDENT_IDENTITY_ORACLE_REQUIRED':
        raise SystemExit('V678 artifact contract did not authorize independent oracle')
    ...
    for n, path in enumerate(sorted(DAILY.glob('*_raw_daily.json.gz')), 1):
        ...
        if n % 250 == 0:
            print(f'V678 progress {n}: terminal_rows={len(all_rows)} errors={len(errors)}', flush=True)
    fields = sorted({k for row in all_rows for k in row})
    csv_path = OUT / 'v678_terminal_chain_rows.csv'
    ...
    decision = 'V679_INDEPENDENT_ORACLE_IDENTITIES_READY__COMPARE_TO_FROZEN_V678' if not errors and len(identities) == len(ready) and chronology else 'V679_ORACLE_CONTRACT_FAIL__STOP_BEFORE_COMPARISON'
    report = {'version': 'V679_INDEPENDENT_WDH_IDENTITY_ORACLE_NO_WRITE', 'generated_at': datetime.now().isoformat(timespec='seconds'), 'no_write': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False, 'source_gate': source.get('decision'), 'semantic_detector': 'V677 primitives_b independent formulation', 'input_symbols': len(list(DAILY.glob('*_raw_daily.json.gz'))), 'terminal_counts': dict(terminal), 'ready_chain_count': len(ready), 'unique_ready_identity_count': len(identities), 'chronology_pass': chronology, 'symbol_errors': len(errors), 'error_samples': errors[:100], 'decision': decision, 'artifact': str(csv_path), 'oracle_identity_fields': ['symbol','weekly_permission_time','daily_ssl_time','daily_break_time','daily_ob_time','daily_first_touch_time','h60_ssl_time','h60_break_time','h60_ob_time','h60_hold_time']}
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v678_report.json').write_text(text, encoding='utf-8'); LATEST.write_text(text, encoding='utf-8')
    print(text)
