#!/usr/bin/env python3
"""V509 one-shot frozen serial strict-T+1 replay for weekly IFVG support."""
from __future__ import annotations
import csv,json,math,statistics
from collections import Counter,defaultdict
from datetime import datetime
from pathlib import Path
ROOT=Path('/root/.hermes');KDIR=ROOT/'kline_cache';AUD=ROOT/'smc_audit';SRC=AUD/'v508_weekly_ifvg_support_oracle_latest.json'
OUT=AUD/f"v509_weekly_ifvg_support_frozen_t1_no_write_{datetime.now():%Y%m%d_%H%M%S}";LATEST=AUD/'v509_weekly_ifvg_support_frozen_t1_replay_latest.json'
...
STOP={'WEEKLY_IFVG_SL_T1','SL_GAP_T1','SL_TP_COLLISION_CONSERVATIVE_T1'}
...
def replay(seed,bars,targets):
    ...
    for i in range(first,last+1):
        ...
        if hs:x,price,reason=i,sl,'WEEKLY_IFVG_SL_T1';break
...
def main():
    ...
    if src.get('decision')!='WEEKLY_IFVG_SUPPORT_ORACLE_PASS__FROZEN_T1_REPLAY_ALLOWED':raise RuntimeError('V508 gate failed')
    ...
    for i,(sym,items) in enumerate(grouped.items(),1):
        bars=load(sym);targets=weekly_targets(bars);busy_until=-1
        for s in sorted(items,key=lambda x:integer(x['eligible_entry_idx'])):
            entry_idx=integer(s['eligible_entry_idx'])
            if entry_idx<=busy_until:
                rows.append({**s,'status':'OVERLAP_SUPPRESSED_SERIAL','entry_idx':entry_idx});continue
            result=replay(s,bars,targets)
            rows.append({**s,'execution_contract':'NEXT_OPEN__WEEKLY_IFVG_SL1PCT__VISIBLE_WEEKLY_BSL__TIME30__SERIAL_T1__FEE0P2',**result})
    ...
    file=OUT/'v509_frozen_t1_rows.csv';fields=sorted({k for r in rows for k in r})
    with file.open('w',newline='') as h:w=csv.DictWriter(h,fieldnames=fields);w.writeheader();w.writerows(rows)
    result={'version':'V509_WEEKLY_IFVG_SUPPORT_FROZEN_T1_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,
      'frozen_before_outcomes':{'entry':'next open after daily IFVG hold','sl':'weekly IFVG zone_low*0.99','target':'nearest confirmed weekly swing high visible by hold','exit':'strict T+1 target/SL/time30, gap-aware, collision=SL','serial':'one open setup per symbol; overlaps suppressed','fee_pct':FEE,'search_count':1,'promotion_gate':GATE},
      ...
      'decision':'WEEKLY_IFVG_SUPPORT_PROMOTION_GATE_PASS__INDEPENDENT_METRIC_AUDIT_NEXT' if passed else 'WEEKLY_IFVG_SUPPORT_ECONOMIC_GATE_FAIL__INDEPENDENT_METRIC_AUDIT_THEN_CLOSE','artifacts':{'out_dir':str(OUT),'rows':str(file),'latest':str(LATEST)}}
    text=json.dumps(result,ensure_ascii=False,indent=2);(OUT/'v509_report.json').write_text(text);LATEST.write_text(text);print(text)
