...
TRADES = ROOT / 'smc_opt_v65' / 'v65_trades.json'
...
def main():
    ...
    (OUT / 'v65_closed_loop_90d_review.json').write_text(json.dumps(report, ensure_ascii=False, indent=2))
    ...
    for k, v in report['closed_loop_schedule'].items():
        md.append(f"- {k}: {v}\n")
    (OUT / 'v65_closed_loop_90d_review.md').write_text(''.join(md))
