#!/usr/bin/env python3
"""V624: one frozen strict-T+1 replay for V622 after exact V623 Oracle equality."""
...
SEED = AUDIT / 'v622_controlling_pledge_creation_ssl_exhaustion_seed_latest.json'
ORACLE = AUDIT / 'v623_v622_independent_raw_oracle_latest.json'
LATEST = AUDIT / 'v624_v622_frozen_strict_t1_replay_latest.json'
OUT = AUDIT / f'v624_v622_frozen_strict_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
...
def main() -> None:
    ...
    if oracle['decision'] != 'V623_ORACLE_PASS__ONE_FROZEN_STRICT_T1_REPLAY_AUTHORIZED':
        raise RuntimeError('V623 exact Oracle pass required')
    ...
    for count, (symbol, items) in enumerate(sorted(grouped.items()), 1):
        ...
        for seed in sorted(items, key=lambda row: (row['planned_entry_date'], row['event_date'], row['announcement_id'])):
            ...
            trades.append({
                ...
                'execution_contract': 'PIT_CONTROLLING_HOLDER_PLEDGE_CREATE_D_PRIOR>SSL_SWEEP>LH_BREAK>DEMAND_RECLAIM>D_PLUS_1_OPEN>STRICT_T1_STRUCTURE_SL_TP_TIME20_FEE0P2',
            })
        if count % 500 == 0:
    ...
    path = OUT / 'v624_frozen_t1_trades.csv'
    ...
    report = {
        'version': 'V624_V622_ONE_FROZEN_STRICT_T1_REPLAY',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'research_only': True,
        'production_write': False,
        'frontend_write': False,
        'watchlist_write': False,
        'input_contract': 'V622 outcome-blind controlling-holder pledge-creation+SSL-exhaustion+SMC seeds after V623 exact independent raw Oracle identity equality.',
        ...
        'decision': 'V624_RESEARCH_GATE_PASS__INDEPENDENT_METRIC_AUDIT_REQUIRED' if passed else 'V624_FROZEN_REPLAY_GATE_FAIL__CLOSE_V622_ONTOLOGY_NO_VARIANTS',
        'artifacts': {'out_dir': str(OUT), 'trades': str(path), 'latest': str(LATEST), 'v622': str(SEED), 'v623': str(ORACLE)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v624_report.json').write_text(text)
