#!/usr/bin/env python3
"""V614: independent metrics and chronology audit for the single V613 frozen replay."""
...
REPLAY = AUDIT / 'v613_v611_frozen_strict_t1_replay_latest.json'
LATEST = AUDIT / 'v614_v611_independent_metric_audit_and_closure_latest.json'
YEARS = ('2023', '2024', '2025')
GATE = {'n_min': 1000, 'year_n_min': 300, 'wr_pct_min': 55.0, 'avg_net_pct_min': 0.50, 'pf_min': 1.15, 'payoff_min': 0.70, 't1_violations': 0}
CONTRACT = 'PIT_PROFIT_DISTRIBUTION_PLAN_D_PRIOR>BSL_ACCEPTANCE>DEMAND_RECLAIM>D_PLUS_1_OPEN>STRICT_T1_STRUCTURE_SL_TP_TIME20_FEE0P2'
...
def main() -> None:
    ...
    report = {
        'version': 'V614_V611_INDEPENDENT_METRIC_AUDIT_AND_CLOSURE_NO_WRITE',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'lineage': {
            'catalog': str(AUDIT / 'v609_cash_dividend_plan_event_catalog_latest.json'),
            'preregistration': str(AUDIT / 'v610_profit_distribution_demand_retest_preregistration_latest.json'),
            'seed': str(AUDIT / 'v611_profit_distribution_demand_retest_seed_latest.json'),
            'oracle': str(AUDIT / 'v612_v611_independent_raw_oracle_latest.json'),
        ...
        'mechanism_diagnosis': {
            'finding': 'PIT profit-distribution-plan -> confirmed BSL acceptance -> demand-OB retest/reclaim has full outcome-blind support but fails the fixed quality gate under its preregistered frozen next-open execution.',
            'evidence': [
        ...
        'decision': 'CLOSED_ECONOMIC__V611_PROFIT_DISTRIBUTION_DEMAND_RETEST__NO_SELECTOR_TIMING_STOP_TARGET_HOLD_CALENDAR_SYMBOL_OR_THRESHOLD_VARIANTS',
