#!/usr/bin/env python3
"""V320 fast no-write raw generator audit.
Reduced but representative matrix after first full grid timed out.
"""
...
OUT=AUD/f'v320_fast_raw_compression_breakout_retest_no_write_{TS}'; LATEST=AUD/'v320_fast_raw_compression_breakout_retest_latest.json'
...
PARAMS=[
 {'lookback':15,'max_range':10,'break_buf':1.0,'retest_wait':5,'retest_tol':1.0},
 {'lookback':20,'max_range':12,'break_buf':1.0,'retest_wait':5,'retest_tol':1.0},
 {'lookback':20,'max_range':15,'break_buf':1.0,'retest_wait':8,'retest_tol':1.0},
 {'lookback':30,'max_range':15,'break_buf':1.0,'retest_wait':8,'retest_tol':1.5},
 {'lookback':20,'max_range':12,'break_buf':1.5,'retest_wait':8,'retest_tol':1.0},
 {'lookback':30,'max_range':18,'break_buf':1.5,'retest_wait':8,'retest_tol':1.5},
]
...
def gen(sym,bars,p):
 ...
 for i in range(L+1,len(bars)-25):
  win=bars[i-L:i]; hi=max(x['h'] for x in win); lo=min(x['l'] for x in win); rng=pct(hi,lo)
  if rng is None or rng>p['max_range']: continue
  b=bars[i]; rngbar=b['h']-b['l']
  if b['c']<=hi*(1+p['break_buf']/100) or b['c']<=b['o'] or (b['c']-b['l'])/(rngbar+1e-9)<0.65: continue
  dz_low=lo; dz_high=hi
  for j in range(i-1,max(i-L,0),-1):
   if bars[j]['c']<=bars[j]['o']:
    dz_low=min(bars[j]['l'],bars[j]['o'],bars[j]['c']); dz_high=max(bars[j]['o'],bars[j]['c']); break
  for r in range(i+1,min(len(bars)-2,i+1+p['retest_wait'])):
   rb=bars[r]
   if not (rb['l']<=hi*(1+p['retest_tol']/100) and rb['c']>=hi and rb['c']>rb['o']): continue
   ei=r+1
   if ei<=last+5: break
   entry=bars[ei]['o']; sl=min(dz_low,rb['l'])*0.995; risk=pct(entry,sl)
   if risk is None or risk<2 or risk>9: continue
   meta={'event_type':'RAW_COMPRESSION_BREAKOUT_RETEST_RECLAIM','breakout_date':b['t'],'retest_date':rb['t'],'lookback':L,'compression_range_pct':round(rng,4),'break_buf':p['break_buf'],'retest_wait':p['retest_wait'],'retest_tol':p['retest_tol'],'risk_pct':round(risk,4),'zone_low':round(dz_low,4),'zone_high':round(dz_high,4),'prior_range_high':round(hi,4),'raw_generator':'V320_FAST'}
...
def main():
 ...
 for p in PARAMS:
  ...
  for ex in EXITS:
   ...
   key=f"L{p['lookback']}_R{p['max_range']}_B{p['break_buf']}_W{p['retest_wait']}_T{p['retest_tol']}_RR{ex['rr']}_H{ex['hold']}"
 ...
 report={'version':'V320_FAST_RAW_COMPRESSION_BREAKOUT_RETEST_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,'gate':GATE,'scanned_files':len(files),'usable_symbols':len(series),'param_count':len(PARAMS),'exit_count':len(EXITS),'results_count':len(results),'production_pass_count':len(passes),'production_pass_top10':passes[:10],'frontier_top30':ranked[:30],'best':best,'decision':'V320_FAST_PRODUCTION_PASS__REQUIRES_CURRENT_SCANNER_DRYRUN' if passes else 'V320_FAST_NO_PRODUCTION_PASS__RAW_COMPRESSION_BREAKOUT_RETEST_CLOSED','artifacts':{'report':str(OUT/'v320_fast_report.json'),'all_results':str(OUT/'v320_fast_all_results.json'),'best_rows':str(OUT/'v320_fast_best_rows.json'),'latest':str(LATEST)}}
 json.dump(report,open(OUT/'v320_fast_report.json','w'),ensure_ascii=False,indent=2); json.dump(ranked,open(OUT/'v320_fast_all_results.json','w'),ensure_ascii=False,indent=2); json.dump(best_rows,open(OUT/'v320_fast_best_rows.json','w'),ensure_ascii=False,indent=2); json.dump(report,open(LATEST,'w'),ensure_ascii=False,indent=2)
