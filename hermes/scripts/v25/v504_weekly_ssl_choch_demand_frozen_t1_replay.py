#!/usr/bin/env python3
"""V504 one-shot frozen strict-T+1 serial replay for weekly SSL/CHOCH/demand transfer."""
...
ROOT=Path('/root/.hermes');KDIR=ROOT/'kline_cache';AUD=ROOT/'smc_audit';SRC=AUD/'v503_weekly_ssl_choch_demand_oracle_latest.json'
OUT=AUD/f"v504_weekly_ssl_choch_demand_frozen_t1_no_write_{datetime.now():%Y%m%d_%H%M%S}";LATEST=AUD/'v504_weekly_ssl_choch_demand_frozen_t1_replay_latest.json'
...
STOP={'WEEKLY_RAID_STRUCTURE_SL_T1','SL_GAP_T1','SL_TP_COLLISION_CONSERVATIVE_T1'}
...
def replay(seed,bars,targets):
    ...
    entry=bars[e]['o'];sl=f(seed['weekly_raid_low'])*.99;target=visible_target(targets,seed['hold_date'],entry)
    ...
    for i in range(first,last+1):
        ...
        if hit_sl:x,price,reason=i,sl,'WEEKLY_RAID_STRUCTURE_SL_T1';break
...
def main():
    ...
    if src.get('decision')!='WEEKLY_SSL_CHOCH_DEMAND_ORACLE_PASS__FROZEN_T1_REPLAY_ALLOWED':raise RuntimeError('V503 gate failed')
    ...
    for n,(sym,items) in enumerate(grouped.items(),1):
        bars=load(sym);targets=completed_week_highs(bars);busy_until=-1
        for seed in sorted(items,key=lambda x:integer(x['eligible_entry_idx'])):
            ...
            result=replay(seed,bars,targets);rows.append({**seed,'execution_contract':'NEXT_OPEN__WEEKLY_RAID_STRUCTURE_SL1PCT__VISIBLE_WEEKLY_BSL__TIME30__SERIAL_T1__FEE0P2',**result})
    ...
    file=OUT/'v504_frozen_t1_rows.csv';fields=sorted({k for r in rows for k in r})
    with file.open('w',newline='') as h:w=csv.DictWriter(h,fieldnames=fields);w.writeheader();w.writerows(rows)
    result={'version':'V504_WEEKLY_SSL_CHOCH_DEMAND_FROZEN_T1_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,
      'frozen_before_outcomes':{'entry':'next open after post-CHOCH daily touch/reclaim/hold','sl':'weekly raid low*0.99','target':'nearest higher confirmed weekly swing high visible by hold','exit':'strict T+1 target/SL/time30, gap-aware, collision=SL','serial':'one open setup per symbol; overlapping entries suppressed','fee_pct':FEE,'search_count':1,'promotion_gate':GATE},
      ...
      'decision':'WEEKLY_SSL_CHOCH_DEMAND_PROMOTION_GATE_PASS__INDEPENDENT_METRIC_AUDIT_NEXT' if passed else 'WEEKLY_SSL_CHOCH_DEMAND_ECONOMIC_GATE_FAIL__INDEPENDENT_METRIC_AUDIT_THEN_CLOSE','artifacts':{'out_dir':str(OUT),'rows':str(file),'latest':str(LATEST)}}
    text=json.dumps(result,ensure_ascii=False,indent=2);(OUT/'v504_report.json').write_text(text);LATEST.write_text(text);print(text)
