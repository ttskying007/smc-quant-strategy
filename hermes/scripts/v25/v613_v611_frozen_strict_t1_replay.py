#!/usr/bin/env python3
"""V613: one frozen strict-T+1 replay for V611 after exact V612 Oracle equality."""
...
ORACLE = AUDIT / 'v612_v611_independent_raw_oracle_latest.json'
LATEST = AUDIT / 'v613_v611_frozen_strict_t1_replay_latest.json'
OUT = AUDIT / f'v613_v611_frozen_strict_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
...
def main() -> None:
    ...
    if oracle['decision'] != 'V612_ORACLE_PASS__ONE_FROZEN_STRICT_T1_REPLAY_AUTHORIZED':
        raise RuntimeError('V612 exact Oracle pass required')
    ...
    for count, (symbol, items) in enumerate(sorted(grouped.items()), 1):
        ...
        for seed in sorted(items, key=lambda row: (row['planned_entry_date'], row['event_date'], row['announcement_id'])):
            ...
            trades.append({
                ...
                'execution_contract': 'PIT_PROFIT_DISTRIBUTION_PLAN_D_PRIOR>BSL_ACCEPTANCE>DEMAND_RECLAIM>D_PLUS_1_OPEN>STRICT_T1_STRUCTURE_SL_TP_TIME20_FEE0P2',
            })
        if count % 500 == 0:
    ...
    path = OUT / 'v613_frozen_t1_trades.csv'
    ...
    report = {
        'version': 'V613_V611_ONE_FROZEN_STRICT_T1_REPLAY',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'research_only': True,
        'production_write': False,
        'frontend_write': False,
        'watchlist_write': False,
        'input_contract': 'V611 outcome-blind profit-distribution+SMC seeds after V612 exact independent raw Oracle identity equality.',
        ...
        'decision': 'V613_RESEARCH_GATE_PASS__INDEPENDENT_METRIC_AUDIT_REQUIRED' if passed else 'V613_FROZEN_REPLAY_GATE_FAIL__CLOSE_V611_ONTOLOGY_NO_VARIANTS',
        'artifacts': {'out_dir': str(OUT), 'trades': str(path), 'latest': str(LATEST), 'v611': str(SEED), 'v612': str(ORACLE)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v613_report.json').write_text(text)
