#!/usr/bin/env python3
"""V541 one frozen strict-T+1 replay for V539/V540 Sina m15 SMC identities.

Frozen before opening outcomes:
- entry = the V539 identity's next 15m bar open;
...
V540 = AUDIT / 'v540_sina_m15_ssl_bos_fvg_independent_oracle_latest.json'
OUT = AUDIT / f'v541_sina_m15_ssl_bos_fvg_frozen_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUDIT / 'v541_sina_m15_ssl_bos_fvg_frozen_t1_replay_latest.json'
LEFT = RIGHT = 3
STOP_BUFFER = 0.997
MIN_RR = 1.50
...
def target_cache(rows: list[dict[str, Any]], pivots: set[int], index: dict[str, int], seeds: list[dict[str, str]], highs: list[list[float]]) -> dict[tuple[str, str], tuple[int, float] | None]:
    ...
    for seed in seeds:
        if seed['entry_time'] not in index or seed['sweep_time'] not in index:
            continue
        entry_i, sweep_i = index[seed['entry_time']], index[seed['sweep_time']]
        entry, stop = rows[entry_i]['o'], rows[sweep_i]['l'] * STOP_BUFFER
        key = (seed['sweep_time'], seed['entry_time'])
        if stop < entry:
...
def replay(seed: dict[str, str], rows: list[dict[str, Any]], index: dict[str, int], target_choice: tuple[int, float] | None) -> dict[str, Any]:
    needed = (seed['sweep_time'], seed['entry_time'])
    if any(stamp not in index for stamp in needed):
        return {'status': 'SKIP', 'reason': 'SEED_TIME_NOT_IN_SOURCE_CACHE'}
    sweep_index, entry_index = index[seed['sweep_time']], index[seed['entry_time']]
...
def main() -> None:
    gate, oracle = json.loads(V539.read_text()), json.loads(V540.read_text())
    if gate.get('decision') != 'V539_SUPPORT_PASS__INDEPENDENT_ORACLE_REQUIRED' or oracle.get('decision') != 'V540_ORACLE_PASS__ONE_FROZEN_PARTIAL_RANGE_REPLAY_AUTHORIZED':
        raise RuntimeError('V539/V540 preregistered gates have not passed')
    ...
    for symbol, symbol_seeds in by_symbol.items():
        ...
        for seed in symbol_seeds:
            if busy_until >= seed['entry_time']:
                ...
                continue
            result = replay(seed, rows, index, targets.get((seed['sweep_time'], seed['entry_time'])))
    ...
    invariant = {'frozen_seed_count_matches_oracle': len(seeds) == oracle['expected_identities'] == oracle['oracle_identities'], 'all_entries_after_reclaim': all(row['entry_time'] > row['reclaim_time'] for row in closed), 'all_targets_visible_pre_entry': all(row['target_pivot_time'] < row['entry_time'] for row in closed), 'all_planned_rr_gte_1_5': all(float(row['planned_rr']) >= MIN_RR for row in closed), 't1_violations': sum(bool(row['same_day_exit_violation']) for row in closed), 'all_writes_false': True}
    ...
    trade_file = OUT / 'v541_frozen_t1_trades.csv'
    if closed:
    ...
    report = {'version': 'V541_SINA_M15_SSL_BOS_FVG_FROZEN_T1_REPLAY_NO_WRITE', 'generated_at': datetime.now().isoformat(timespec='seconds'), 'research_only': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False, 'scope': 'SINA_SOURCE_ISOLATED_PARTIAL_RANGE_2025_04_TO_2026_07_ONLY__NOT_PRODUCTION', 'source_contract': 'Sina source-isolated m15 only; no Baostock/Tencent substitution.', 'frozen_execution_contract': 'next-15m open; stop=sweep low*0.997; nearest visible unconsumed 3L/3R high with planned RR>=1.5; exits only next trade date onward; stop-first; time80; fee0.20%; serial symbol position.', 'seed_count': len(seeds), 'closed_trade_count': len(closed), 'open_data_count': len(open_data), 'skip_counts': dict(skipped), 'overall': overall, 'yearly': yearly, 'promotion_gate': GATE, 'promotion_checks': checks, 'invariants': invariant, 'partial_research_gate_pass': all(checks.values()), 'decision': 'V541_PARTIAL_RANGE_RESEARCH_PASS__FULL_HISTORY_AND_PRODUCTION_STILL_BLOCKED' if all(checks.values()) else 'V541_PARTIAL_RANGE_RESEARCH_FAIL__CLOSE_OBJECT', 'artifacts': {'out_dir': str(OUT), 'trades': str(trade_file), 'latest': str(LATEST), 'v539': str(V539), 'v540': str(V540)}}
