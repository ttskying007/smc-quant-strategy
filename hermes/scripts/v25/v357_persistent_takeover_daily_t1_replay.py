#!/usr/bin/env python3
"""V357 research-only daily executable replay for V353 persistent takeovers.

Fixed, source-safe contract (not a parameter search):
- input only V353 PERSISTENT_TAKEOVER rows;
...
OUT = AUD / f'v357_persistent_takeover_daily_t1_replay_no_write_{STAMP}'
LATEST = AUD / 'v357_persistent_takeover_daily_t1_replay_latest.json'
HOLD_BARS = 30
STOP_BUFFER = 0.99
...
def main() -> None:
    ...
    for row in source_rows:
        ...
        rows.append({
            ...
            'ob_date': row.get('ob_date', ''), 'zone_low': row.get('zone_low', ''), 'zone_high': row.get('zone_high', ''),
            'touch_date': row.get('touch_date', ''), 'reclaim_date': row.get('reclaim_date', ''), 'takeover_date': row.get('takeover_date', ''),
            'lifecycle_state': row.get('lifecycle_state', ''), 'execution_contract': 'V357_PERSISTENT_TAKEOVER_DAILY_T1',
            'no_write': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False, **result,
        })
    fields = sorted({key for row in rows for key in row})
    with (OUT / 'v357_daily_t1_replay_rows.csv').open('w', newline='') as handle:
        writer = csv.DictWriter(handle, fieldnames=fields); writer.writeheader(); writer.writerows(rows)
    result = {
        'version': 'V357_PERSISTENT_TAKEOVER_DAILY_T1_REPLAY_NO_WRITE',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'no_write': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False,
        'input_contract': 'V353 PERSISTENT_TAKEOVER only',
        ...
        'artifacts': {'out_dir': str(OUT), 'rows': str(OUT / 'v357_daily_t1_replay_rows.csv'), 'latest': str(LATEST)},
    }
