#!/usr/bin/env python3
"""V66 trade provenance audit.

Audits existing candidate trades (V66 if present, otherwise V49) against the V66
signal snapshot. This makes gaps explicit before V66 engine exists.
"""
...
V66_TRADES = ROOT / 'smc_opt_v66' / 'v66_trades.json'
V49_TRADES = ROOT / 'smc_opt_v49_exit_optimized' / 'v49_trades.json'
OUT_JSON = ROOT / 'smc_audit' / 'v66_trade_provenance_audit.json'
OUT_MD = ROOT / 'smc_audit' / 'v66_trade_provenance_audit.md'
...
def _trade_file() -> Path:
    return V66_TRADES if V66_TRADES.exists() else V49_TRADES


def _families_for_key(key: str, trade: Dict[str, Any]) -> List[str]:
    # V66 engine writes explicit execution/confirmation overlay ids into trades.
...
def _check_idx(trade: Dict[str, Any], signals: List[Dict[str, Any]], key: str) -> Dict[str, Any]:
    ...
    if not sig:
        # Entry/exit are execution markers and may not exist in raw signal snapshot before V66 engine.
...
def main() -> None:
    ...
    summary = {
        ...
        'note': 'Before V66 engine, V49 trades may lack source_event_idx/zone_idx/conf_index. This audit exposes required fixes.'
    }
    out = {'summary': summary, 'rows': rows}
    OUT_JSON.write_text(json.dumps(out, ensure_ascii=False, indent=2))
    md = ['# V66 Trade Provenance Audit\n\n', '```json\n', json.dumps(summary, ensure_ascii=False, indent=2), '\n```\n\n']
    for r in fatal[:50]:
