#!/usr/bin/env python3
"""V487 one-shot frozen strict-T+1 replay for V485/V486."""
...
SRC=AUD/'v486_bsl_acceptance_retest_oracle_latest.json'
OUT=AUD/f"v487_bsl_acceptance_retest_frozen_t1_no_write_{datetime.now():%Y%m%d_%H%M%S}"
LATEST=AUD/'v487_bsl_acceptance_retest_frozen_t1_replay_latest.json'
...
STOP_REASONS={'STRUCTURAL_RETEST_SL_T1','SL_GAP_T1','SL_TP_COLLISION_CONSERVATIVE_T1'}
...
def replay(seed,bars):
    eligible=integer(seed['eligible_entry_idx']); confirm=integer(seed['reexpand_idx'])
    ...
    exit_idx=last; exit_price=bars[last]['c']; reason='TIME20_NEXT_BSL_UNREACHED'; collision=False
    for idx in range(first,last+1):
        bar=bars[idx]
        if bar['o']<=sl: exit_idx,exit_price,reason=idx,bar['o'],'SL_GAP_T1'; break
        if bar['o']>=target: exit_idx,exit_price,reason=idx,bar['o'],'NEXT_BSL_GAP_TP_T1'; break
        ...
        if hit_sl: exit_idx,exit_price,reason=idx,sl,'STRUCTURAL_RETEST_SL_T1'; break
        if hit_tp: exit_idx,exit_price,reason=idx,target,'NEXT_BSL_TP_T1'; break
...
def stats(rows):
    ...
    return {'n':len(rows),'gross_wr_pct':round(sum(x>0 for x in gross)/len(rows)*100,4),
      ...
      'payoff_rr':round(aw/abs(al),4) if al else 0,'profit_factor':round(sum(wins)/abs(sum(losses)),4) if losses and sum(losses) else 0,
...
def pass_gate(overall,yearly,t1):
    return (overall.get('n',0)>=GATE['n'] and overall.get('gross_wr_pct',0)>=GATE['gross_wr_pct']
      ...
      and all(yearly[y].get('n',0)>=GATE['each_year_n'] and yearly[y].get('gross_wr_pct',0)>=GATE['each_year_gross_wr_pct']
              and yearly[y].get('avg_net_pnl_pct',-999)>GATE['each_year_avg_net_pnl_pct'] for y in ('2023','2024','2025','2026')) and t1==0)


def main():
    source=json.loads(SRC.read_text())
    if source.get('decision')!='BSL_ACCEPTANCE_ORACLE_PASS__FROZEN_T1_REPLAY_ALLOWED': raise RuntimeError('V486 oracle gate not passed')
    ...
    for n,(sym,items) in enumerate(grouped.items(),1):
        bars=load(sym)
        for seed in items: rows.append({**seed,'execution_contract':'NEXT_OPEN__RETEST_LOW_1PCT_SL__NEXT_VISIBLE_BSL_TP__TIME20__STRICT_T1__FEE0P2',**replay(seed,bars)})
    ...
    row_file=OUT/'v487_frozen_t1_rows.csv'; fields=sorted({k for r in rows for k in r})
    with row_file.open('w',newline='') as h:
        w=csv.DictWriter(h,fieldnames=fields); w.writeheader(); w.writerows(rows)
    result={'version':'V487_BSL_ACCEPTANCE_RETEST_FROZEN_T1_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),
      'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,
      'frozen_before_outcomes':{'entry':'next open after re-expansion above retest high','sl':'retest low * 0.99','target':'nearest higher BSL visible before acceptance','exit':'strict T+1 target/SL/time20, gap-aware, collision=SL','fee_pct':FEE_PCT,'search_count':1,'promotion_gate':GATE},
      ...
      'promotion_gate_pass':passed,'decision':'BSL_ACCEPTANCE_FROZEN_REPLAY_PASS__INDEPENDENT_METRIC_AUDIT_NEXT' if passed else 'BSL_ACCEPTANCE_ECONOMIC_GATE_FAIL__INDEPENDENT_METRIC_AUDIT_THEN_CLOSE',
      'artifacts':{'out_dir':str(OUT),'rows':str(row_file),'latest':str(LATEST)}}
    text=json.dumps(result,ensure_ascii=False,indent=2); (OUT/'v487_report.json').write_text(text); LATEST.write_text(text); print(text)
