...
def structure_state_machine(klines: List[Dict], pivots: Dict, atr: List[float], level: str = 'swing', sweeps: Optional[List[Dict]] = None) -> List[Dict]:
    ...
    for i, b in enumerate(klines):
        ...
        # FIX(2026-08-22): 1-bar 不 retrace 确认（研究：BOS 42.9%→59.8% / CHOCH 45.7%→63.3%）
        # 触发后下一 bar close 保持在 pivot 上方/下方才确认；否则标记 failed（过滤假信号）
        for ev in events:
            if ev.get('confirm') == 'pending':
                if ev.get('direction') == 'bull' and b['c'] > ev.get('price', 0):
                    ev['confirm'] = 'confirmed'
                elif ev.get('direction') == 'bear' and b['c'] < ev.get('price', 0):
                    ev['confirm'] = 'confirmed'
                else:
                    ev['confirm'] = 'failed'
...
def sweep_signals_stateful(klines: List[Dict], pivots: Dict, eqs: List[Dict], atr: List[float], profile: Dict) -> List[Dict]:
    ...
    for i, b in enumerate(klines):
        emitted_dir = set()
        # 3-bar cooldown per direction: prevent same-direction sweeps within 3 bars
        if i >= 3:
            for d in ('bull', 'bear'):
                for prev in out[-3:]:
...
def fvg_list_pine_like(klines: List[Dict], atr: List[float], profile: Dict) -> List[Dict]:
    ...
    for i in range(2, len(klines)):
        ...
        # Bullish FVG per user's Pine visual rule: three consecutive bullish candles
        # with a true three-candle gap (current low > high[2]).
        gap = b['l'] - prev2['h']
        three_bull = prev2['c'] > prev2['o'] and prev['c'] > prev['o'] and b['c'] > b['o']
        if three_bull and gap > min_gap and prev['c'] > prev2['h'] and delta_pct >= auto_thresh * 0.35:
            f = {'type': 'FVG', 'direction': 'bull', 'index': i, 'date': _date(b),
                 'gap_low': prev2['h'], 'gap_high': b['l'], 'zone_low': prev2['h'], 'zone_high': b['l'],
                 'raw_zone_low': prev2['h'], 'raw_zone_high': b['l'], 'mid': (prev2['h'] + b['l']) / 2,
                 'width': gap, 'atr': atr[i], 'confidence': min(0.9, 0.55 + gap / max(atr[i], 1e-9) * 0.1),
                 'fvg_candle_rule': 'THREE_CONSECUTIVE_BULLISH',
                 'mitigation_type': '50% Fill'}
            f.update(attach_raw_display_fields(f, atr=atr[i], price=f['mid']))
            out.append(f)
        # Bearish FVG: three consecutive bearish candles with current high < low[2].
        gap = prev2['l'] - b['h']
        three_bear = prev2['c'] < prev2['o'] and prev['c'] < prev['o'] and b['c'] < b['o']
        if three_bear and gap > min_gap and prev['c'] < prev2['l'] and delta_pct >= auto_thresh * 0.35:
            f = {'type': 'FVG', 'direction': 'bear', 'index': i, 'date': _date(b),
                 'gap_low': b['h'], 'gap_high': prev2['l'], 'zone_low': b['h'], 'zone_high': prev2['l'],
                 'raw_zone_low': b['h'], 'raw_zone_high': prev2['l'], 'mid': (b['h'] + prev2['l']) / 2,
                 'width': gap, 'atr': atr[i], 'confidence': min(0.9, 0.55 + gap / max(atr[i], 1e-9) * 0.1),
                 'fvg_candle_rule': 'THREE_CONSECUTIVE_BEARISH',
                 'mitigation_type': '50% Fill'}
            f.update(attach_raw_display_fields(f, atr=atr[i], price=f['mid']))
            out.append(f)
    return out
...
def ob_signals_pine_like(klines: List[Dict], struct_events: List[Dict], atr: List[float], profile: Dict) -> List[Dict]:
    """
    Pine-quality OB detection.

    OB selection follows the corrected SMC rule: use the nearest opposite-color
    candle before the structure break. Displacement is metadata/quality, not a
    ...
    displacement_mult = float(profile.get('ob_displacement_mult', 1.3))
    ...
    for ev in struct_events:
        ...
        break_price = ev.get('break_price', klines[idx]['c'])
        found = None
        # Scan backward from the structure break and take the nearest opposite-color candle.
        start = max(0, idx - max_back)
        for j in range(idx - 1, start - 1, -1):
            b = klines[j]
            body = abs(b['c'] - b['o'])
            body_pct = body / max(b['o'], 0.001) * 100 if b['o'] > 0 else 0
            # Skip doji / tiny body candles (noise filter)
            min_body_atr = profile.get('ob_min_body_atr', 0.3)
            if body_pct < atr[j] * min_body_atr:
                continue
            # Bullish OB = nearest bearish candle before bullish break.
            if direction == 'bull' and b['c'] < b['o']:
                found = j
                break
            # Bearish OB = nearest bullish candle before bearish break.
        ...
        if direction == 'bull':
            ...
            invalidation = zh
        # Calculate displacement ratio for metadata/quality only.
        preceding_range = b['h'] - b['l']
        if direction == 'bull':
            disp_ratio = round((break_price - b['l']) / max(preceding_range, 0.001), 2) if preceding_range > 0 else 0
        else:
            disp_ratio = round((b['h'] - break_price) / max(preceding_range, 0.001), 2) if preceding_range > 0 else 0
        displacement_quality = max(0.0, min(1.0, disp_ratio / max(displacement_mult, 1e-9)))
        
        ob = {'type': 'OB', 'direction': direction, 'index': found, 'date': _date(b),
              'confirm_index': idx, 'confirm_date': ev['date'], 'source_event': ev['type'],
              'source_level': ev.get('source_level',''), 'level_type': ev.get('source_level',''),
              'structure_type': ev.get('type',''), 'zone_low': zl, 'zone_high': zh,
              'raw_zone_low': zl, 'raw_zone_high': zh, 'raw_bottom': zl, 'raw_top': zh,
              'invalidation': invalidation, 'mid': (zl + zh) / 2, 'mitigated': False,
              'displacement_ratio': disp_ratio, 'displacement_quality': round(displacement_quality, 3),
              'confidence': round((0.80 if ev.get('source_level') == 'swing' else 0.65) + displacement_quality * 0.05, 3)}
        ob.update(attach_raw_display_fields(ob, atr=atr[found], price=(zl+zh)/2))
        active.append(ob)
        active = active[-max_active:]
        out.append(ob)
        # Mitigate older active OBs up to current bar.
        # Fix: use wick touch (low <= zone_high), not close < zone_low
        for old in active:
            if old is ob or old.get('mitigated'):
                continue
            kb = klines[idx]
            if old['direction'] == 'bull' and kb['l'] <= old['zone_high']:
                old['mitigated'] = True; old['mitigated_idx'] = idx
            if old['direction'] == 'bear' and kb['h'] >= old['zone_low']:
                old['mitigated'] = True; old['mitigated_idx'] = idx
    return out
