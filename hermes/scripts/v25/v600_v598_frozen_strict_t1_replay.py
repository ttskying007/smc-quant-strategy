#!/usr/bin/env python3
"""One frozen strict-T+1 replay for V598 after V599 exact Oracle equality."""
...
SEED = AUDIT / 'v598_contract_award_demand_retest_seed_latest.json'
ORACLE = AUDIT / 'v599_v598_independent_raw_oracle_latest.json'
LATEST = AUDIT / 'v600_v598_frozen_strict_t1_replay_latest.json'
OUT = AUDIT / f'v600_v598_frozen_strict_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}'
YEARS = ('2024', '2025')
FEE, HOLD = 0.20, 20
GATE = {'n_min': 1000, 'year_n_min': 300, 'wr_pct_min': 55.0, 'avg_net_pct_min': 0.50,
        'pf_min': 1.15, 'payoff_min': 0.70, 'each_year_avg_net_positive': True, 't1_violations': 0}


def main() -> None:
    oracle = json.loads(ORACLE.read_text())
    if oracle['decision'] != 'V599_ORACLE_PASS__ONE_FROZEN_STRICT_T1_REPLAY_AUTHORIZED':
        raise RuntimeError('V599 exact oracle equality required')
    seed_report = json.loads(SEED.read_text())
    ...
    for count, (symbol, items) in enumerate(sorted(grouped.items()), 1):
        ...
        index = {row['d']: i for i, row in enumerate(stock_bars)}
        busy_until = -1
        for seed in sorted(items, key=lambda row: (row['planned_entry_date'], row['event_date'], row['announcement_id'])):
            ...
            busy_until = exit_i
            trades.append({
                'symbol': symbol, 'announcement_id': seed['announcement_id'], 'event_date': seed['event_date'],
                'signal_date': seed['reclaim_date'], 'entry_date': stock_bars[entry_i]['d'],
                'entry_price': round(entry, 8), 'stop_price': round(stop, 8), 'target_price': round(target, 8),
                'planned_rr': round((target - entry) / (entry - stop), 6), 'exit_date': exit_date,
                'exit_price': round(exit_price, 8), 'exit_reason': reason, 'hold_bars': exit_i - entry_i,
                'net_pnl_pct': round((exit_price / entry - 1) * 100 - FEE, 6),
                'execution_contract': 'PIT_CONTRACT_AWARD_D_PRIOR>BSL_ACCEPTANCE>DEMAND_RECLAIM>D_PLUS_1_OPEN>STRICT_T1_STRUCTURE_SL_TP_TIME20_FEE0P2',
            })
        if count % 500 == 0:
    ...
    trades_path = OUT / 'v600_frozen_t1_trades.csv'
    ...
    report = {
        'version': 'V600_V598_ONE_FROZEN_STRICT_T1_REPLAY', 'generated_at': datetime.now().isoformat(timespec='seconds'),
        'research_only': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False,
        'input_contract': 'V598 outcome-blind contract-award+SMC seeds after V599 exact independent raw Oracle identity equality.',
        ...
        'decision': 'V600_RESEARCH_GATE_PASS__INDEPENDENT_METRIC_AUDIT_REQUIRED' if passed else 'V600_FROZEN_REPLAY_GATE_FAIL__CLOSE_V598_ONTOLOGY_NO_VARIANTS',
        'artifacts': {'out_dir': str(OUT), 'trades': str(trades_path), 'latest': str(LATEST), 'v598': str(SEED), 'v599': str(ORACLE)},
    }
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v600_report.json').write_text(text)
