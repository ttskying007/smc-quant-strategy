#!/usr/bin/env python3
"""V620: independent metrics and chronology audit for the single V619 frozen replay."""
...
REPLAY = AUDIT / 'v619_v617_frozen_strict_t1_replay_latest.json'
LATEST = AUDIT / 'v620_v617_independent_metric_audit_and_closure_latest.json'
YEARS = ('2023', '2024', '2025')
GATE = {'n_min': 1000, 'year_n_min': 300, 'wr_pct_min': 55.0, 'avg_net_pct_min': 0.50, 'pf_min': 1.15, 'payoff_min': 0.70, 't1_violations': 0}
CONTRACT = 'PIT_CONTROLLING_HOLDER_PLEDGE_RELEASE_D_PRIOR>BSL_ACCEPTANCE>DEMAND_RECLAIM>D_PLUS_1_OPEN>STRICT_T1_STRUCTURE_SL_TP_TIME20_FEE0P2'
...
def main() -> None:
    ...
    report = {
        'version': 'V620_V617_INDEPENDENT_METRIC_AUDIT_AND_CLOSURE_NO_WRITE',
        'generated_at': datetime.now().isoformat(timespec='seconds'),
        'lineage': {
            'catalog': str(AUDIT / 'v615_controlling_pledge_pit_event_catalog_latest.json'),
            'preregistration': str(AUDIT / 'v616_controlling_pledge_release_demand_retest_preregistration_latest.json'),
            'seed': str(AUDIT / 'v617_controlling_pledge_release_demand_retest_seed_latest.json'),
            'oracle': str(AUDIT / 'v618_v617_independent_raw_oracle_latest.json'),
        ...
        'mechanism_diagnosis': {
            'finding': 'PIT controlling-holder pledge release -> confirmed BSL acceptance -> demand-OB retest/reclaim has full outcome-blind support but fails the fixed quality gate under its preregistered frozen next-open execution.',
            'evidence': [
        ...
        'decision': 'CLOSED_ECONOMIC__V617_CONTROLLING_HOLDER_PLEDGE_RELEASE_DEMAND_RETEST__NO_SELECTOR_TIMING_STOP_TARGET_HOLD_CALENDAR_SYMBOL_OR_THRESHOLD_VARIANTS',
