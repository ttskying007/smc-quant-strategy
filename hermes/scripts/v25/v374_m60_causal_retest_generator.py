#!/usr/bin/env python3
"""V374 no-write: full-history causal 60m sweep→MSS→OB-retest generator.

Input is the Sina 60m cache. A signal can use only bars already closed before
entry. Invalid 60m source days are hard boundaries. Entry is the next 60m open
only after sweep, MSS, first POI touch, reclaim, and one hold bar. Exits never
begin until the next trading date (A-share T+1).
"""
...
OUT = AUDIT / f'v374_m60_causal_retest_generator_no_write_{TS}'
LATEST = AUDIT / 'v374_m60_causal_retest_generator_latest.json'
...
def replay(bars: list[dict], entry_i: int, sl: float, tp: float) -> dict | None:
    ...
    for k in range(entry_i + 1, min(len(bars), entry_i + 41)):
        ...
        if b['d'] == eday:
            continue
        if b['l'] <= sl:
            return {'exit_i': k, 'exit_date': b['d'], 'exit_price': sl, 'reason': 'SL_HIT'}
        if b['h'] >= tp:
            return {'exit_i': k, 'exit_date': b['d'], 'exit_price': tp, 'reason': 'TP_HIT'}
    eligible = [k for k in range(entry_i + 1, min(len(bars), entry_i + 41)) if bars[k]['d'] != eday]
    if not eligible:
        return None
    k = eligible[-1]
    return {'exit_i': k, 'exit_date': bars[k]['d'], 'exit_price': bars[k]['c'], 'reason': 'TIME_10_SESSIONS'}
...
def one_symbol(symbol: str, path: Path) -> list[dict]:
    ...
    for i, b in enumerate(bars):
                    ...
                    if entry_i < len(bars):
                        ...
                        if 1.5 <= risk <= 10 and 1.5 <= rr <= 5:
                            ...
                            if result:
                                ...
                                out.append({'symbol': symbol, 'sweep_time': bars[s['sweep_i']]['t'], 'mss_time': bars[s['mss_i']]['t'],
                                            ...
                                            'exit_price': result['exit_price'], 'exit_reason': result['reason'],
        ...
        if i < 30 or i < cooldown or not known_lo or not known_hi:
            continue
        # SSL sweep of a confirmed pivot visible before this bar; closest valid pool wins.
        ...
        if pools and highs:
            ...
            active.append({'state': 'WAIT_MSS', 'sweep_i': i, 'pool_i': p, 'pool': pool,
                           'break_level': max(price for _, price in highs)})
    return out
...
def main() -> None:
    ...
    with (OUT / 'v374_trades.csv').open('w', newline='') as h:
    ...
    report = {'version': 'V374_FULL_HISTORY_CAUSAL_60M_SWEEP_MSS_OB_RETEST_NO_WRITE', 'generated_at': datetime.now().isoformat(timespec='seconds'),
              'no_write': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False,
              'contract': 'confirmed 3/3 SSL sweep -> close MSS -> event-anchored bearish OB -> first touch -> reclaim -> hold -> next 60m open; raw 60m only; T+1 exits',
              ...
              'artifacts': {'trades': str(OUT / 'v374_trades.csv'), 'latest': str(LATEST)}}
    text = json.dumps(report, ensure_ascii=False, indent=2); (OUT / 'v374_report.json').write_text(text); LATEST.write_text(text); print(text)
