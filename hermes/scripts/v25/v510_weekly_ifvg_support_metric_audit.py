#!/usr/bin/env python3
"""V510 independent metrics, chronology, and serial-position audit for V509."""
import csv,json,statistics
from collections import Counter,defaultdict
from datetime import datetime
from pathlib import Path
ROOT=Path('/root/.hermes');AUD=ROOT/'smc_audit';SRC=AUD/'v509_weekly_ifvg_support_frozen_t1_replay_latest.json';LATEST=AUD/'v510_weekly_ifvg_support_metric_audit_latest.json'
...
def main():
    ...
    for sym,rows in grouped.items():
        ...
        for a,b in zip(rows,rows[1:]):
            if int(b['entry_idx'])<=int(a['exit_idx']):overlaps.append((sym,a['entry_date'],a['exit_date'],b['entry_date']))
    chronology['serial_overlap_failures']=len(overlaps)
    passed=exact['overall'] and exact['exit_reason_counts'] and all(exact['yearly'].values()) and not any(chronology.values())
    result={'version':'V510_WEEKLY_IFVG_SUPPORT_INDEPENDENT_METRIC_AUDIT_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,'source_version':report['version'],'closed_rows':len(closed),'recomputed_overall':overall,'recomputed_yearly':yearly,'matches':exact,'invariants':{**chronology,'search_count':report['invariants']['search_count']},'audit_pass':passed,'promotion_gate_pass':bool(report['promotion_gate_pass'] and passed),'decision':'WEEKLY_IFVG_SUPPORT_VERIFIED_ECONOMIC_FAIL__CLOSE_ONTOLOGY_NO_VARIANTS' if passed and not report['promotion_gate_pass'] else ('WEEKLY_IFVG_SUPPORT_INDEPENDENT_PASS__PROMOTION_CONFIRMED' if passed else 'INDEPENDENT_AUDIT_FAIL__DO_NOT_USE_RESULT'),'closure_reason':'Aggregate WR clears 55% and payoff clears 0.7, but AvgNet and PF fail; 2023 and 2026 are negative. Do not tune inversion threshold, lifecycle window, SL, TP, hold, year, or regime.','artifacts':{'rows':report['artifacts']['rows'],'latest':str(LATEST)}}
