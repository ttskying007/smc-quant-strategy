#!/usr/bin/env python3
"""V605: outcome-blind seeds for the frozen V604 equity-incentive ontology."""
...
PRE = AUDIT / 'v604_equity_incentive_demand_retest_preregistration_latest.json'
LATEST = AUDIT / 'v605_equity_incentive_demand_retest_seed_latest.json'
OUT = AUDIT / f'v605_equity_incentive_demand_retest_seed_no_outcome_{datetime.now():%Y%m%d_%H%M%S}'
...
def event_rows(catalog: dict) -> list[dict]:
    ...
    for line in source.open(encoding='utf-8'):
        ...
        eligible = (len(symbol) == 9 and event_date[:4] in YEARS and '草案' in title and
                    any(term in title for term in INCLUDE) and not any(term in title for term in EXCLUDE))
        if eligible:
            row = {'symbol': symbol, 'announcement_id': str(item.get('announcement_id') or ''), 'event_date': event_date, 'publication_time': str(item.get('publication_time') or ''), 'title': title, 'matched_terms': '|'.join(item.get('matched_terms') or []), 'external_event': 'PIT_EQUITY_INCENTIVE_PLAN'}
...
def seed_for(event: dict, rows: list[dict]) -> tuple[str, dict | None]:
    dates, start = [row['d'] for row in rows], None
    ...
    for break_i in range(start, min(start + 30, len(rows))):
        ...
        for reclaim_i in range(break_i + 1, min(break_i + 16, len(rows))):
            ...
            if bar['l'] <= high and bar['h'] >= low and bar['c'] >= high and reclaim_i + 1 < len(rows):
                entry_i = reclaim_i + 1
                return 'SEED', {**event, 'response_start_date': rows[start]['d'], 'bsl_anchor_date': rows[bsl_i]['d'], 'bsl_anchor_confirm_date': rows[bsl_confirm_i]['d'], 'bsl_break_date': rows[break_i]['d'], 'poi_date': rows[poi_i]['d'], 'zone_low': round(low, 6), 'zone_high': round(high, 6), 'reclaim_date': rows[reclaim_i]['d'], 'planned_entry_date': rows[entry_i]['d'], 'causal_path': 'PIT_EQUITY_INCENTIVE_PLAN>CONFIRMED_BSL_ACCEPTANCE>DEMAND_OB_RETEST_RECLAIM>NEXT_OPEN'}
...
def main() -> None:
    ...
    if catalog['decision'] != 'SOURCE_CATALOG_COMPLETE__SEMANTIC_PREREGISTRATION_NEXT' or preregistration['decision'] != 'PREREGISTRATION_COMPLETE__OUTCOME_BLIND_SEED_GENERATION_AUTHORIZED':
        raise RuntimeError('V603 catalog and V604 preregistration required')
    ...
    for name, rows in [('v605_all_equity_incentive_events.csv', all_rows), ('v605_outcome_blind_seeds.csv', seeds)]:
    ...
    invariants = {'no_outcome_or_trade_files_read': True, 'all_events_precede_response': all(row['event_date'] < row['response_start_date'] for row in seeds), 'all_bsl_anchors_confirmed_before_break': all(row['bsl_anchor_confirm_date'] < row['bsl_break_date'] for row in seeds), 'all_causal_nodes_before_entry': all(row['event_date'] < row['bsl_break_date'] < row['reclaim_date'] < row['planned_entry_date'] for row in seeds), 'one_seed_per_symbol_entry_date': len(seeds) == len({(row['symbol'], row['planned_entry_date']) for row in seeds})}
    report = {'version': 'V605_EQUITY_INCENTIVE_DEMAND_RETEST_SEED_NO_OUTCOME', 'generated_at': datetime.now().isoformat(timespec='seconds'), 'research_only': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False, 'input_contract': 'V603 immutable equity-incentive metadata plus daily OHLC only through planned entry; no outcome/trade/PnL/exit/MFE/MAE file read.', 'raw_canonical_event_count': len(source), 'raw_canonical_event_years': {year: sum(row['event_date'].startswith(year) for row in source) for year in YEARS}, 'seed_status_counts': dict(Counter(row['seed_status'] for row in all_rows)), 'canonical_seed_count': len(seeds), 'canonical_seed_years': years, 'unique_symbols': len({row['symbol'] for row in seeds}), 'support_checks': checks, 'invariants': invariants, 'decision': 'V605_SUPPORT_PASS__INDEPENDENT_RAW_ORACLE_REQUIRED' if all(checks.values()) and all(invariants.values()) else 'V605_SUPPORT_INSUFFICIENT__NO_OUTCOMES_OPENED__CLOSE_ONTOLOGY', 'artifacts': {'out_dir': str(OUT), 'all_events': str(OUT / 'v605_all_equity_incentive_events.csv'), 'seeds': str(OUT / 'v605_outcome_blind_seeds.csv'), 'latest': str(LATEST)}}
    text = json.dumps(report, ensure_ascii=False, indent=2)
    (OUT / 'v605_report.json').write_text(text); LATEST.write_text(text); print(text)
