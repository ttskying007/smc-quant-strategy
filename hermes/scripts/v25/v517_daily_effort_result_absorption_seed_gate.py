#!/usr/bin/env python3
"""V517 outcome-blind support gate for a distinct daily volume/price ontology.
...
3) The sweep volume is in the top quintile of the preceding 20 completed
   sessions and the next completed bar closes above the sweep high.
...
AUD = ROOT / 'smc_audit'
OUT = AUD / f'v517_daily_effort_result_absorption_seed_gate_no_write_{datetime.now():%Y%m%d_%H%M%S}'
LATEST = AUD / 'v517_daily_effort_result_absorption_seed_gate_latest.json'
...
def scan_symbol(symbol: str, bars: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for i in range(start, len(bars) - 2):
        sweep = bars[i]
        response = bars[i + 1]
        prior_volume = [bars[k]['v'] for k in range(i - VOL_LOOKBACK, i)]
        rank = volume_rank_prior(prior_volume, sweep['v'])
        if not (rank >= VOL_TOP_QUINTILE and response['c'] > sweep['h']):
            continue
        ...
        seeds.append({
            ...
            'prior20_volume_rank': round(rank, 6),
            'response_idx': i + 1,
...
def main() -> None:
    ...
    csv_path = OUT / 'v517_outcome_blind_seeds.csv'
    ...
    report = {
        'version': 'V517_DAILY_EFFORT_RESULT_ABSORPTION_SEED_GATE_NO_WRITE',
        ...
        'constants': {'swing_left': SWING_LEFT, 'swing_right': SWING_RIGHT, 'sweep_pct': SWEEP_PCT, 'volume_lookback': VOL_LOOKBACK, 'volume_top_quintile': VOL_TOP_QUINTILE},
        ...
        'decision': 'V517_SUPPORT_PASS__INDEPENDENT_ORACLE_REQUIRED_BEFORE_SINGLE_FROZEN_T1_REPLAY' if support_pass else 'V517_SUPPORT_FAIL__CLOSE_WITHOUT_OUTCOMES__DO_NOT_RELAX',
