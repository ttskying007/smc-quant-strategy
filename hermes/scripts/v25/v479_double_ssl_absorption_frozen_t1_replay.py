#!/usr/bin/env python3
"""V479 one-shot frozen strict-T+1 replay for V477/V478.

The contract is fixed before outcomes: next open, deepest raid low * 0.99 SL,
nearest pre-entry confirmed BSL or time20, 0.2% fees, conservative collision.
No parameter or exit search is performed.
"""
...
SRC=AUD/'v478_double_ssl_absorption_oracle_latest.json'
OUT=AUD/f"v479_double_ssl_absorption_frozen_t1_replay_no_write_{datetime.now():%Y%m%d_%H%M%S}"
LATEST=AUD/'v479_double_ssl_absorption_frozen_t1_replay_latest.json'
...
def target_at(highs,cutoff,entry):
    known=[(price,pivot_date) for _,visible,price,pivot_date in highs if visible<=cutoff and price>entry]
    return min(known,default=(None,''),key=lambda x:x[0])
...
def replay(seed,bars,highs):
    ...
    for idx in range(first,last+1):
        ...
        if hit_sl:
            exit_idx,exit_price,reason=idx,sl,'STRUCTURAL_DOUBLE_RAID_SL_T1'; break
        if hit_tp:
...
def stats(rows):
    ...
    pf=sum(wins)/abs(sum(losses)) if losses and sum(losses) else 0
    return {'n':len(rows),'gross_wr_pct':round(sum(x>0 for x in gross)/len(rows)*100,4),
      ...
      'median_net_pnl_pct':round(statistics.median(net),4),'avg_win_pct':round(avg_win,4),'avg_loss_pct':round(avg_loss,4),
      'payoff_rr':round(avg_win/abs(avg_loss),4) if avg_loss else 0,'profit_factor':round(pf,4),
...
def main():
    ...
    if source.get('decision')!='DOUBLE_SSL_ABSORPTION_ORACLE_PASS__FROZEN_T1_REPLAY_ALLOWED': raise RuntimeError('V478 oracle gate not passed')
    ...
    for n,(sym,items) in enumerate(grouped.items(),1):
        ...
        for seed in items:
            rows.append({**seed,'execution_contract':'NEXT_OPEN__DEEPEST_RAID_LOW_1PCT_SL__KNOWN_BSL_OR_TIME20__STRICT_T1__FEE0P2',**replay(seed,bars,highs)})
    ...
    row_file=OUT/'v479_frozen_t1_rows.csv'; fields=sorted({k for r in rows for k in r})
    with row_file.open('w',newline='') as h:
        w=csv.DictWriter(h,fieldnames=fields); w.writeheader(); w.writerows(rows)
    result={'version':'V479_DOUBLE_SSL_ABSORPTION_FROZEN_T1_REPLAY_NO_WRITE','generated_at':datetime.now().isoformat(timespec='seconds'),
      'no_write':True,'production_write':False,'frontend_write':False,'watchlist_write':False,
      'frozen_before_outcomes':{'entry':'next open after close above both raid highs','sl':'deepest raid low * 0.99','target':'nearest pre-entry confirmed 3L/3R swing high','exit':'strict T+1, target/SL/time20, gap-aware, collision=SL','fee_pct':FEE_PCT,'search_count':1,'promotion_gate':GATE},
      ...
      'promotion_gate_pass':passed,'decision':'DOUBLE_SSL_ABSORPTION_FROZEN_REPLAY_PASS__INDEPENDENT_METRIC_AUDIT_NEXT' if passed else 'DOUBLE_SSL_ABSORPTION_ECONOMIC_GATE_FAIL__INDEPENDENT_METRIC_AUDIT_THEN_CLOSE',
      'artifacts':{'out_dir':str(OUT),'rows':str(row_file),'latest':str(LATEST)}}
    text=json.dumps(result,ensure_ascii=False,indent=2); (OUT/'v479_report.json').write_text(text); LATEST.write_text(text); print(text)
