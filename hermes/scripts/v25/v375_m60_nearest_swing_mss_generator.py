#!/usr/bin/env python3
"""V375 no-write: causal 60m SSL→nearest-swing MSS→OB-retest generator.

This is a semantic correction to V374, not a parameter search: a bullish MSS
must break the most recent already-confirmed swing high, rather than the
highest high in a 60-bar window. A signal can use only closed bars before entry.
Invalid 60m source days are hard boundaries. Entry is the next 60m open only
after sweep, MSS, first POI touch, reclaim, and one hold bar. Exits begin only
on a later trading date (A-share T+1).
"""
...
OUT = AUDIT / f'v375_m60_nearest_swing_mss_no_write_{TS}'
LATEST = AUDIT / 'v375_m60_nearest_swing_mss_latest.json'
...
def main() -> None:
    ...
    with (OUT / 'v375_trades.csv').open('w', newline='') as h:
    ...
    report = {'version': 'V375_FULL_HISTORY_CAUSAL_60M_NEAREST_SWING_MSS_OB_RETEST_NO_WRITE', 'generated_at': datetime.now().isoformat(timespec='seconds'),
              'no_write': True, 'production_write': False, 'frontend_write': False, 'watchlist_write': False,
              'contract': 'confirmed 3/3 SSL sweep -> close breaks nearest prior confirmed swing high by 0.2% -> event-anchored bearish OB -> first touch -> reclaim -> hold -> next 60m open; raw 60m only; T+1 exits',
              ...
              'artifacts': {'trades': str(OUT / 'v375_trades.csv'), 'latest': str(LATEST)}}
    text = json.dumps(report, ensure_ascii=False, indent=2); (OUT / 'v375_report.json').write_text(text); LATEST.write_text(text); print(text)
