#!/usr/bin/env python3
"""
Proxy Guardian V8 — 全自动Mihomo代理监控守护进程
================================================
三重检测 + 自动节点切换 + 进程自动恢复 + 多目录状态同步

运行模式:
  python3 proxy_guardian_v8.py         守护模式 (每30秒)
  python3 proxy_guardian_v8.py --single 单次检测
  python3 proxy_guardian_v8.py --status 打印实时状态JSON
"""
...
CONFIG = '/home/lei/.clash_config_new.yaml'
...
# ── Logging ──
logger = logging.getLogger('proxy_guardian_v8')
logger.setLevel(logging.INFO)
handler = RotatingFileHandler(str(LOGS_DIR / 'proxy_guardian_v8.log'),
...
class ProxyGuardianV8:
    ...
    def check_connectivity(self):
        """三层连通性检测: gstatic → google → baidu (区分GFW vs 本地网络)"""
        ...
        gfw_diagnosis = not ok3  # Baidu也失败 → 本地网络问题
        return {
            'ok': False,
            'latency_ms': 0,
            'method': 'failed',
            ...
            'gfw_suspected': True if ok3 else False,  # baidu ok → GFW
    ...
    def get_proxy_groups(self):
        ...
        groups = self.query_mihomo('/groups')
        if groups and 'proxyGroups' in groups:
            for g in groups.get('proxyGroups', []):
                name = g.get('name', '')
                now = g.get('now', '')
                all_nodes = g.get('all', [])
                if name and all_nodes:
                    return name, now, all_nodes
            return None, None, []
    ...
    def switch_node(self, group_name, target_node=None):
        """通过API切换到指定节点或延迟最低节点"""
        if not group_name:
            return False
        ...
        if target_node and target_node in all_nodes:
            ...
            candidates = sorted(all_nodes,
                key=lambda n: (
                    proxies.get('proxies', {}).get(n, {})
                    .get('history', [{}])[-1].get('delay', 99999)
                )
            )
            # 过滤掉不可用节点
            candidates = [n for n in candidates
                if proxies.get('proxies', {}).get(n, {}).get('alive', False)]
    ...
    def restart_mihomo(self):
        """重启mihomo进程"""
        logger.warning("🔄 正在重启 mihomo...")
        try:
            subprocess.run(['pkill', '-9', '-x', 'mihomo'], capture_output=True, timeout=5)
            time.sleep(1)
        except:
            pass
            ...
            try:
                ...
                for retry in range(5):
                    ...
                    if proc_ok and port_ok:
                        logger.info(f"✅ mihomo 启动成功! PID={pid}")
    ...
    def write_status(self, state):
        ...
        status = {
            ...
            'source': 'proxy_guardian_v8',
            # 兼容旧API
            'all_ok': all([
                state.get('process_ok', False),
                state.get('port_ok', False),
                state.get('internet_ok', False),
            ]),
            'ok': all([
                state.get('process_ok', False),
                state.get('port_ok', False),
    ...
    def run(self):
        """主循环"""
        logger.info("=" * 50)
        logger.info("Proxy Guardian V8 启动")
            ...
            try:
                ...
                if all_ok:
                    ...
                    if self.fail_count >= MAX_FAIL_BEFORE_RESTART:
                        ...
                        if group_name:
                            logger.info(f"🔄 尝试切换节点 (组: {group_name})")
                        ...
                        # 节点切换失败 → 重启进程
                        logger.warning("🔄 节点切换无效, 重启 mihomo...")
        ...
        logger.info("Proxy Guardian V8 已停止")
