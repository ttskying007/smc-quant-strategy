#!/usr/bin/env python3
"""
Dual-engine backtest comparison framework.
Toggle SWITCH at line 13 to run V11 or V12 signals with same V467 exit logic.

Usage:
    python3 backtest_compare.py    # runs SWITCH=current on 200 stocks

Design:
    - Same entry logic (OB+FVG + cooldown + projected RR)
    - Same exit logic (V467 progressive BE + trailing)
    - Same swing calculation for TP
    - Produces comparable WR/RR/P&L metrics
"""
import sys, json, os, time, math
sys.path.insert(0, '/root/.hermes/scripts/v11')

# ── ENGINE SELECT ──────────────────────────────────────────────────
SWITCH = 'v11'  # 'v11' or 'v12'
# ──────────────────────────────────────────────────────────────────────
...
def extract_entries(sigs, ohlcv):
    """Normalize signal entries from either V11 or V12 format."""
    n = len(ohlcv)
    entries = []
    sw_high = []

    if 'OB_Bull' in sigs:  # V12 format
        ...
        for s in sigs.get('FVG_Bull', []):
            entries.append({'idx': s['idx'],
                            'price': (s.get('upper', 0) + s.get('lower', 0)) / 2,
                            'type': 'FVG', 'upper': s.get('upper', 0),
                            'lower': s.get('lower', 0),
                            'strength': s.get('strength', 0)})
        sw_high = sigs.get('swing_highs', [])

    elif 'ob' in sigs:  # V11 format
...
def backtest_stock(ohlcv, code, adaptive):
    n = len(ohlcv)
    sigs = detect_signals(ohlcv, {'adaptive': adaptive, 'require_volume': True})
    ...
    for e in entries:
        ...
        sl_hit = False; tp_hit = False; won = False
        best_price = entry_price; be_locked = False; sl_adj = sl_price

        for j in range(entry_bar + 1, n):
            bar = ohlcv[j]; low, high = bar['l'], bar['h']
            if low <= sl_adj:
                sl_hit = True; exit_price = sl_adj; won = False; break
            if high > best_price:
                best_price = high
            if proj_tp and high >= proj_tp and j > entry_bar:
                tp_hit = True; exit_price = proj_tp; won = True; break
        ...
        trades.append({
            ...
            'entry_type': e['type'], 'hold': j - entry_bar,
        })
        last_entry = i

    wins = sum(1 for t in trades if t['won']); total = len(trades)
...
def main():
    ...
    for idx, code in enumerate(stocks):
        ...
        try:
            ...
            results.append(r)
            print(f'[{idx+1}/200] {code}: {r["trades"]} trades, '
                  f'WR={r["win_rate"]}%, RR={r["avg_rr"]}x '
                  f'P&L={r["total_pnl"]}% ({time.time()-t0:.1f}s)', flush=True)
