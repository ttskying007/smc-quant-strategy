# -*- coding: utf-8 -*-
"""回填策略化结构分层：按 src/stage/adx 动态选择锚点指标（含 anchor_note）"""
import io, json, sys
...
for t in led:
    ...
    if sig8 not in dates:
        continue
    i = dates.index(sig8)
    st, _ = ps.stage_and_deep(bs, i) if i >= 91 else ("DOWNTREND", False)
    adx = ps.adx14_of(bs, i) or 0
    tp1, tp2, tp3, tp4, sl1, sl2, note = ps.structural_sltp(code, sig8, src='EVENT', stage=st, adx=adx)
    ...
    n += 1
ps.save_ledger(led)
print(f"回填 {n} 笔策略化分层（含 anchor_note）")
for t in led[:5]:
    print(f"  {t.get('code')} {t.get('name')} stage={t.get('stage')} TP1={t.get('tp1')} TP2={t.get('tp2')} TP3={t.get('tp3')} TP4={t.get('tp4')} SL1={t.get('sl1')} SL2={t.get('sl2')}")
    print(f"    note: {t.get('anchor_note','')[:80]}")
